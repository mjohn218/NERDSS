 ...
 * \ingroup Parser
 * \brief Parameter and related classes to hold simulation parameters provided
 * by the user
 ...
 * ### TODO List
 * ***
 * - Simulate until $X seconds parameter (automatically determines iterations
 * based on timestep)
 */
#pragma once
...
enum class ParamKeyword : int {
  ...
  numTotalSpecies = 1,  //!< total number of interfaces in the system, including
                        //!< states and product states
  nItr = 2,             //!< number of iterations requested
  fromRestart = 3,  //!< boolean for whether or not the simulation is starting
                    //!< from a restart file
  ...
  overlapSepLimit = 12,  //!< distance limit between centers of mass. if less
                         //!< than this for any proteins in a complex,
  //!< association is canceled
  name = 13,              //!< name of the simulation
  checkPoint = 14,        //!< interval to write checkpoint
  scaleMaxDisplace = 15,  //!< scalar of average displacement that is acceptable
                          //!< upon association.
...
enum class MolKeyword : int {
  ...
  isPoint = 11,      //!< is the Molecule a point (i.e. do all its interfaces
                     //!< overlap with the COM)
...
/*! \enum RxnKeyword
 * \ingroup Parser
 * \brief Reaction block keywords. For use with class RxnBase and its derived
 * classes.
 */
enum class RxnKeyword : int {
  ...
  norm1 = 4,  //!< norm of the first reactant, for use in the phi angle, if
              //!< applicable
  norm2 = 5,  //!< norm of the second reactant, for use in the phi angle, if
              //!< applicable
  sigma = 6,  //!< sigma vector (will eventually be changed to an integer)
  assocAngles = 7,  //!< angles for association
                    //!< [$\theta_1$,$\theta_2$,$\phi_1$,$\phi_2$,$\omega$]. See
                    //!< \ref association and Angles
  onMem = 8,        //!< boolean for if the reaction occurs only on the membrane
                    //!< (eventually change to detecting a lipid in
  //!< the reaction
  rate = 9,
  isCoupled = 10,     //!< is the reaction coupled to another reaction?
  isObserved = 11,    //!< not used, but set by the parser
  observeLabel = 12,  //!< label for the observable which tracks the product of
                      //!< the reaction
  bindRadSameCom =
      13,  //!< binding separaton to force association within same complex
  irrevRingClosure = 14,  //!< is ring closure irreversible, i.e. binding within
                          //!< complex has probability unity
  creationRadius = 15,  //!< the radius of the sphere around the parent molecule
                        //!< where a molecule is created
  loopCoopFactor = 16,  //!< the factor to multiple the loop closure rate, i.e.
                        //!< loopCoopFactor=exp(-dG/kT)
  ...
  excludeVolumeBound = 21,  //!< once two sites bound, still exclude their
                            //!< partners if this is set true
};
...
struct Parameters {
  ...
  struct Debug {
    ...
    int verbosity{0};
    /*
    Function serialize serializes the Debug info
    into array of bytes.
    */
    ...
    /*
    Function deserialize deserializes the Debug info
    from array of bytes.
    */
  ...
  //   int numIfaces { 0 }; //!< total number of interfaces on all MolTemplates.
  //   used to be nifaces
  int numTotalSpecies{0};  //!< total number of interfaces and states (including
                           //!< products!) possible in the system.
  long long int nItr{
      0};  //!< number of timesteps requested by user. used to be int Nit.
  int numTotalComplex{0};  //!< number of complexes in the system at start
  unsigned numTotalUnits{
      0};  //!< number of total molecules + interfaces in the system at start
  ...
  long long int itrRestartFrom{
      0};  //!< number of timesteps from when the simlation restart
  double timeRestartFrom{
      0.0};  //!< time from when the simulation restart (unit: s)

  static double dt;  //!< timestep, in microseconds.
  static std::vector<long long int>
      lastUpdateTransition;  //!< steps that last update transition matrix

  int numLipids{0};  //!< total number of lipids in the system

  double overlapSepLimit{0.1};  //!< in nm. COM-COM distance less than this is
                                //!< cancelled, for checkOverlap molecules
  ...
  double scaleMaxDisplace{
      100.0};  //!< in nm. defaults to a large number, which allows most moves
               //!< even of large magnitude upon association.
  ...
  bool fromRestart{false};  //!< is this simulation initialized from a restart
                            //!< file. used to be int restart
  long long int timeWrite{
      10};  //!< timestep interval to print timestep. used to be statwrite
  long long int trajWrite{10};  //!< timestep interval to write coordinates
                                //!< file. used to be configwrite
  long long int restartWrite{
      10};                       //!< timestep interval to write a restart file
  long long int pdbWrite{-1};    //!< interval to write pdb
  long long int checkPoint{-1};  //!< interval to write checkpoint
  long long int transitionWrite{
      -1};  //!< timestep interval to write transition matrix
  ...
  /*
  Function serialize serializes the Parameters
  into array of bytes.
  */
  void serialize(unsigned char *arrayRank, int &nArrayRank) {
    PUSH(rank);
    ...
    PUSH(dt);
    serialize_primitive_vector<long long int>(lastUpdateTransition, arrayRank,
                                              nArrayRank);
    PUSH(numLipids);
  ...
  /*
  Function deserialize deserializes the Parameters
  ...
  */
  void deserialize(unsigned char *arrayRank, int &nArrayRank) {
    ...
    POP(timeRestartFrom);
    POP(dt);
    deserialize_primitive_vector<long long int>(lastUpdateTransition, arrayRank,
                                                nArrayRank);
    POP(numLipids);
    POP(overlapSepLimit);
    POP(implicitLipid);
