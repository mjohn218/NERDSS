...
enum struct ReactionType : int {
  ...
  bimolecular = 1,        //!< bimolecular reaction. Can include either state or
                          //!< interaction changes, but not both at once.
  biMolStateChange = 2,
  zerothOrderCreation = 3,  //!< creation reaction from concentration (0 -> X).
  destruction = 4,     //!< destruction reactions. Destroys entire complex, not
                       //!< just interface.
...
struct RxnIface {
   ...
   * \ingroup Reactions
   * \brief Holds information on one reactant or product for a reaction
   * (RxnBase)
   */
  ...
  int relIfaceIndex{0};      //!< relative index of this reactant/product in
                             //!< MolTemplate.interfaceList
  char requiresState{'\0'};  //!< the state required for this reaction to occur
                             //!< (Interface::State.iden)
  bool requiresInteraction{
      false};  //!< does the interface need to be bound to something

  bool operator==(const Molecule::Iface&) const;
  friend std::ostream& operator<<(std::ostream& os, const RxnIface& oneIface);

  RxnIface() = default;
  explicit RxnIface(std::string ifaceName, int molTypeIndex, int absIfaceIndex,
                    int relIfaceIndex, char requiresState,
                    bool requiresInteraction);
  /*
  Function serialize serializes the RxnIface
  ...
  */
  ...
  /*
  Function deserialize deserializes the RxnIface
  from given arrayRank of bytes.
  */
...
struct RxnBase {
  /*! \ingroup Reactions
   * \brief Abstract class holding information applicable to all reactions
   * (ForwardRxn, BackRxn, and CreateDestructRxn)
   */

 public:
  struct RateState {
    /*! \struct RxnState
     * \brief Container for the different rates a reaction might have, dependent
     * on interface states, bound status, etc.
     */
    double rate{0.0};  //!< rate for this reaction state (if no conditional
                       //!< reactions, this is the only rate)
    double prob{0.0};  //!< reaction probability, only used for zeroth and first
                       //!< order reactions (constant)
    std::vector<std::vector<RxnIface>>
        otherIfaceLists{};  //!< list of ancillary interfaces which must be
                            //!< present for the reaction to occur

    RateState() = default;
    RateState(double rate,
              const std::vector<std::vector<RxnIface>>& otherIfaceList)
        : rate(rate), otherIfaceLists(otherIfaceList) {}
    /*
    Function serialize serializes the RateState
    into arrayRank of bytes.
    */
    void serialize(unsigned char* arrayRank, int& nArrayRank) {
      PUSH(rate);
      ...
      serialize_abstract_matrix<RxnIface>(otherIfaceLists, arrayRank,
                                          nArrayRank);
    }
    /*
    Function deserialize deserializes the RateState
    from given arrayRank of bytes.
    */
    void deserialize(unsigned char* arrayRank, int& nArrayRank) {
      POP(rate);
      POP(prob);
      deserialize_abstract_matrix<RxnIface>(otherIfaceLists, arrayRank,
                                            nArrayRank);
    }
  };
  ...
  struct CoupledRxn {
    ...
    CoupledRxn() = default;
    CoupledRxn(int _absRxnIndex) : absRxnIndex(_absRxnIndex) {}
    CoupledRxn(std::string _label) : label(_label) {}
    /*
    Function serialize serializes the RxnIface
    into arrayRank of bytes.
    */
  ...
  std::string observeLabel{};  //!< label under which the product is written to
                               //!< the observables file
  // below should be per each reaction rate.
  double loopCoopFactor{
      1.0};  //!< multiple the rate by this factor, used only when closing loops
  double bindRadSameCom{1.1};  //!< distance between two reactants to force
                               //!< reaction within the same complex
  /**< reactant and product arrays*/
  bool isSymmetric{false};     //!< are both reactants of the reaction identical
  ...
  ReactionType
      rxnType{};       //!< what is the type of this reaction (see ReactionType)
  int absRxnIndex{0};  //!< absolute index of the reaction
  int relRxnIndex{0};  //!< relative index of the reaction
  bool isCoupled{false};
  double length3Dto2D{
      -1};  //!< in nm, length scale to convert 3D rate to 2D rate, by default
            //!< will be set to 2* bindrad if not read in from file.
  std::string rxnLabel{
      "nonspecified"};  //!< label of the reaction, used for coupled; default
                        //!< value is nonspecified
  std::string coupledRxnLabel{
      "none"};  //!< lable of the coupled reaction, default value is none
  CoupledRxn coupledRxn;
  bool excludeVolumeBound{false};  //!< once two sites bound, still exclude
                                   //!< their partners if this is set true

  std::vector<int> intProductList{};   //!< list of absolute interface state
                                       //!< indices of the product(s)
  std::vector<int> intReactantList{};  //!< list of absolute interface state
                                       //!< indices of the reactant(s)

  static unsigned numberOfRxns;  //!< total number of unique ForwardRxns,
                                 //!< CreateDestructRxns, and ConditionalRates
  static int totRxnSpecies;  //!< total number of unique reactants and products

  std::vector<RxnIface>
      productListNew{};  //!< list of the product interfaces. indexed to  0 and
                         //!< 1, even for single products (enters duplicate as
                         //!< dummy)
  std::vector<RxnIface>
      reactantListNew{};  //!< list of the reactant interfaces. indexed to 0 and
                          //!< 1, even for single reactants (enters duplicate as
                          //!< dummy)

  std::vector<RateState> rateList{};  //!< list of rates for the reaction
  std::pair<RxnIface, RxnIface>
      stateChangeIface;  //!< interfaces which don't change interaction but
                         //!< change state
  /**< Contains rates dependent on different parameters, e.g. bound interfaces,
  ...
  */
  ...
  void deserialize_base(unsigned char* arrayRank, int& nArrayRank) {
    ...
    deserialize_abstract_vector<RxnIface>(productListNew, arrayRank,
                                          nArrayRank);
    deserialize_abstract_vector<RxnIface>(reactantListNew, arrayRank,
                                          nArrayRank);
...
struct ForwardRxn : public RxnBase {
 public:
  struct Angles {
     ...
     * \ingroup Reactions
     * \ingroup Associate
     * \brief Data structure for holding the association angles of a bimolecular
     * reaction.
     */
    ...
    double
        omega;  //!< (reactant 1 center of mass)-(reactant1
                //!< interface)-(sigma)-(reactant 2 center of mass)-(reactant 2
    //!< interface)
    ...
    Angles(double theta1, double theta2, double phi1, double phi2, double omega)
        : theta1(theta1),
          ...
          omega(omega) {}
    /*
    Function serialize serializes the Angles
    into array of bytes.
    */
    void serialize(unsigned char* arrayRank, int& nArrayRank) {
      PUSH(theta1);
    ...
    /*
    Function deserialize deserializes the Angles
  ...
  bool isReversible{
      false};  //!< is the reaction reversible (does it have a back reaction)
  int conjBackRxnIndex{
      -1};  //!< index of the ForwardRxn's BackRxn, if it is reversible
  bool irrevRingClosure{false};  //!< when true, reaction probability within
                                 //!< sigma in same complex is unity
  std::vector<RxnBase>::iterator
      conjBackRxn;  //!< iterator the reaction's BackRxn, if reversible (not
                    //!< implemented)
  std::string productName{
      "default"};  //!< the product as a string (just the reacting interfaces),
                   //!< for writing to species file
  //    std::string fullProductName {}; //!< the full product as written in the
  //    parameters file
  ...
  Vector norm1{0, 0, 1};  //!< arbitrary 'normal' vector determined from an
                          //!< ancillary interface for reactant 1
  Vector norm2{0, 0, 1};  //!< arbitrary 'normal' vector determined from an
                          //!< ancillary interface for reactant 2
  ...
  Angles assocAngles{
      ...
      std::numeric_limits<double>::quiet_NaN(),
      std::numeric_limits<double>::quiet_NaN(),
      std::numeric_limits<double>::quiet_NaN()};  //!< Angles relative to sigma
                                                  //!< for association
  ...
  ForwardRxn() = default;
  /*!
   * \brief This constructor takes a fully parsedRxn and converts it into a
   ForwardRxn for use during the simulation.
   */
  explicit ForwardRxn(ParsedRxn& parsedRxn,
                      const std::vector<MolTemplate>& molTemplateList);
  ForwardRxn(double bindRadius, const Angles& assocAngles)
      : bindRadius(bindRadius), assocAngles(assocAngles) {}
  ForwardRxn(double bindRadius, const Angles& assocAngles, const double& sigma,
             const Vector& norm1, const Vector& norm2)
      // TODO:ASK sigma? Other problems?
  ...
  /*
  Function serialize serializes the ForwardRxn
  into array of bytes.
  */
  void serialize(unsigned char* arrayRank, int& nArrayRank) {
...
struct BackRxn : public RxnBase {
   ...
   * \ingroup SimulClasses
   * \ingroup Reactions
   * \brief Holds information on the back reaction counterpart to a reversible
   * ForwardRxn
   *
   * This can be bimolecular or unimolecular
   */

  size_t conjForwardRxnIndex{
      0};  //!< the index of this reaction's ForwardRxn counterpart
  std::vector<RxnBase>::iterator
      conjForwardRxn;  //!< iterator to this reaction's ForwardRxn (not
                       //!< implemented)
  ...
  /*!
   * \brief This constructor creates a conjugate BackRxn from a reversible
   * ForwardRxn
   */
  ...
  void serialize(unsigned char* arrayRank, int& nArrayRank) {
    ...
    // std::vector<RxnBase>::iterator conjForwardRxn; //!< iterator to this
    // reaction's ForwardRxn (not implemented) PUSH(conjForwardRxn);
  }
  void deserialize(unsigned char* arrayRank, int& nArrayRank) {
    this->deserialize_base(arrayRank, nArrayRank);
    ...
    // std::vector<RxnBase>::iterator conjForwardRxn; //!< iterator to this
    // reaction's ForwardRxn (not implemented) POP(conjForwardRxn);
  }
};
...
struct CreateDestructRxn : public RxnBase {
   ...
   *
   * ## Limitations
   *   - Currently only allows the creation of the default product from
   * MolTemplate
   *   - Involved Molecules which have Interface States must have them
   * explicitly included.
  ...
  struct CreateDestructMol {
    ...
    CreateDestructMol(int _molTypeIndex,
                      const std::vector<RxnIface>& _interfaceList)
        : molTypeIndex(_molTypeIndex), interfaceList(_interfaceList) {}
    void serialize(unsigned char* arrayRank, int& nArrayRank) {
      PUSH(molTypeIndex);
    ...
    void deserialize(unsigned char* arrayRank, int& nArrayRank) {
      ...
      deserialize_abstract_vector<RxnIface>(interfaceList, arrayRank,
                                            nArrayRank);
    }
  };
  ...
  double creationRadius{1.0};  //!< the radius of the sphere within which the
                               //!< created specie can be placed
  ...
  /*!
   * \brief This function takes a fully parsed reaction block and converts it to
   * a CreateDestructRxn for use during the simulation
   */
  explicit CreateDestructRxn(ParsedRxn& parsedRxn,
                             const std::vector<MolTemplate>& molTemplateList);
  ...
  void serialize(unsigned char* arrayRank, int& nArrayRank) {
    ...
    serialize_abstract_vector<CreateDestructMol>(reactantMolList, arrayRank,
                                                 nArrayRank);
    serialize_abstract_vector<CreateDestructMol>(productMolList, arrayRank,
                                                 nArrayRank);
    PUSH(conjBackRxnIndex);
    PUSH(creationRadius);
  }
  void deserialize(unsigned char* arrayRank, int& nArrayRank) {
    ...
    deserialize_abstract_vector<CreateDestructMol>(reactantMolList, arrayRank,
                                                   nArrayRank);
    deserialize_abstract_vector<CreateDestructMol>(productMolList, arrayRank,
                                                   nArrayRank);
    POP(conjBackRxnIndex);
    POP(creationRadius);
  }
