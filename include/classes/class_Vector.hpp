...
struct Vector : public Coord {
  ...
  Vector operator/(double val) { return {x / val, y / val, z / val}; }
  Vector operator+(const Coord& crd) {
    return {this->x + crd.x, this->y + crd.y, this->z + crd.z};
  }
  ...
  /*!
   * \brief Calculates the magnitude of the Vector. Note that this is no longer
   * done when the Vector is constructed.
   */
  void calc_magnitude();
  ...
  /*!
   * \brief Returns a Vectorwhich is the crossproduct of the Vector with another
   * Vector. \params[in] this Vector 1. \params[in] vec Vector 2.
   */
  ...
  double dot(const Vector& vec)
      const;  //!< returns the dot product of the vector and another
   ...
   */
  double dot_theta(
      const Vector& vec) const;  //!< returns the angle between two vectors
