...
enum class ObservableType {
  ...
  sum = 4,  //!< perform an addition of observables. only performed once per
            //!< timeWrite interval
};

/*! \ingroup SpeciesTracker
 * \brief Holds information on an Observable provided by the user in an
 * observables block in the parameters file.
 *
 * TODO: function to check complex against Observable at points where molecules
 * and complexes change
 */
struct Observable {
  /*!
  ...
  struct Constituent {
    struct Iface {
      ...
      Iface() = default;
      Iface(unsigned _relIndex, unsigned _absIndex, char _state, bool _isBound,
            int _bondIndex)
          : relIndex(_relIndex),
            absIndex(_absIndex),
            state(_state),
  ...
  std::vector<Constituent>
      constituentList;  //!< Molecules which compose the Observable
  ...
  std::array<int, 2>
      coupledRxn;  //!< type (0 = forward, 1 = back, 2 = createdestruct) and
                   //!< index of reaction which forms this Observable
