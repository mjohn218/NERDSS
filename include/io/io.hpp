...
/*! \defgroup IO
 * \brief Functions for input/output of coordinates, parameters, trajectories,
 * etc.
 */
...
void write_crds(std::string name, const std::vector<Complex>& Complexlist,
...
/*!
 * \brief Writes the coordinates of all Molecules in the system to an XYZ
 * coordinate file.
 */
...
/*! \ingroup IO
 * \brief Writes/appends the coordinates of all Molecules in the system to the
 * trajectory file.
 */
...
/*! \ingroup IO
 * \brief Writes/appends the transition matrix in the system to the transition
 * matrix file.
 */
void write_transition(double iter, std::ofstream& transitionFile,
                      const std::vector<MolTemplate>& molTemplateList);

/*! \ingroup IO
 * \brief Debugging IO function to write an xyz file for a associating
 * complexes.
 */
void write_xyz_assoc(std::string filename, const Complex& reactCom1,
 ...
 */
void write_complex_crds(std::string name, const Complex& complex1,
                        const Complex& complex2,
                        std::vector<Molecule>& moleculeList);

/*! \ingroup IO
 * \brief Writes out timestep information, such as the different molecular types
 * in the system.
 *
 * TODO: This description needs more information
 */
void write_timestep_information(long long int simItr, std::ofstream& outFile,
...
/*! \ingroup IO
 * \brief Writes the number of each component Molecule in all complexes at the
 * current timestep
 */
void write_complex_components(long long int simItr, std::ofstream& complexFile,
                              const Parameters& params,
                              std::vector<Molecule>& moleculeList,
                              std::vector<Complex>& complexList,
                              const std::vector<MolTemplate>& molTemplateList);
...
/*! \ingroup IO
 * \brief Writes a plain text restart file at intervals specified in the
 * Parameters file.
 *
 * This is a formatted text file, which is essentially illegible to the user,
 * but it's not like they'd need to look at it anyway.
 */
 ...
 */
void init_print_dimers(std::ofstream& outfile, Parameters params,
                       std::vector<MolTemplate>& molTemplateList);
...
/*! \ingroup IO
 * \brief Nbound pairs are counting all directly bound pairs of protein A and
 * partner B. Does not matter if A or B are also bound via other interfaces to
 * other proteins.
 */
void init_NboundPairs(copyCounters& counterArray, std::ofstream& outfile,
                      Parameters params,
                      std::vector<MolTemplate>& molTemplateList,
                      std::vector<Molecule>& moleculeList);

/*! \ingroup IO
 * \brief write: Nbound pairs are counting all directly bound pairs of protein A
 * and partner B. Does not matter if A or B are also bound via other interfaces
 * to other proteins.
 */
...
/*! \ingroup IO
 * \brief print calculated histogram of distinct types of complexes, based on
...
/*! \ingroup IO
 * \brief initialize array of counterArrays.copyNumSpecies, based on the initial
 * molecule Species and their interface States.
 */
...
void write_all_species(double simTime, std::vector<Molecule>& moleculeList,
                       std::ofstream& speciesFile, copyCounters& counterArray,
                       const Membrane& membraneObject, MpiContext& mpiContext,
                       SimulVolume& simulVolume);

/*! \ingroup IO
 * \brief Prints out information about all species in the system (Molecules and
 * Complexes)
 */
void print_system_information(long long int simItr, std::ofstream& systemFile,
                              const std::vector<Molecule>& moleculeList,
                              const std::vector<Complex>& complexList,
                              const std::vector<MolTemplate>& molTemplateList);
