 ...
 *
 * Returns boolean for if the mol1 COM - mol2 COM vector is shorter than the
 * combined radii of the two molecules before spending the time determining if
 * their interfaces ovelap.
 */
 ...
 *
 * ## Algorithm
 *   1. Extrapolate along the mol1 iface - mol2 iface vector using each
 * molecule's interface, to see if they even can be overlapping
 */
...
/*!
 * \ingroup SystemSetup
 * \brief Sets the RMax for the simulation, e.g. the maximum distance any
 * protein can diffuse per timestep. \param[in] params Simulation Parameters as
 * provided by user. \param[in] moleculeList List of all Molecules in the
 * system. \param[in] forwardRxns List of all provided ForwardRxns.
 *
...
// void create(const MolTemplate& oneTemp, std::vector<int>& emptyMolList,
// std::vector<int>& emptyComList, 	    std::vector<Molecule>& moleculeList,
// std::vector<Complex>& complexList);
...
// functions to generate new added molecules and complexes fo a restart
// simulation
