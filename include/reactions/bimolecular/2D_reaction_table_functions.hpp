...
size_t size_lookup(double bindRadius, double Dtot, const Parameters& params,
                   double Rmax);

/*! \ingroup 2DReactions
 * \brief Main function for 2D reaction tables
 */
void create_DDMatrices(gsl_matrix*& survMatrix, gsl_matrix*& normMatrix,
                       gsl_matrix*& pirMatrix, double bindRadius, double Dtot,
                       double comRMax, double ktemp, const Parameters& params);
 ...
 */
double get_prevNorm(gsl_matrix* normMatrix, double RStepSize, double r0,
                    double bindRadius);

/*! \ingroup 2DReactions
 * \brief 2D lookup table for psurv
 */
double get_prevSurv(const gsl_matrix* survMatrix, double Dtot, double deltaT,
...
double pirr_pfree_ratio_psF(double rCurr, double r0, double tCurr, double Dtot,
                            double bindrad, double alpha, double ps_prev,
                            double rtol);

/*!
 * \brief 2D lookup table for pirr
 */
double calc_pirr(gsl_matrix* pirMatrix, gsl_matrix* survMatrix,
                 double RStepSize, double r, double r0, double a);

/* INTEGRATION FUNCTIONS */
/*! \ingroupd 2DReactions
 * \brief Integrator for the functions involved in the creation of the 2D
 * reaxtion matrix creation
 */
 ...
 */
void create_normMatrix(gsl_matrix*& normMatrix, double bindRadius, double Dtot,
                       double kr, double comRMax, double RStepSize,
                       const Parameters& params);
