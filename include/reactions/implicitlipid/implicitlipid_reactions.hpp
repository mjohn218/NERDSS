...
/*!
 * \brief Gets the distance between one Molecule's Interface and the membrane
 * surface, and determines if it is within Rmax, and can therefore react.
 */
bool get_distance_to_surface(int pro1, int pro2, int iface1, int iface2,
                             int rxnIndex, int rateIndex,
                             bool isStateChangeBackRxn, double& sep, double& R1,
                             double Rmax, std::vector<Complex>& complexList,
                             const ForwardRxn& currRxn,
                             std::vector<Molecule>& moleculeList,
                             const Membrane& membraneObject);

// binding probabilities for 3D and 2D
// h is the time-step dt; sigma is the bind_radius, rho is the lipid density on
// the surface.
