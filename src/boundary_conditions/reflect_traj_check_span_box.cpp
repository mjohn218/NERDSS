...
void reflect_traj_check_span_box(const Parameters& params, Complex& targCom,
  ...
  while (checkItr < maxItr && needsRecheck) {
    ...
    for (auto& memMol : targCom.memberList) {
      ...
      if (currX > posWallX) posWallX = currX;  // farthest point in +X
      if (currX < negWallX) negWallX = currX;  // farthest point in -X

      if (currY > posWallY) posWallY = currY;  // farthest point in +Y
      if (currY < negWallY) negWallY = currY;  // farthest point in -Y

      if (currZ > posWallZ) posWallZ = currZ;  // farthest point in +Z
      if (currZ < negWallZ) negWallZ = currZ;  // farthest point in -Z
    ...
    if (outsideBox) {
      ...
      if (outsideNegX && outsidePosX) {
        // For a large complex, test if checkItr could be pushed back out the
        // other side std::cout << " extends in both directions of X. negwall "
        // << negWallX
        //           << " poswall: " << posWallX << '\n';
      ...
      if (outsideNegX && !outsidePosX) {
        ...
        if (posWallX - 2.0 * (negWallX - negX) > posX) {
          moveFailed = true;
          // std::cout << " Will push out the other side. negwall " << negWallX
          // << " poswall: " << posWallX
      ...
      if (outsideNegY && outsidePosY) {
        // For a large complex, test if checkItr could be pushed back out the
        // other side std::cout << " extends in both directions of Y. negwall "
        // << negWallY
        //           << " poswall: " << posWallY << '\n';
      ...
      if (outsideNegY && !outsidePosY) {
        ...
        if (posWallY - 2.0 * (negWallY - negY) > posY) {
          moveFailed = true;
          // std::cout << " Will push out the other side. negwall " << negWallY
          // << " poswall: " << posWallY
      ...
      if (outsideNegZ && outsidePosZ) {
        // For a large complex, test if checkItr could be pushed back out the
        // other side std::cout << " extends in both directions of Z. negwall "
        // << negWallZ
        //           << " poswall: " << posWallZ << '\n';
      ...
      if (outsideNegZ && !outsidePosZ) {
        ...
        if (posWallZ - 2.0 * (negWallZ - negZ) > posZ) {
          moveFailed = true;
          // std::cout << " Will push out the other side. negwall " << negWallZ
          // << " poswall: " << posWallZ
    ...
    if (moveFailed == true) {
      ...
      targCom.trajTrans.x =
          sqrt(2.0 * params.timeStep * targCom.D.x) * GaussV();
      targCom.trajTrans.y =
          sqrt(2.0 * params.timeStep * targCom.D.y) * GaussV();
      targCom.trajTrans.z =
          sqrt(2.0 * params.timeStep * targCom.D.z) * GaussV();
      ...
      ++checkItr;
      needsRecheck =
          true;  // will need to recheck after resampling traj and trajR
    }
  ...
  if (needsRecheck) {
    ...
    for (auto memMol : targCom.memberList) {
      ...
      std::cout << "i: " << checkItr << " P: " << memMol << " com:"
                << targCom.comCoord.x + targCom.trajTrans.x + rotComVec.x << ' '
