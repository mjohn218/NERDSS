...
void reflect_traj_complex_rad_rot_sphere(const Parameters& params,
  ...
  // reflecting-surface RS3D for explicit-lipid model, membraneObject.RS3D = 0.
  // And, for those proteins that are bound on surface, they are not allowed to
  // reflect along Z-axis for the complex on the surface, its propagation is
  // different from those inside the sphere
  double RS3D;
  ...
  /*first just test Complex COM+ radius, if it fits inside
    membraneObject.sphereR-RS3D. if yes, then test if all interfaces inside
    sphereR. then, if they are outside, what is max displacement beyond the
    sphereR. move it along the radial direction inside by the displacement.
  ...
  if (recheck) {
    ...
    // THAT COMPLEX DOES NOT SPAN SPHERE IN SUBROUTINE
    // REFLECT_TRAJ_COMPLEX_RAD_ROT. Complex: "
    //           << targCom.index << " size:" << targCom.memberList.size() <<
    //           '\n';
    reflect_traj_check_span(params, targCom, moleculeList, membraneObject,
                            RS3Dinput);
  }
}