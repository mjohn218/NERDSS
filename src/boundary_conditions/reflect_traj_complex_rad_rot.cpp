...
void reflect_traj_complex_rad_rot(const Parameters& params,
  ...
  if (membraneObject.isSphere == true)
    ...
    reflect_traj_complex_rad_rot_box(params, moleculeList, targCom,
                                     membraneObject, RS3Dinput);

  // // NOTE: it only works for a box system with the membrane surface located
  // on the Z-bottom.

  // // for implicit-lipid model, the boundary surface must consider the
  // reflecting-surface RS3D
  // // for explicit-lipid model, RS3D = 0.
  // double RS3D = RS3Dinput;
  // //    std::cout <<" RS3D: "<<membraneObject.RS3D<<std::endl;
  // /*This routine updated March 2017 to test if a large complex that spans the
  // box could extend out in both directions if so, it attempts to correct for
  // this by resampling the complex's translational and rotational updates.
  // */
  // double xtot { 0 };
  // double ytot { 0 };
  // double ztot { 0 };
  ...
  // /*This is to test based on general size if it is close to boundaries,
  // before doing detailed evaluation below.*/

  // bool canBeOutsideX { false };
  // double xchg = currx - membraneObject.waterBox.x / 2.0;
  // if ((xchg + targCom.radius) > 0 || (xchg - targCom.radius) <
  // -membraneObject.waterBox.x)
  //     canBeOutsideX = true;

  // bool canBeOutsideY { false };
  // double ychg = curry - membraneObject.waterBox.y / 2.0;
  // if ((ychg + targCom.radius) > 0 || (ychg - targCom.radius) <
  // -membraneObject.waterBox.y)
  //     canBeOutsideY = true;

  // bool canBeOutsideZ { false };
  // double zchg = currz - membraneObject.waterBox.z / 2.0;
  // if ((zchg + targCom.radius) > 0 || (zchg - targCom.radius) <
  // -(membraneObject.waterBox.z - RS3D))
  //     canBeOutsideZ = true;
  ...
  //         double dxrot { row[0] * comVec.x + row[1] * comVec.y + row[2] *
  //         comVec.z }; currx = targCom.comCoord.x + targCom.trajTrans.x +
  //         dxrot;

  //         if (membraneObject.waterBox.x / 2.0 - currx < posWall)
  //             posWall = membraneObject.waterBox.x / 2.0 - currx; // shortest
  //             distance in x from the +wall
  //         if (currx + membraneObject.waterBox.x / 2.0 < negWall)
  //             negWall = currx + membraneObject.waterBox.x / 2.0; // shortest
  //             distance in x from the -wall
  ...
  //             dxrot = row[0] * ifaceVec.x + row[1] * ifaceVec.y + row[2] *
  //             ifaceVec.z; currx = targCom.comCoord.x + targCom.trajTrans.x +
  //             dxrot; if (membraneObject.waterBox.x / 2.0 - currx < posWall)
  //                 posWall = membraneObject.waterBox.x / 2.0 - currx; //
  //                 shortest distance in x from the +wall
  //             if (currx + membraneObject.waterBox.x / 2.0 < negWall)
  //                 negWall = currx + membraneObject.waterBox.x / 2.0; //
  //                 shortest distance in x from the -wall
  ...
  //             /*For a large complex, test if it could be pushed back out the
  //             other side*/ std::cout << "xtot: " << xtot << " but also
  //             extends in other direction. negwall " << negWall
  //                       << " poswall: " << posWall << '\n';
  //             recheck = true;
  //         } else if (outsideNeg && posWall < (2.0 * xtot)) {
  //             /*Also need to check that update will not push you out the
  //             other side*/
  //             // xtot is positive.
  //             recheck = true;
  //             std::cout << " Will push out the other side. negwall " <<
  //             negWall << " poswall: " << posWall
  //                       << " 2*xtot: " << 2.0 * xtot << '\n';
  ...
  //         double dyrot = row[0] * comVec.x + row[1] * comVec.y + row[2] *
  //         comVec.z; curry = targCom.comCoord.y + targCom.trajTrans.y + dyrot;
  //         if (membraneObject.waterBox.y / 2.0 - curry < poswall)
  //             poswall = membraneObject.waterBox.y / 2.0 - curry; // shortest
  //             distance in y from the +wall
  //         if (curry + membraneObject.waterBox.y / 2.0 < negwall)
  //             negwall = curry + membraneObject.waterBox.y / 2.0; // shortest
  //             distance in y from the -wall
  ...
  //             /*only need y component*/
  //             dyrot = row[0] * ifaceVec.x + row[1] * ifaceVec.y + row[2] *
  //             ifaceVec.z; curry = targCom.comCoord.y + targCom.trajTrans.y +
  //             dyrot; if (membraneObject.waterBox.y / 2.0 - curry < poswall)
  //                 poswall = membraneObject.waterBox.y / 2.0 - curry; //
  //                 shortest distance in y from the +wall
  //             if (curry + membraneObject.waterBox.y / 2.0 < negwall)
  //                 negwall = curry + membraneObject.waterBox.y / 2.0; //
  //                 shortest distance in y from the -wall
  ...
  //             /*For a large complex, test if it could be pushed back out the
  //             other side*/ std::cout << "ytot: " << ytot << " but also
  //             extends in other direction. negwall " << negwall
  //                       << " poswall: " << poswall << '\n';
  //             recheck = true;
  ...
  //         double dzrot = row[0] * comVec.x + row[1] * comVec.y + row[2] *
  //         comVec.z; currz = targCom.comCoord.z + targCom.trajTrans.z + dzrot;
  //         if (membraneObject.waterBox.z / 2.0 - currz < poswall)
  //             poswall = membraneObject.waterBox.z / 2.0 - currz; // shortest
  //             distance in z from the +wall
  //         if (currz + membraneObject.waterBox.z / 2.0 - RS3D < negwall)
  //             negwall = currz + membraneObject.waterBox.z / 2.0 - RS3D; //
  //             shortest distance in z from the -wall
  ...
  //             dzrot = row[0] * ifaceVec.x + row[1] * ifaceVec.y + row[2] *
  //             ifaceVec.z; currz = targCom.comCoord.z + targCom.trajTrans.z +
  //             dzrot;

  //             if (membraneObject.waterBox.z / 2.0 - currz < poswall)
  //                 poswall = membraneObject.waterBox.z / 2.0 - currz; //
  //                 shortest distance in z from the +wall
  //             if (currz + membraneObject.waterBox.z / 2.0 - RS3D < negwall)
  //                 negwall = currz + membraneObject.waterBox.z / 2.0 - RS3D;
  //                 // shortest distance in z from the -wall
  ...
  //         /*Put back inside the box*/
  //         if (targCom.D.z == 0)
  //             std::cout << "Putting complex " << targCom.index << " with ztot
  //             " << ztot
  //                       << ", zchg " << (targCom.comCoord.z +
  //                       targCom.trajTrans.z) - membraneObject.waterBox.z
  //                       / 2.0
  //                       << ", currz " << targCom.comCoord.z +
  //                       targCom.trajTrans.z << " and trajTrans.z "
  //                       << targCom.trajTrans.z << " back in box." << '\n';
  //         targCom.trajTrans.z += 2.0 * ztot;
  //         if (outsideNeg && outsidePos) {
  //             /*For a large complex, test if it could be pushed back out the
  //             other side*/ std::cout << "ztot: " << ztot << " but also
  //             extends in other direction. negwall " << negwall
  //                       << " poswall: " << poswall << '\n';
  //             recheck = true;
  //         } else if (outsideNeg && poswall < (2.0 * ztot)) {
  //             /*Also need to check that update will not push zou out the
  //             other side*/
  //             // ztot is positive.
  //             recheck = true;
  //             std::cout << " Will push out the other side. negwall " <<
  ...
  //     /*Test that new coordinates have not pushed you out of the box for a
  //     very large complex, if so, resample
  //      * rotation matrix.*/
  //     std::cout << "RECHECK THAT COMPLEX DOES NOT SPAN BOX IN SUBROUTINE
  //     REFLECT TRAJ COMPLEX RAD ROT. Complex: "
  //               << targCom.index << " size:" << targCom.memberList.size() <<
  //               '\n';
  //     reflect_traj_check_span(xtot, ytot, ztot, params, targCom,
  //     moleculeList, M, membraneObject, RS3Dinput);
  // }
}
