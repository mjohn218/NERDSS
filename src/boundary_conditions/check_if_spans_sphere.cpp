...
void check_if_spans_sphere(bool& cancelAssoc, const Parameters& params,
  ...
  double sphereR =
      membraneObject.sphereR;  // no considering the reflecting-surface, because
                               // here we are checking whether to span the box

  // find the new COM and new radius, to check whether the new radius is larger
  ...
  for (auto& memMol : reactCom1.memberList) {
    if (moleculeList[memMol].isImplicitLipid) continue;

    Vector disVec{moleculeList[memMol].tmpComCoord - newCom};
    disVec.calc_magnitude();
    if (disVec.magnitude > newRadius) newRadius = disVec.magnitude;
  ...
  // std::cout << "CHECK SPHERE SPAN. Complex 1 Radius, Complex 2 radius " <<
  // reactCom1.radius << ' ' << reactCom2.radius << '\n'; The approximate size
  // of the complex (max size) puts it as outside, now test interface positions.
  bool outside{false};
  ...
  if (outside == true) {
    ...
    for (int memMol : reactCom1.memberList) {
      ...
      for (auto& iface : moleculeList[memMol].tmpICoords) iface += trans;
    }
