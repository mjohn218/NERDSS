...
void reflect_traj_complex_rad_rot_nocheck(const Parameters& params,
  ...
  if (membraneObject.isSphere == true)
    reflect_traj_complex_rad_rot_nocheck_sphere(params, targCom, moleculeList,
                                                membraneObject, RS3Dinput);
  else
    reflect_traj_complex_rad_rot_nocheck_box(params, targCom, moleculeList,
                                             membraneObject, RS3Dinput);
}
...
//         // these need to be what current positions due to translation and
//         rotation are for (auto& memMol : targCom.memberList) {
...
//             comVec.z; curry = targCom.comCoord.y + targCom.trajTrans.y +
//             dyrot;
...
//                 // only need y component
//                 dyrot = (row[0] * ifaceVec.x) + (row[1] * ifaceVec.y) +
//                 (row[2] * ifaceVec.z); curry = targCom.comCoord.y +
//                 targCom.trajTrans.y + dyrot;
...
//             comVec.z; currz = targCom.comCoord.z + targCom.trajTrans.z +
//             dzrot;
