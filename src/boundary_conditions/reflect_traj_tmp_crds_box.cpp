...
void reflect_traj_tmp_crds_box(const Parameters& params,
                               ...
                               double RS3Dinput) {
  // TRACE();
  /*This routine updated October 2019 to test if a large complex that spans the
  box could extend out in both directions if so, it attempts to correct for this
  by resampling the complex's translational and rotational updates.
  */
  double RS3D;
  ...
  if (canBeOutsideX) {
    ...
    for (auto& memMol : targCom.memberList) {
      ...
      if (currx > posWall) posWall = currx;  // farthest point in +X
      if (currx < negWall) negWall = currx;  // farthest point in -X
    ...
    if (outside) {
      ...
      if (outsideNeg && outsidePos) {
        ...
        // side std::cout << " extends in both directions of X. negwall " <<
        // negWall
        //           << " poswall: " << posWall << '\n';
        recheck = true;
      }
      if (outsideNeg && !outsidePos) {
        ...
        if (posWall - 2.0 * (negWall - negX) > posX) {
          recheck = true;
          // std::cout << " Will push out the other side. negwall " << negWall
          // << " poswall: " << posWall
          //           << "traj[0]: " << traj[0] << '\n';
        }
      }
  ...
  if (canBeOutsideZ > 0) {
    ...
    if (outside) {
      ...
      membraneObject.waterBox.z / 2.0
                    << ", currz " << targCom.tmpComCoord.z + traj[2] << " and
      traj[2] "
                    << traj[2] << " back in box." << '\n';
      */
      if (outsideNeg && outsidePos) {
