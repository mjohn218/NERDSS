...
void reflect_complex_rad_rot_sphere(const Membrane& membraneObject,
  ...
  if ((currR + targCom.radius) > sphereR) canBeOutsideX = true;

  if (canBeOutsideX == true) {
    // find the farthest point
    Coord targcrds{0, 0, sphereR};  // to store the furthest point' coords.
    double rtmp =
        sphereR;  // to store the furthest point' distance to the sphere center.
    ...
    while (outside == true) {
      ...
      if (times > 100) {
        // so many times reflection still cannot make the complex back inside
        // the sphere, thus we may need report 'WRONG!!'
        std::cout << "ALREADY UPDATED POSITIONS.BUT, IN REFLECT "
                     "COMPLEX_RAD_ROT_SPHERE, 100 TIMES REFLECTIONS CAN'T MOVE "
                     "THE COMPLEX BACK INSIDE SPHERE."
                  << '\n';
        std::cout << "COMPLEX " << targCom.index << ", COMPLEX COM "
                  << targCom.comCoord.x << ", " << targCom.comCoord.y << ", "
                  << targCom.comCoord.z << '\n';
        std::cout << "COMPLEX RADIUS " << targCom.radius << ", COMPLEX SIZE "
                  << targCom.memberList.size() << '\n';
        std::cout << "EXITING..." << '\n';
        exit(1);
      }
    }  // end of while-loop
  ...
  // ALSO, the position update may cause lipids off sphere, so adjust lipids
  // back onto surface,
  int molnumber = -1;
  double dr = 0.0;
  Coord dtrans;
