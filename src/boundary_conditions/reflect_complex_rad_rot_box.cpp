...
void reflect_complex_rad_rot_box(const Membrane& membraneObject,
  ...
  if (canBeOutsideX) {
    ...
    for (auto& memMol : targCom.memberList) {
      ...
      if (currx > posWall) posWall = currx;
      if (currx < negWall) negWall = currx;
    ...
    if (posWall > posX) {
      ...
      outsidePos = true;
      // TODO: nenadko:
      // ACTION_COM_SHOULD_KNOW outsidePos
      posdx = posWall - posX;
    }
    if (negWall < negX) {
      outside = true;
      outsideNeg = true;
      // TODO: nenadko:
      // ACTION_COM_SHOULD_KNOW outsideNeg
      negdx = negWall - negX;
    }
    if (outside) {
      if (outsideNeg && outsidePos) {
        ...
        std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in BOTH directions of "
                     "X . ALREADY UPDATED POSITIONS. EXITING..."
                  << '\n';
        exit(1);
      }
      if (outsideNeg && !outsidePos) {
        if (posWall - 2.0 * negdx > posX) {
          std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND in "
                       "NEGATIVE side of X: try to put back in the box, x: "
        ...
        // update positions.Put back inside the box
        // TODO: nenadko:
        // ACTION_FROM_COM_TO_UPDATE_BASED_ON_HIS_DECISION
      ...
      if (!outsideNeg && outsidePos) {
        if (negWall - 2.0 * posdx < negX) {
          std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND in "
                       "POSITIVE side of X: try to put back in the box, x: "
  ...
  if (canBeOutsideY) {
    ...
    for (auto& memMol : targCom.memberList) {
      ...
      if (curry > posWall) posWall = curry;
      if (curry < negWall) negWall = curry;
      // measure each interface
      for (auto& iface : moleculeList[memMol].interfaceList) {
        curry = iface.coord.y;
        if (curry > posWall) posWall = curry;
    ...
    if (posWall > posY) {
      ...
      outsidePos = true;
      // TODO: nenadko:
      // ACTION_COM_SHOULD_KNOW outsidePosY
      posdy = posWall - posY;
    }
    if (negWall < negY) {
      outside = true;
      outsideNeg = true;
      // TODO: nenadko:
      // ACTION_COM_SHOULD_KNOW outsideNegY
      negdy = negWall - negY;
    }
    if (outside) {
      if (outsideNeg && outsidePos) {
        ...
        //  extends out both the front and back.
        std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in BOTH directions of "
                     "Y . ALREADY UPDATED POSITIONS. EXITING..."
                  << '\n';
        exit(1);
      }
      if (outsideNeg && !outsidePos) {
        if (posWall - 2.0 * negdy > posY) {
          std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND in "
                       "NEGATIVE side of Y: try to put back in the box, y: "
        ...
        // update positions.Put back inside the box
        // TODO: nenadko:
        // ACTION_FROM_COM_TO_UPDATE_BASED_ON_HIS_DECISION_COMES_AT_THE_END_OF_ITERATION
      ...
      if (!outsideNeg && outsidePos) {
        if (negWall - 2.0 * posdy < negY) {
          std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND in "
                       "POSITIVE side of Y: try to put back in the box, y: "
  ...
  if (canBeOutsideZ) {
    ...
    for (auto& memMol : targCom.memberList) {
      ...
      if (currz > posWall) posWall = currz;
      if (currz < negWall) negWall = currz;
      // measure each interface
      for (auto& iface : moleculeList[memMol].interfaceList) {
        currz = iface.coord.z;
        if (currz > posWall) posWall = currz;
    ...
    if (outside) {
      if (outsideNeg && outsidePos) {
        // extends out both the front and back.
        std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in BOTH directions of "
                     "Z . ALREADY UPDATED POSITIONS. EXITING..."
                  << '\n';
        exit(1);
      }
      if (outsideNeg && !outsidePos) {
        if (posWall - 2.0 * negdz > posZ) {
          std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND in "
                       "NEGATIVE side of Z: try to put back in the box, z: "
      ...
      if (!outsideNeg && outsidePos) {
        if (negWall - 2.0 * posdz < negZ) {
          std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND in "
                       "POSITIVE side of Z: try to put back in the box, z: "
