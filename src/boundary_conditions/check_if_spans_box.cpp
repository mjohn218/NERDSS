...
void check_if_spans_box(bool& cancelAssoc, const Parameters& params,
  ...
  // TRACE();
  // Associating proteins have been moved to contact. Before assigning them to
  // the complexsame complex, test to see if the complex is too big to fit in
  // the box. declare the six boundary sides of the system box;
  ...
  double negZ = -membraneObject.waterBox.z /
                2.0;  // no considering the reflecting-surface, because here we
                      // are checking whether to span the box

  bool canBeOutsideX{((reactCom1.radius + reactCom2.radius) >
                      (membraneObject.waterBox.x / 2.0))};
  bool canBeOutsideY{((reactCom1.radius + reactCom2.radius) >
                      (membraneObject.waterBox.y / 2.0))};
  bool canBeOutsideZ{((reactCom1.radius + reactCom2.radius) >
                      (membraneObject.waterBox.z / 2.0))};

  if (canBeOutsideZ) {
    // std::cout << "CHECK BOX SPAN in Z. Complex 1 Radius, Complex 2 radius "
    // << reactCom1.radius << ' ' << reactCom2.radius << '\n'; The approximate
    // size of the complex in z (max size) puts it as outside, now test
    // interface positions.
    bool outsidePos{false};
    bool outsideNeg{false};
    double posWall =
        -membraneObject.waterBox.z / 2.0;  // initialize to negative value.
    ...
    for (int memMol : reactCom1.memberList) {
      ...
      zplus = currz - posZ;
      zneg = negZ - currz;
      if (zplus > posWall) posWall = zplus;  // distance to +z
      if (zneg > negWall) negWall = zneg;    // distance to -z
    ...
    if (posWall > 0) outsidePos = true;
    if (negWall > 0) outsideNeg = true;
    // translation or cancel
    if (outsideNeg && outsidePos) {
      // std::cout << "STICKS OUT BOTH SIZES in Z, CANCEL ASSOCIATION " << '\n';
      cancelAssoc = true;
    }
    /*Also check if it sticks out far enough in one direction, that pushing back
       in will cause it to stick out the other side.
         */
    ...
    if (outsideNeg && !outsidePos) {
      ...
      for (int memMol : reactCom1.memberList) {
        ...
        for (auto& iface : moleculeList[memMol].tmpICoords) iface.z -= transZ;
      }
  ...
  if (canBeOutsideY) {
    // std::cout << "CHECK BOX SPAN in Y. Complex 1 Radius, Complex 2 radius "
    // << reactCom1.radius << ' ' << reactCom2.radius << '\n'; The approximate
    // size of the complex in y (max size) puts it as outside, now test
    // interface positions.
    bool outsidePos{false};
    bool outsideNeg{false};
    // double posWall{ negY };
    ...
    double curry;
    double posWall =
        -membraneObject.waterBox.y / 2.0;  // initialize to negative value.
    ...
    for (int memMol : reactCom1.memberList) {
      ...
      yplus = curry - posY;
      yneg = negY - curry;
      if (yplus > posWall) posWall = yplus;
      if (yneg > negWall) negWall = yneg;
      // measure each interface
      for (const auto& iface : moleculeList[memMol].tmpICoords) {
        curry = iface.y;
        yplus = curry - posY;
        yneg = negY - curry;
        if (yplus > posWall) posWall = yplus;  // distance to +y
        if (yneg > negWall) negWall = yneg;    // distance to -y
      }
    }
     ...
     */
    if (posWall + negWall > 0) {
      // std::cout << "WILL STICK OUT BOTH SIZES in Y, CANCEL ASSOCIATION " <<
      // posWall << ' ' << negWall << '\n';
      cancelAssoc = true;
    }
    ...
    if (outsideNeg && !outsidePos) {
      ...
      for (int memMol : reactCom1.memberList) {
        ...
        // update interface coords
        for (auto& iface : moleculeList[memMol].tmpICoords) iface.y -= transy;
      }
  ...
  if (canBeOutsideX) {
    // std::cout << "CHECK BOX SPAN in X. Complex 1 Radius, Complex 2 radius "
    // << reactCom1.radius << ' ' << reactCom2.radius << '\n'; The approximate
    // size of the complex in x (max size) puts it as outside, now test
    // interface positions.
    bool outsidePos{false};
    bool outsideNeg{false};
    double posWall =
        -membraneObject.waterBox.x / 2.0;  // initialize to negative value.
    ...
    for (int memMol : reactCom1.memberList) {
      ...
      xplus = currx - posX;
      xneg = negX - currx;
      if (xplus > posWall) posWall = xplus;  // distance to +x
      if (xneg > negWall)
      ...
      for (const auto& iface : moleculeList[memMol].tmpICoords) {
        ...
        xneg = negX - currx;
        if (xplus > posWall) posWall = xplus;  // distance to +x
        if (xneg > negWall) negWall = xneg;    // distance to -x
      }
    }
    ...
    if (outsideNeg && !outsidePos) {
      ...
      for (int memMol : reactCom1.memberList) {
        ...
        // update interface coords
        for (auto& iface : moleculeList[memMol].tmpICoords) iface.x -= transx;
      }
