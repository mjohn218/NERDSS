...
void reflect_traj_complex_rad_rot_box(const Parameters& params,
  ...
  // his routine updated March 2017 to test if a large complex that spans the
  ...
  // This is to test based on general size if it is close to boundaries, before
  // doing detailed evaluation below.

  bool canBeOutsideX{false};
  if ((currx + targCom.radius) > PosSideX ||
      (currx - targCom.radius) < NegSideX)
    canBeOutsideX = true;

  bool canBeOutsideY{false};
  if ((curry + targCom.radius) > PosSideY ||
      (curry - targCom.radius) < NegSideY)
    canBeOutsideY = true;

  bool canBeOutsideZ{false};
  if ((currz + targCom.radius) > PosSideZ ||
      (currz - targCom.radius) < NegSideZ)
    canBeOutsideZ = true;

  if (VERBOSE) {
    printf(
        "First check if the position of complex COM + radius can be out of the "
        "box: \n");
    printf("canBeOutsideX (%d), canBeOutsideY (%d), canBeOutsideZ (%d)\n",
           canBeOutsideX, canBeOutsideY, canBeOutsideZ);
    printf(
        "currx + radius (%.3f), PosSideX (%.3f), currx - radius (%.3f), "
        "NegSideX (%.3f)\n",
        currx + targCom.radius, PosSideX, currx - targCom.radius, NegSideX);
    printf(
        "curry + radius (%.3f), PosSideY (%.3f), curry - radius (%.3f), "
        "NegSideY (%.3f)\n",
        curry + targCom.radius, PosSideY, curry - targCom.radius, NegSideY);
    printf(
        "currz + radius (%.3f), PosSideZ (%.3f), currz - radius (%.3f), "
        "NegSideZ (%.3f)\n",
        currz + targCom.radius, PosSideZ, currz - targCom.radius, NegSideZ);
  }
  ...
  if (canBeOutsideX) {
    ...
    double Posdx =
        0;  // to store the largest distance outside positive X side. mark +
    double Negdx =
        0;  // to store the largest distance outside negative X side. make -
    double PosWallX =
        NegSideX;  // to store the farthest position in positive X direction.
    double NegWallX =
        PosSideX;  // to store the farthest position in negative X direction.
    ...
    // these need to be what current positions due to translation and rotation
    // are
    for (auto& memMol : targCom.memberList) {
      ...
      if (currx > PosWallX) PosWallX = currx;
      if (currx < NegWallX) NegWallX = currx;
    ...
    if (outside) {
      ...
      if (outsideNeg && outsidePos) {
        // For a large complex, test if it could be pushed back out the other
        // side std::cout << "Complex extends both X sides of the box. Out posX
        // side " << Posdx
        ...
        recheck = true;
        // std::cout << " Will push out the other side. negwallx " << NegWallX
        // << ", poswallx: " << PosWallX
        //           << ". 2*xtot: " << -2.0 * Negdx << '\n';
  ...
  if (canBeOutsideY) {
    ...
    double Posdy =
        0;  // to store the largest distance outside positive Y side. mark +
    double Negdy =
        0;  // to store the largest distance outside negative Y side. make -
    double PosWallY =
        NegSideY;  // to store the farthest position in positive Y direction.
    double NegWallY =
        PosSideY;  // to store the farthest position in negative Y direction.
    ...
    for (auto& memMol : targCom.memberList) {
      ...
      if (curry > PosWallY) PosWallY = curry;
      if (curry < NegWallY) NegWallY = curry;
    ...
    if (outside) {
      ...
      if (outsideNeg && outsidePos) {
        // For a large complex, test if it could be pushed back out the other
        // side std::cout << "Complex extends both Y sides of the box. Out posY
        // side " << Posdy
        ...
        // Also need to check that update will not push you out the other side
        recheck = true;
        // std::cout << " Will push out the other side. negwally " << NegWallY
        // << ", poswally: " << PosWallY
        //           << ". 2*ytot: " << -2.0 * Negdy << '\n';
  ...
  if (canBeOutsideZ) {
    ...
    double Posdz =
        0;  // to store the largest distance outside positive Z side. mark +
    double Negdz =
        0;  // to store the largest distance outside negative Z side. make -
    double PosWallZ =
        NegSideZ;  // to store the farthest position in positive Z direction.
    double NegWallZ =
        PosSideZ;  // to store the farthest position in negative Z direction.
    ...
    for (auto& memMol : targCom.memberList) {
      ...
      if (currz > PosWallZ) PosWallZ = currz;
      if (currz < NegWallZ) NegWallZ = currz;
    ...
    if (outside) {
      ...
      if (outsideNeg && outsidePos) {
        // For a large complex, test if it could be pushed back out the other
        // side std::cout << "Complex extends both Z sides of the box. Out posZ
        // side " << Posdz
        ...
        // Also need to check that update will not push you out the other side
        recheck = true;
        // std::cout << " Will push out the other side. negwallz " << NegWallZ
        // << ", poswallz: " << PosWallZ
        //           << ". 2*ztot: " << -2.0 * Negdz << '\n';
  ...
  if (recheck) {
    ...
    // Test that new coordinates have not pushed you out of the box for a very
    // large complex, if so, resample  rotation matrix. std::cout << "RECHECK
    // THAT COMPLEX DOES NOT SPAN BOX IN SUBROUTINE
    // REFLECT_TRAJ_COMPLEX_RAD_ROT. Complex: "
    //           << targCom.index << " size:" << targCom.memberList.size() <<
    //           '\n';
    reflect_traj_check_span_box(params, targCom, moleculeList, membraneObject,
                                RS3Dinput);
  }
}