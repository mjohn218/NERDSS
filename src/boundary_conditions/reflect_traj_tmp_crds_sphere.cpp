...
void reflect_traj_tmp_crds_sphere(const Parameters& params,
                                  ...
                                  double RS3Dinput) {
  // TRACE();
  /*This routine updated April 2020 to test if a large complex that spans the
  sphere could extend out in both directions if so, it attempts to correct for
  this by resampling the complex's translational updates.
  */
  ...
  if ((curr.get_magnitude() + targCom.radius) > sphereR) canBeOutsphere = true;
  ...
  if (canBeOutsphere == true) {
    ...
    if (outside) {
      ...
      double lamda = -2.0 * (targcrds.get_magnitude() - sphereR) /
                     targcrds.get_magnitude();
