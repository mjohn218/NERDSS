...
void reflect_complex_rad_rot(const Membrane& membraneObject, Complex& targCom,
  ...
  if (membraneObject.isSphere == true)
    reflect_complex_rad_rot_sphere(membraneObject, targCom, moleculeList,
                                   RS3Dinput);
  else
    reflect_complex_rad_rot_box(membraneObject, targCom, moleculeList,
                                RS3Dinput);
}
...
//         bool outside = false;
//         double posdx = 0; // to store the largest distance outside positive X
//         side. mark + double negdx = 0; // to store the largest distance
//         outside negative X side. make -
...
//                 std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in BOTH
//                 directions of X . ALREADY UPDATED POSITIONS. EXITING..." <<
//                 '\n'; exit(1);
//             }
//             if (outsideNeg && !outsidePos) {
//                 if (posWall - 2.0 * negdx > posX) {
//                     std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND
//                     in NEGATIVE side of X: try to put back in the box, x: "
//                               << -negdx << "BUT will EXTEND again in POSITIVE
//                               side of X" << '\n';
//                     exit(1);
//                 }
//                 std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in NEGATIVE
//                 direction of X: put back in the box, x: " << -negdx << '\n';
...
//                     std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND
//                     in POSITIVE side of X: try to put back in the box, x: "
//                               << -posdx << "BUT will EXTEND again in NEGATIVE
//                               side of X" << '\n';
//                     exit(1);
//                 }
//                 std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in POSITIVE
//                 direction of X: put back in the box, x: " << -posdx << '\n';
//                 // just update positions.Put back inside the box
...
//         bool outsidePos { false };
//         bool outside { false };
//         double posdy = 0; // to store the largest distance outside positive Y
//         side. mark + double negdy = 0; // to store the largest distance
//         outside negative Y side. make -
...
//                 // extends out both the front and back.
//                 std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in BOTH
//                 directions of Y . ALREADY UPDATED POSITIONS. EXITING..." <<
//                 '\n'; exit(1);
//             }
//             if (outsideNeg && !outsidePos) {
//                 if (posWall - 2.0 * negdy > posY) {
//                     std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND
//                     in NEGATIVE side of Y: try to put back in the box, y: "
//                               << -negdy << "BUT will EXTEND again in POSITIVE
//                               side of Y" << '\n';
//                     exit(1);
//                 }
//                 std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in NEGATIVE
//                 direction of Y: put back in the box, y: " << -negdy << '\n';
//                 // just update positions.Put back inside the box
...
//                     std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND
//                     in POSITIVE side of Y: try to put back in the box, y: "
//                               << -posdy << "BUT will EXTEND again in NEGATIVE
//                               side of Y" << '\n';
//                     exit(1);
//                 }
//                 std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in POSITIVE
//                 direction of Y: put back in the box, y: " << -posdy << '\n';
//                 // just update positions.Put back inside the box
...
//         bool outsidePos { false };
//         bool outside { false };
//         double posdz = 0; // to store the largest distance outside positive Z
//         side. mark + double negdz = 0; // to store the largest distance
//         outside negative Z side. make -
...
//                 // extends out both the front and back.
//                 std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in BOTH
//                 directions of Z . ALREADY UPDATED POSITIONS. EXITING..." <<
//                 '\n'; exit(1);
//             }
//             if (outsideNeg && !outsidePos) {
//                 if (posWall - 2.0 * negdz > posZ) {
//                     std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND
//                     in NEGATIVE side of Z: try to put back in the box, z: "
//                               << -negdz << "BUT will EXTEND again in POSITIVE
//                               side of Z" << '\n';
//                     exit(1);
//                 }
//                 std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in NEGATIVE
//                 direction of Z: put back in the box, z: " << -negdz << '\n';
//                 // just update positions.Put back inside the box
...
//                     std::cout << "PROBLEM: IN REFLECT COMPLEX RAD ROT, EXTEND
//                     in POSITIVE side of Z: try to put back in the box, z: "
//                               << -posdz << "BUT will EXTEND again in NEGATIVE
//                               side of Z" << '\n';
//                     exit(1);
//                 }
//                 std::cout << "IN REFLECT COMPLEX RAD ROT, EXTEND in POSITIVE
//                 direction of Z: put back in the box, z: " << -posdz << '\n';
//                 // just update positions.Put back inside the box
