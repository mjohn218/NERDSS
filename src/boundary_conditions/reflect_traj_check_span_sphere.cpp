 ...
 * make the complex outside the sphere boundary
 * ***
 *
 * ### Notes
 * ***
...
void reflect_traj_check_span_sphere(const Parameters& params, Complex& targCom,
  ...
  if (targCom.D.z < 1E-14 ||
      targCom.OnSurface) {  // for the complex on the sphere surface
    // in this case, the movement only involves theta and phi, and R doesn't
    // change, so it won't make the complex outside the sphere.
    ...
    while (checkItr < maxItr && needsRecheck) {
      ...
      double rtmp = sphereR;  // to store the furthest point' distance to the
                              // sphere center.
      ...
      // check whether this complex is out of the box, if so, change trajTrans
      // by considering the reflection
      if (rtmp > sphereR + 1E-15) {
        ...
        targcrds =
            Coord(0, 0, sphereR);  // to store the furthest point' coords.
        rtmp = sphereR;  // to store the furthest point' distance to the sphere
                         // center.
        for (auto& memMol : targCom.memberList) {
          // measure each protein COM
          Vector comVec{moleculeList[memMol].comCoord - targCom.comCoord};
      ...
      // recheck whether this complex is still out sphere, if so, regenerate
      // trajTrans
      if (rtmp > sphereR + 1E-15) {
        targCom.trajTrans.x =
        ...
        targCom.trajRot.y =
            sqrt(2.0 * params.timeStep * targCom.Dr.y) * GaussV();
        targCom.trajRot.z =
            sqrt(2.0 * params.timeStep * targCom.Dr.z) * GaussV();
  ...
  //    std::cout << "ITERATIONS TO CONVERGE POSITION WITHIN SPHERE: " <<
  //    checkItr
