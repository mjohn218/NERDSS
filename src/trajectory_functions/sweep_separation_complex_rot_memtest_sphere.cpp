...
void sweep_separation_complex_rot_memtest_sphere(
  ...
  while (itr < maxItr) {
    ...
    for (unsigned memMolItr{0};
      ...
      for (int crossMemItr{0};
        ...
        if (moleculeList[p2].isImplicitLipid) continue;
        ...
        if (comIndex1 != comIndex2) {
          ...
          if (complexList[comIndex1].D.z <
            ...
            dx1 = complexList[comIndex1].comCoord.x + iface1Vec.x +
                  complexList[comIndex1].trajTrans.x;
            dy1 = complexList[comIndex1].comCoord.y + iface1Vec.y +
                  complexList[comIndex1].trajTrans.y;
            dz1 = complexList[comIndex1].comCoord.z + iface1Vec.z +
          ...
          if (complexList[comIndex2].D.z <
            ...
            dx2 = complexList[comIndex2].comCoord.x + iface2Vec.x +
                  complexList[comIndex2].trajTrans.x;
            dy2 = complexList[comIndex2].comCoord.y + iface2Vec.y +
                  complexList[comIndex2].trajTrans.y;
            dz2 = complexList[comIndex2].comCoord.z + iface2Vec.z +
          ...
          if (dr2 <
            ...
            // we neglect that the bindRadius on sphere surface is different
            // from planar surface. since they are just a little different
    ...
    if (hasOverlap) {
      ...
      if (complexList[comIndex1].D.z < 1E-15) {  // complex on sphere surface
        ...
        complexList[comIndex1].trajRot.x =
            sqrt(2.0 * params.timeStep * complexList[comIndex1].Dr.x) *
            GaussV();
        complexList[comIndex1].trajRot.y =
            sqrt(2.0 * params.timeStep * complexList[comIndex1].Dr.y) *
            GaussV();
        complexList[comIndex1].trajRot.z =
            sqrt(2.0 * params.timeStep * complexList[comIndex1].Dr.z) *
            GaussV();
      } else {
        complexList[comIndex1].trajTrans.x =
      ...
      for (int checkMolItr{0}; checkMolItr < numOverlap; checkMolItr++) {
        ...
        if (resampleList[comIndex2] == 0) {
          if (p2 > startProIndex &&
              (moleculeList[p2].trajStatus == TrajStatus::none ||
         ...
         */
            ...
            reflect_traj_complex_rad_rot(params, moleculeList,
                                         complexList[comIndex2], membraneObject,
                                         RS3Dinput);
            // reflectList[comIndex2] = 1;
    ...
    if (itr == maxItr - 1) {
      if (comIndex1 != comIndex2) {
        std::cout
            << " WARNING ***************************************************** "
            << '\n';
        std::cout << "At simItr " << simItr
                  << ", pair check can't solve overlap: " << pro1Index
                  << " max cross: " << moleculeList[pro1Index].crossbase.size()
                  << " n overlap: " << tsave << " pro1: " << overlapList[0]
                  << " D: " << complexList[comIndex1].D.x << " "
                  << complexList[comIndex2].D.x
                  << " Last separation: " << sqrt(dr2) << '\n';
        ...
        for (int c{0}; c < com1Size; ++c) {
          ...
          for (int crossMolItr{0};
            ...
            // std::cout << "cross num: " << crossMolItr << " i1: " << relIface1
            // << " i2: " << relIface2
            //           << " pro: " << pro2Index << ' ' << " nfree; "
        ...
        std::cout << "Reached max iterations in sweep (memCheckList) without "
