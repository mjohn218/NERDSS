...
void sweep_separation_complex_rot_sphere(
  ...
  /*
    ...
    In this _complex_ version, itr tests overlap not just for each protein, but
   for each complex, so all the proteins in a complex, before performing
   ...
   given by its current position +traj. If itr does overlap, the displacement by
   traj is rejected and a new position for itself and any overlapping partners
   are selected. Once itr no longer overlaps anyone, this protein and its
   complex are moved and the partners retain their stored new displacements. If
   a protein has already updated its position (done in sequential order) then
   itr cannot resample a new position, the current protein must still continue
   to avoid overlapping, however.
  */
  // TRACE();
  ...
  //     complexList[com1Index], membraneObject, RS3Dinput);
  ...
  while (itr < maxItr) {
    ...
    for (unsigned memMolItr{0}; memMolItr < com1Size; ++memMolItr) {
      pro1Index = complexList[com1Index].memberList[memMolItr];
      for (int crossMolItr{0};
           crossMolItr < moleculeList[pro1Index].crossbase.size();
           ++crossMolItr) {
        ...
        if (com1Index != com2Index) {
          ...
          std::array<int, 3> rxnItr =
              moleculeList[pro1Index].crossrxn[crossMolItr];
          ...
          dx1 = complexList[com1Index].comCoord.x + iface1Vec.x +
                complexList[com1Index].trajTrans.x;
          dy1 = complexList[com1Index].comCoord.y + iface1Vec.y +
                complexList[com1Index].trajTrans.y;
          dz1 = complexList[com1Index].comCoord.z + iface1Vec.z +
                complexList[com1Index].trajTrans.z;

          /*Now complex 2*/
          // if (reflectList[com2Index] == 0) {
          //     reflect_traj_complex_rad_rot(params, moleculeList,
          //     complexList[com2Index], membraneObject, RS3Dinput);
          //     reflectList[com2Index] = 1;
          // }
          double dx2, dy2, dz2;
          if (complexList[com2Index].D.z <
              1E-10) {  // complex on sphere surface
            Coord ifacecrds = moleculeList[pro2Index].interfaceList[i2].coord;
            Coord iface_final = calculate_update_position_interface(
                complexList[com2Index], ifacecrds);
            dx2 = iface_final.x;
            dy2 = iface_final.y;
            dz2 = iface_final.z;
            ...
            dx2 = complexList[com2Index].comCoord.x + iface2Vec.x +
                  complexList[com2Index].trajTrans.x;
            dy2 = complexList[com2Index].comCoord.y + iface2Vec.y +
                  complexList[com2Index].trajTrans.y;
            dz2 = complexList[com2Index].comCoord.z + iface2Vec.z +
                  complexList[com2Index].trajTrans.z;
          }

          if (sqrt(dx2 * dx2 + dy2 * dy2 + dz2 * dz2) >
              membraneObject.sphereR) {
            // std::cout << "In sweep, current pro2Index protein interface is
            // outside the sphere membrane: index: "
            //           << pro2Index << " ,iface: " << i2 << ", dz: " << dz2 <<
            //           " iteration: " << simItr
            //           << '\n';
          }
    ...
    /*Now resample positions of p1 and overlapList, if numOverlap>0, otherwise
     no overlap, so break from loop*/
    if (hasOverlap) {
      ++itr;
      complexList[com1Index].trajTrans.x =
          sqrt(2.0 * params.timeStep * complexList[com1Index].D.x) * GaussV();
      complexList[com1Index].trajTrans.y =
          sqrt(2.0 * params.timeStep * complexList[com1Index].D.y) * GaussV();
      complexList[com1Index].trajTrans.z =
          sqrt(2.0 * params.timeStep * complexList[com1Index].D.z) * GaussV();
      complexList[com1Index].trajRot.x =
          sqrt(2.0 * params.timeStep * complexList[com1Index].Dr.x) * GaussV();
      complexList[com1Index].trajRot.y =
          sqrt(2.0 * params.timeStep * complexList[com1Index].Dr.y) * GaussV();
      complexList[com1Index].trajRot.z =
          sqrt(2.0 * params.timeStep * complexList[com1Index].Dr.z) * GaussV();
         ...
         */
            ...
            if (complexList[com2Index].D.z <
              ...
              complexList[com2Index].trajRot.x =
                  sqrt(2.0 * params.timeStep * complexList[com2Index].Dr.x) *
                  GaussV();
              complexList[com2Index].trajRot.y =
                  sqrt(2.0 * params.timeStep * complexList[com2Index].Dr.y) *
                  GaussV();
              complexList[com2Index].trajRot.z =
                  sqrt(2.0 * params.timeStep * complexList[com2Index].Dr.z) *
                  GaussV();
            } else {
              complexList[com2Index].trajTrans.x =
                  sqrt(2.0 * params.timeStep * complexList[com2Index].D.x) *
                  GaussV();
              complexList[com2Index].trajTrans.y =
                  sqrt(2.0 * params.timeStep * complexList[com2Index].D.y) *
                  GaussV();
              complexList[com2Index].trajTrans.z =
                  sqrt(2.0 * params.timeStep * complexList[com2Index].D.z) *
                  GaussV();
              complexList[com2Index].trajRot.x =
                  sqrt(2.0 * params.timeStep * complexList[com2Index].Dr.x) *
                  GaussV();
            ...
            reflect_traj_complex_rad_rot(params, moleculeList,
                                         complexList[com2Index], membraneObject,
                                         RS3Dinput);
            // reflectList[com2Index] = 1;
    ...
    if (itr == maxItr - 1) {
      if (com1Index != com2Index) {
        ...
        //           << " max cross: " <<
        //           moleculeList[pro1Index].crossbase.size() << " n overlap: "
        //           << tsave
        ...
        for (int memMolItr{0}; memMolItr < com1Size; ++memMolItr) {
          ...
          for (int crossMolItr{0};
            ...
            // std::cout << "cross num: " << crossMolItr << " relIface1: " <<
            // relIface1
            //           << " relIface2: " << relIface2 << " pro: " << pro2Index
            //           << ' ' << " nfree; "
        ...
        // std::cout << "Reached max iterations in sweep without solving
        // overlap, but proteins are "
