...
void write_timestep_information(long long int simItr, std::ofstream& outFile,
  ...
  int WIDTH =
      2 + 2 * molTemplateList.size();  // max number of different molecules in a
                                       // complex is Nprotypes

  int** adjmat;
  // Allocate memory for complex info
  adjmat = new int*[HEIGHT];
  for (int i = 0; i < HEIGHT; i++) adjmat[i] = new int[WIDTH];
  ...
  for (int i = 0; i < HEIGHT; i++) {
    ...
    for (int k = 1; k < 2 * (numUniqueMols) + 2;
         k++) {  // calculate unique compid
  ...
  for (int i = 0; i < Complex::currNumberComTypes; i++) {
    ...
    if (uniqmolexists ==
        0) {  // this is for keeping track of unique types of molecules produced
  ...
  for (int i = 0; i < HEIGHT; i++) delete[] adjmat[i];
  delete[] adjmat;

  for (int i = 0; i < Complex::numberOfComplexes; i++) delete[] complexstat[i];
  delete[] complexstat;
}
