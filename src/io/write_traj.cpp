...
void write_traj(long long int iter, std::ofstream& trajFile,
  ...
  // TRACE();
  // TODO: I need to change this so that it fills up the trajectory with empties
  // so that molecules are always in the same line in the trajectory
  ...
  for (auto& mol : moleculeList) {
    if (mol.isLeftGhost || mol.isRightGhost) continue;
    if (mol.isEmpty || mol.isImplicitLipid) continue;
    {
      trajFile << std::setw(4) << molTypeNames[mol.molTypeIndex] << ' '
               << std::fixed << mol.comCoord << '\n';
      ++numWritten;
      for (auto& iface : mol.interfaceList) {
        trajFile << std::setw(4) << molTypeNames[mol.molTypeIndex] << ' '
                 << std::fixed << iface.coord << '\n';
        ++numWritten;
      }
      trajFile << std::flush;
    }
  ...
  while (numWritten < params.numTotalUnits) {
    trajFile << std::setw(4) << "EMTY" << ' ' << std::fixed
             << membraneObject.waterBox.x / 2.0 << std::fixed
             << membraneObject.waterBox.y / 2.0 << std::fixed
