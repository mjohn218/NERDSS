...
void write_restart(long long int simItr, std::ofstream& restartFile,
                   const Parameters& params, const SimulVolume& simulVolume,
                   ...
                   const copyCounters& counterArrays) {
  // TRACE();
  // Write parameters
  restartFile.precision(20);
  {
    ...
    restartFile << "currSimTime (s) = " << std::scientific
                << (simItr - params.itrRestartFrom) * params.timeStep * 1E-6 +
    ...
    restartFile << "simulDimensions = " << membraneObject.waterBox.x << ' '
                << membraneObject.waterBox.y << ' ' << membraneObject.waterBox.z
                << '\n';
    restartFile << "membrane = " << membraneObject.implicitlipidIndex << ' '
                << membraneObject.nSites << ' ' << membraneObject.nStates << ' '
                << membraneObject.No_free_lipids << ' '
                << membraneObject.No_protein << ' ' << membraneObject.totalSA
                << '\n';
    ...
    restartFile << '\n';
    restartFile << "implicitLipidsParams = " << membraneObject.implicitLipid
                << ' ' << membraneObject.TwoD << ' ' << membraneObject.isBox
                << ' ' << membraneObject.isSphere << ' '
    ...
    restartFile << "clusterOverlapCheck = " << params.clusterOverlapCheck
                << '\n';
    restartFile << "checkUnimoleculeReactionPopulation = "
                << params.checkUnimoleculeReactionPopulation << '\n';
    restartFile << "hasRankCommunicationForLargeComplex = "
  ...
  */
  // write MolTemplates
  {
    ...
    for (auto& num : MolTemplate::numEachMolType) restartFile << ' ' << num;
    restartFile << '\n';
    restartFile << MolTemplate::absToRelIface.size();
    for (auto& iface : MolTemplate::absToRelIface) restartFile << ' ' << iface;
    restartFile << '\n';
    ...
    for (const auto& oneTemp : molTemplateList) {
      ...
      restartFile << oneTemp.copies << ' ' << oneTemp.mass << ' '
                  << oneTemp.radius << '\n';
      restartFile << oneTemp.isLipid << ' ' << oneTemp.isImplicitLipid << ' '
                  << oneTemp.isRod << ' ' << oneTemp.isPoint << ' '
                  << oneTemp.checkOverlap << ' ' << oneTemp.countTransition
      ...
      for (const auto& oneIface : oneTemp.interfaceList) {
        ...
        restartFile << std::fixed << oneIface.iCoord.x << ' '
                    << oneIface.iCoord.y << ' ' << oneIface.iCoord.z << '\n';
        ...
        for (auto& oneState : oneIface.stateList) {
          ...
          for (auto elem : oneState.rxnPartners) restartFile << ' ' << elem;
          restartFile << '\n';

          // reaction lists
          restartFile << oneState.myForwardRxns.size();
          for (auto elem : oneState.myForwardRxns) restartFile << ' ' << elem;
          restartFile << '\n';
        ...
        // EXCLUDEVOLUMEBOUND
        restartFile << oneIface.excludeVolumeBoundList.size();
        for (auto elem : oneIface.excludeVolumeBoundList)
          restartFile << ' ' << elem;
        restartFile << '\n';

        restartFile << oneIface.excludeVolumeBoundIfaceList.size();
        for (auto elem : oneIface.excludeVolumeBoundIfaceList)
          restartFile << ' ' << elem;
        restartFile << '\n';

        restartFile << oneIface.excludeVolumeBoundReactList.size();
        for (auto elem : oneIface.excludeVolumeBoundReactList)
          restartFile << ' ' << elem;
        restartFile << '\n';

        restartFile << oneIface.excludeRadiusList.size();
        for (auto elem : oneIface.excludeRadiusList) restartFile << ' ' << elem;
        restartFile << '\n';
      }
  ...
  // write Reactions
  {
    ...
    restartFile << RxnBase::numberOfRxns << ' ' << forwardRxns.size() << ' '
                << backRxns.size() << ' ' << createDestructRxns.size() << ' '
    ...
    for (const auto& oneRxn : forwardRxns) {
      restartFile << oneRxn.absRxnIndex << ' ' << oneRxn.relRxnIndex << ' '
                  << oneRxn.rxnLabel << '\n';
      restartFile << static_cast<std::underlying_type<ReactionType>::type>(
                         oneRxn.rxnType)
                  << ' ' << oneRxn.isSymmetric << ' ' << oneRxn.isOnMem << ' '
                  << oneRxn.hasStateChange << '\n';
      restartFile << oneRxn.isObserved << ' ' << oneRxn.observeLabel << ' '
                  << oneRxn.productName << '\n';
      restartFile << oneRxn.isReversible << ' ' << oneRxn.conjBackRxnIndex
                  << ' ' << oneRxn.irrevRingClosure << ' '
                  << oneRxn.bindRadSameCom << ' ' << oneRxn.loopCoopFactor
                  << '\n'
                  << oneRxn.length3Dto2D << '\n';
      restartFile << std::setprecision(20) << oneRxn.bindRadius << ' '
                  << oneRxn.assocAngles.theta1 << ' '
                  << oneRxn.assocAngles.theta2 << ' ' << oneRxn.assocAngles.phi1
                  << ' ' << oneRxn.assocAngles.phi2 << ' '
                  << oneRxn.assocAngles.omega << '\n';
      restartFile << oneRxn.norm1.x << ' ' << oneRxn.norm1.y << ' '
                  << oneRxn.norm1.z << '\n';
      restartFile << oneRxn.norm2.x << ' ' << oneRxn.norm2.y << ' '
                  << oneRxn.norm2.z << '\n';
      restartFile << oneRxn.excludeVolumeBound << '\n';
      restartFile << oneRxn.isCoupled;
      if (oneRxn.isCoupled)
        restartFile << ' ' << oneRxn.coupledRxn.absRxnIndex << ' '
                    << oneRxn.coupledRxn.relRxnIndex << ' '
                    << static_cast<std::underlying_type<ReactionType>::type>(
                           oneRxn.coupledRxn.rxnType)
                    << ' ' << oneRxn.coupledRxn.label << ' '
                    << std::setprecision(20) << oneRxn.coupledRxn.probCoupled;
      restartFile << '\n';
      ...
      for (const auto& oneReact : oneRxn.reactantListNew) {
        ...
        restartFile << oneReact.requiresState << ' '
      ...
      for (const auto& oneProd : oneRxn.productListNew) {
        ...
        restartFile << oneProd.requiresState << ' '
                    << oneProd.requiresInteraction << '\n';
      }
        ...
        for (const auto& otherIfaceList : oneRate.otherIfaceLists) {
          ...
          for (const auto& anccIface : otherIfaceList) {
            restartFile << anccIface.molTypeIndex << ' ' << anccIface.ifaceName
                        << ' ' << anccIface.absIfaceIndex << ' '
                        << anccIface.relIfaceIndex << ' '
                        << anccIface.requiresState << ' '
                        << anccIface.requiresInteraction << '\n';
          }
        }
      }
    }
    ...
    for (auto& oneRxn : createDestructRxns) {
      ...
      for (auto& oneReact : oneRxn.reactantMolList) {
        restartFile << oneReact.molTypeIndex << ' ' << oneReact.molName << ' '
                    << oneReact.interfaceList.size() << '\n';
        for (auto& oneIface : oneReact.interfaceList) {
          restartFile << oneIface.molTypeIndex << '\n';
          restartFile << oneIface.ifaceName << ' ' << oneIface.absIfaceIndex
                      << ' ' << oneIface.relIfaceIndex << '\n';
          restartFile << oneIface.requiresState << ' '
                      << oneIface.requiresInteraction << '\n';
        }
      }
      ...
      for (auto& oneProd : oneRxn.productMolList) {
        restartFile << oneProd.molTypeIndex << ' ' << oneProd.molName << ' '
                    << oneProd.interfaceList.size() << '\n';
      ...
      for (auto& oneRate : oneRxn.rateList) {
        ...
        //         restartFile << anccIface.molTypeIndex << ' ' <<
        //         anccIface.ifaceName << ' '
        //                     << anccIface.absIfaceIndex << ' ' <<
        //                     anccIface.relIfaceIndex << ' '
        //                     << anccIface.requiresState << ' ' <<
  ...
  // write Molecules
  {
    ...
    restartFile << moleculeList.size() << ' ' << Molecule::numberOfMolecules
                << ' ' << Molecule::maxID << '\n';
    // restartFile << Molecule::mapIdToIndex.size();
    // for (const auto& pair : Molecule::mapIdToIndex) {
    //   restartFile << ' ' << pair.first << ' ' << pair.second;
    // }
    // restartFile << '\n';
    for (auto& oneMol : moleculeList) {
      restartFile << oneMol.index << ' ' << oneMol.isEmpty << ' '
                  << oneMol.myComIndex << ' ' << oneMol.molTypeIndex << ' '
                  << oneMol.mySubVolIndex << '\n';
      restartFile << oneMol.mass << ' ' << oneMol.isLipid << ' '
                  << oneMol.isImplicitLipid << ' ' << oneMol.linksToSurface
                  << ' ' << oneMol.isEmpty << ' '  << ' ' << oneMol.id
                  << ' ' << oneMol.isLeftGhost << ' ' << oneMol.isLeftEdge << ' '<< oneMol.isRightEdge << ' '<< oneMol.isRightGhost << ' '
                  << oneMol.receivedFromNeighborRank << ' ' << oneMol.complexId
                  << '\n';

      // center of mass
      restartFile << std::fixed << oneMol.comCoord.x << ' ' << oneMol.comCoord.y
                  << ' ' << oneMol.comCoord.z << '\n';
      ...
      for (auto& oneIface : oneMol.interfaceList) {
        restartFile << oneIface.index << ' ' << oneIface.relIndex << ' '
                    << oneIface.molTypeIndex << ' ' << oneIface.stateIndex
                    << ' ' << oneIface.stateIden << ' ' << oneIface.isBound
                    << ' ' << oneIface.excludeVolume << '\n';
        restartFile << std::fixed << oneIface.coord.x << ' ' << oneIface.coord.y
                    << ' ' << oneIface.coord.z << '\n';

        if (oneIface.isBound) {
          restartFile << oneIface.interaction.partnerIndex << ' '
                      << oneIface.interaction.partnerId << ' '
                      << oneIface.interaction.partnerIfaceIndex << ' '
      ...
      for (const auto& oneElem : oneMol.prevlist) restartFile << ' ' << oneElem;
      restartFile << '\n';
      ...
      for (const auto& oneElem : oneMol.prevnorm) restartFile << ' ' << oneElem;
      restartFile << '\n';
      restartFile << oneMol.ps_prev.size();
      for (const auto& oneElem : oneMol.ps_prev) restartFile << ' ' << oneElem;
      restartFile << '\n';
      restartFile << oneMol.prevsep.size();
      for (const auto& oneElem : oneMol.prevsep) restartFile << ' ' << oneElem;
      restartFile << '\n';
    }

    restartFile << Molecule::emptyMolList.size();
    for (auto& index : Molecule::emptyMolList) restartFile << ' ' << index;
    restartFile << '\n';
  }
  ...
  // write Complexes
  {
    ...
    restartFile << complexList.size() << ' ' << Complex::numberOfComplexes
                << ' ' << Complex::maxID << '\n';
    // restartFile << Complex::mapIdToIndex.size();
    // for (const auto& pair : Complex::mapIdToIndex) {
    //   restartFile << ' ' << pair.first << ' ' << pair.second;
    // }
    // restartFile << '\n';
    for (const auto& oneCom : complexList) {
      restartFile << oneCom.index << ' ' << oneCom.isEmpty << ' '
                  << oneCom.radius << ' ' << oneCom.mass << ' ' << oneCom.id
                  << ' ' << oneCom.isLeftGhost
                  << ' ' << oneCom.isLeftEdge
                  << ' ' << oneCom.isRightEdge
                  << ' ' << oneCom.isRightGhost
                  << ' ' << oneCom.ownerRank << ' '
                  << oneCom.receivedFromNeighborRank << '\n';
      restartFile << oneCom.linksToSurface << ' ' << oneCom.iLipidIndex << ' '
                  << oneCom.OnSurface << '\n';
      restartFile << std::fixed << oneCom.comCoord.x << ' ' << oneCom.comCoord.y
                  << ' ' << oneCom.comCoord.z << '\n';
      restartFile << std::fixed << oneCom.D.x << ' ' << oneCom.D.y << ' '
                  << oneCom.D.z << '\n';
      restartFile << std::fixed << oneCom.Dr.x << ' ' << oneCom.Dr.y << ' '
                  << oneCom.Dr.z << '\n';

      // member molecule lists
      restartFile << oneCom.memberList.size();
      for (const auto& memMol : oneCom.memberList) restartFile << ' ' << memMol;
      restartFile << '\n';
      restartFile << oneCom.numEachMol.size();
      for (const auto& memMol : oneCom.numEachMol) restartFile << ' ' << memMol;
      restartFile << '\n';
    ...
    for (auto& index : Complex::emptyComList) restartFile << ' ' << index;
    restartFile << '\n';
  }
  ...
  // Write counterArrays
  {
    ...
    restartFile << counterArrays.nLoops << ' '
                ...
                << counterArrays.nAssocSuccess << ' '
                << counterArrays.eventArraySize << '\n';
    // write events3D, 3Dto2D, and 2D
    for (auto& event : counterArrays.events3D) restartFile << event << ' ';
    restartFile << '\n';
    for (auto& event : counterArrays.events3Dto2D) restartFile << event << ' ';
    restartFile << '\n';
    for (auto& event : counterArrays.events2D) restartFile << event << ' ';
    restartFile << '\n';
  }
}
