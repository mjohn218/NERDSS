    ...
    for (const auto& mol : moleculeList) {
        ...
        if (!mol.isEmpty) {
            ...
            pdbFile << "ATOM  " << std::right << std::setw(5) << i % 100000 << " " << std::left << std::setw(4) << "COM"
                    ...
                    << std::right << std::setw(8) << std::fixed << std::setprecision(3) << (mol.comCoord.x + membraneObject.waterBox.x / 2) << std::right << std::setw(8)
            ...
            for (unsigned j { 0 }; j < mol.interfaceList.size(); ++j) {
                pdbFile << "ATOM  " << std::right << std::setw(5) << i % 100000 << " " << std::left << std::setw(4) // << ' '
                        ...
                        << std::right << std::setw(8) << std::fixed << std::setprecision(3) << (mol.interfaceList[j].coord.x + membraneObject.waterBox.x / 2) << std::right << std::setw(8)
