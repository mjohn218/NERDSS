...
void print_system_information(long long int simItr, std::ofstream& systemFile,
    ...
    if (!mol.isEmpty) {
      ...
      for (const auto& iface : mol.interfaceList) {
        ...
        systemFile << "\tInterface name: "
                   << molTemplateList[mol.molTypeIndex]
                          .interfaceList[iface.relIndex]
                          .name
                   << '\n';
        ...
        if (iface.isBound) {
          ...
          systemFile << "\t\tPartner index: " << iface.interaction.partnerIndex
                     << '\n';
          systemFile << "\t\tPartner interface index "
  ...
  for (const auto& complex : complexList) {
    ...
    if (!complex.isEmpty) {
      ...
      for (auto memMol : complex.memberList) systemFile << ' ' << memMol;
      systemFile << '\n';
      systemFile << "Number of each molecule type:";
      for (unsigned i{0}; i < complex.numEachMol.size(); ++i)
        systemFile << '\t' << molTemplateList[i].molName << ": "
                   << complex.numEachMol[i] << '\n';
    }
    systemFile << linebreak;
  }
}
