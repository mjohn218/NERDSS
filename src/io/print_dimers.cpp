...
void print_dimers(std::vector<Complex>& complexList, std::ofstream& outfile,
  ...
  i = 0;
  while (complexList[i].isEmpty == true) i++;
  ...
  // cout <<"New assembly type: "<<index<<" ffrom complex: "<<i<<" size of
  // assemblylist: "<<assemblylist.size()<<endl;
    ...
    if (complexList[i].isEmpty == false) {
      ...
      for (int a = 0; a < assemblylist.size(); a++) {
        if (index == assemblylist[a]) {
          ...
          flag = 1;
          a = assemblylist
                  .size();  // break from loop, found your index already.
        }
      }
      if (flag == 0) {
        // new assembly type.
        ...
        complexrep.push_back(i);  // example complex with this composition
        histogram.push_back(1);   // created a new assembly type with one so far
        //   cout <<"New assembly type: "<<index<<" ffrom complex: "<<i<<" size
        //   of assemblylist:
  ...
  for (int a = 0; a < assemblylist.size(); a++) {
    ...
    if (complexList[c1].memberList.size() == 1) {
      ...
      for (j = 0; j < nTypes; j++) {
        if (complexList[c1].numEachMol[j] == 1 ||
            complexList[c1].numEachMol[j] == 2) {
          dimers[j] +=
              histogram[a];  // SUM OVER ALL POSSIBLE DIMERS FOR PROTEIN J
        }
      }
    }
    // outfile <<infofilenames[j].substr(0,3)<<":
    // "<<complexList[c1].numEachMol[j]<<". ";

    //	  outfile<<endl;

  }  // loop over all assemblies in the system
  outfile << (it - params.itrRestartFrom) * params.timeStep * 1E-6 +
