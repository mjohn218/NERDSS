...
void write_psf(const Parameters& params,
  ...
  for (auto& mol : moleculeList) {
    const MolTemplate& molTemp{molTemplateList[mol.molTypeIndex]};
    if (molTemp.isImplicitLipid == true) continue;

    outFile << std::setw(8) << numWritten << ' ' << std::setw(4)
            << molTemp.molName.substr(0, 3) << ' ' << std::setw(4) << mol.index
            << ' ' << std::setw(4) << "COM" << ' ' << std::setw(4) << "O" << ' '
            << std::setw(4) << "0" << ' ' << std::setw(10) << "0" << ' '
            << std::setw(10) << mol.mass << ' ' << std::setw(10) << "0\n";
    ++numWritten;
    nBond += molTemp.bondList.size();
    for (size_t ifaceItr{0}; ifaceItr < mol.interfaceList.size(); ++ifaceItr) {
      if (ifaceItr < 3) {
        outFile << std::setw(8) << numWritten << ' ' << std::setw(4)
                << molTemp.molName.substr(0, 3) << ' ' << std::setw(4)
                << mol.index << ' ' << std::setw(4)
                << molTemp.interfaceList[ifaceItr].name.substr(0, 3) << ' '
                << std::setw(4) << color1 << ' ' << std::setw(4) << "0" << ' '
                << std::setw(10) << "0" << ' ' << std::setw(10) << mol.mass
                << ' ' << std::setw(10) << "0\n";
        ...
        outFile << std::setw(8) << numWritten << ' ' << std::setw(4)
                ...
                << mol.index << ' ' << std::setw(4)
                << molTemp.interfaceList[ifaceItr].name.substr(0, 3) << ' '
                << std::setw(4) << color2 << ' ' << std::setw(4) << "0" << ' '
                << std::setw(10) << "0" << ' ' << std::setw(10) << mol.mass
                << ' ' << std::setw(10) << "0\n";
  ...
  while (numWritten < params.numTotalUnits + 1) {
    outFile << std::setw(8) << numWritten << ' ' << std::setw(4) << "EMY" << ' '
            << std::setw(4) << emptyItr << ' ' << std::setw(4) << "EMY" << ' '
            << std::setw(4) << "EMY" << ' ' << std::setw(4) << "0" << ' '
  ...
  for (auto& mol : moleculeList) {
    ...
    // nBond += (oneTemp.bondList.size() == 0) ? oneTemp.copies *
    // oneTemp.interfaceList.size()
    ...
    for (size_t bondItr{0}; bondItr < molTemp.bondList.size(); ++bondItr) {
      ...
      ++colNum;
      if (colNum % 4 == 0) outFile << std::endl;
    }
  ...
  //                 for (size_t itr { 0 }; itr < molTemp.interfaceList.size();
  //                 ++itr) {
  //                     int tmp = numWritten + itr + 1;
  //                     outFile << std::setw(8) << numWritten << std::setw(8)
  //                     << tmp;
  //                     ++colNum;
  //                     if (colNum % 4 == 0)
  //                         outFile << '\n';
  ...
  //         } else {
  //             for (size_t i { 0 }; i < molTemp.copies; ++i) {
  //                 for (size_t bondItr { 0 }; bondItr <
  //                 molTemp.bondList.size(); ++bondItr) {
  //                     int tmp = numWritten + i +
  //                     molTemp.bondList[bondItr][0]; int tmp2 = numWritten + i
  //                     + molTemp.bondList[bondItr][1]; outFile << std::setw(8)
  //                     << tmp << std::setw(8) << tmp2;
  //                     ++colNum;
  //                     if (colNum % 4 == 0)
  //                         outFile << '\n';
