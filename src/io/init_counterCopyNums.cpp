...
/*Initialize the array of copyNumSpecies. Based off the copy numbers looped over
  all Molecules, thus should work for either default start or for restart.
 */
void init_counterCopyNums(copyCounters& counterArrays,
  ...
  int index;
  int p1, p2;
  // initialize the copyNums array to zero, for all possible interfaces and
  // states (includes product species)
  /*For products containing two components, it will double count them!
   */
  // int totalSpecies = RxnBase::totRxnSpecies;
          ...
          if (counterArrays.canDissociate[index] == true) {
            ...
            if (complexList[moleculeList[p1].myComIndex].linksToSurface == 0) {
              ...
              std::vector<int>::iterator itr =
                  counterArrays.bindPairList[index].begin();
              for (itr = counterArrays.bindPairList[index].begin();
                   itr != counterArrays.bindPairList[index].end(); ++itr) {
                if (*itr == partnerMolIndex) break;
              }
  ...
  cout << "Initialized copyNumSpecies. Printing all values. total species: "
       << totalSpecies << endl;
  for (i = 0; i < totalSpecies; i++) {
    ...
    cout << " absolute index: " << i
         << " Copy nums: " << counterArrays.copyNumSpecies[i] << endl;
  }
}
