...
void write_xyz(std::string filename, const Parameters& params,
  ...
  for (auto& molTemp : molTemplateList) names.push_back(molTemp.molName);
  ...
  for (auto& mol : moleculeList) {
    ...
    out << std::setw(4) << names[mol.molTypeIndex] << ' ' << std::fixed
        << mol.comCoord << std::endl;
    ++numWritten;
    for (auto& iface : mol.interfaceList) {
      out << std::setw(4) << names[mol.molTypeIndex] << ' ' << std::fixed
          << iface.coord << std::endl;
      ++numWritten;
    }
  }
  while (numWritten < params.numTotalUnits) {
    out << std::setw(4) << "EMTY" << ' ' << std::fixed
        << Coord{150.0, 150.0, 150.0} << std::endl;
    ++numWritten;
  }
}
