...
void read_restart(long long int& simItr, std::ifstream& restartFile,
  ...
  try {
    ...
    restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    {
      ...
      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '=');
      restartFile >> membraneObject.waterBox.x >> membraneObject.waterBox.y >>
          membraneObject.waterBox.z;
      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
      membraneObject.waterBox.volume = membraneObject.waterBox.x *
                                       membraneObject.waterBox.y *
                                       membraneObject.waterBox.z;

      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '=');
      restartFile >> membraneObject.implicitlipidIndex >>
          membraneObject.nSites >> membraneObject.nStates >>
          membraneObject.No_free_lipids >> membraneObject.No_protein >>
      ...
      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '=');
      restartFile >> membraneObject.implicitLipid >> membraneObject.TwoD >>
          membraneObject.isBox >> membraneObject.isSphere >>
      ...
      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '=');
      restartFile >> params.checkUnimoleculeReactionPopulation;
      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '=');
      restartFile >> params.hasRankCommunicationForLargeComplex;
      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    ...
    std::cout << "restart write, pdbWrite: " << params.restartWrite << ' '
              << params.pdbWrite << std::endl;
    /*	std::cout<<"READ IN SUB volume PARTITIONING from restart
    file"<<std::endl;
    // Read Simulation Volume
    {
        restartFile >> simulVolume.numSubCells.x >> simulVolume.numSubCells.y >>
    simulVolume.numSubCells.z
            >> simulVolume.numSubCells.tot;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

        restartFile >> simulVolume.subCellSize.x >> simulVolume.subCellSize.y >>
    simulVolume.subCellSize.z;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

        for (unsigned subCellItr { 0 }; subCellItr <
    simulVolume.numSubCells.tot; ++subCellItr) { SimulVolume::SubVolume subCell
    {}; restartFile >> subCell.absIndex >> subCell.xIndex >> subCell.yIndex >>
    subCell.zIndex;
            restartFile.ignore(std::numeric_limits<std::streamsize>::max(),
    '\n');
    ...
    */
    ...
    // Read MolTemplates
    {
      ...
      for (unsigned itr{0}; itr < MolTemplate::numMolTypes; ++itr) {
        ...
        std::cout << " protein index, name: " << oneTemp.molTypeIndex << ' '
                  << oneTemp.molName << '\n';
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        restartFile >> oneTemp.isLipid >> oneTemp.isImplicitLipid >>
            oneTemp.isRod >> oneTemp.isPoint >> oneTemp.checkOverlap >>
            oneTemp.countTransition >> oneTemp.transitionMatrixSize;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

        restartFile >> oneTemp.comCoord.x >> oneTemp.comCoord.y >>
            oneTemp.comCoord.z;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        restartFile >> oneTemp.excludeVolumeBound >> oneTemp.bindToSurface;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        for (unsigned ifaceItr{0}; ifaceItr < oneTempIfaceSize; ++ifaceItr) {
          ...
          std::cout << " iface index, name: " << tmpIface.index << ' '
                    << tmpIface.name << '\n';
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
          restartFile >> tmpIface.iCoord.x >> tmpIface.iCoord.y >>
              tmpIface.iCoord.z;
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
          ...
          for (unsigned stateItr{0}; stateItr < stateListSize; ++stateItr) {
            ...
            for (unsigned rxnItr{0}; rxnItr < myCreateDestructRxnsSize;
                 ++rxnItr) {
              int rxn{0};
              restartFile >> rxn;
          ...
          // READ EXCLUDEVOLUMEBOUND
          unsigned excludeVolumeBoundListSize{0};
          restartFile >> excludeVolumeBoundListSize;
          for (unsigned tmpValue{0}; tmpValue < excludeVolumeBoundListSize;
               ++tmpValue) {
            int tmpNum{0};
            restartFile >> tmpNum;
            tmpIface.excludeVolumeBoundList.push_back(tmpNum);
          }
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

          unsigned excludeVolumeBoundIfaceListSize{0};
          restartFile >> excludeVolumeBoundIfaceListSize;
          for (unsigned tmpValue{0}; tmpValue < excludeVolumeBoundIfaceListSize;
               ++tmpValue) {
            int tmpNum{0};
            restartFile >> tmpNum;
            tmpIface.excludeVolumeBoundIfaceList.push_back(tmpNum);
          }
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

          unsigned excludeVolumeBoundReactListSize{0};
          restartFile >> excludeVolumeBoundReactListSize;
          for (unsigned tmpValue{0}; tmpValue < excludeVolumeBoundReactListSize;
               ++tmpValue) {
            int tmpNum{0};
            restartFile >> tmpNum;
            tmpIface.excludeVolumeBoundReactList.push_back(tmpNum);
          }
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

          unsigned excludeRadiusListSize{0};
          restartFile >> excludeRadiusListSize;
          for (unsigned tmpValue{0}; tmpValue < excludeRadiusListSize;
               ++tmpValue) {
            int tmpNum{0};
            restartFile >> tmpNum;
            tmpIface.excludeRadiusList.push_back(tmpNum);
          }
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        if (oneTemp.countTransition == true) {
          ...
          for (int indexOne = 0; indexOne < oneTemp.transitionMatrixSize;
               ++indexOne) {
        ...
        if (oneTemp.countTransition == true) {
          ...
          for (int indexOne = 0; indexOne < oneTemp.transitionMatrixSize;
               ++indexOne) {
            oneTemp.transitionMatrix[indexOne].resize(
                oneTemp.transitionMatrixSize);
          }

          for (int indexOne = 0; indexOne < oneTemp.transitionMatrixSize;
               ++indexOne) {
            for (int indexTwo = 0; indexTwo < oneTemp.transitionMatrixSize;
                 ++indexTwo) {
    ...
    // write Reactions
    {
      ...
      restartFile >> RxnBase::numberOfRxns >> forwardRxnsSize >> backRxnsSize >>
          createDestructRxnsSize >> RxnBase::totRxnSpecies;
      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
      std::cout << " Num rxns: " << RxnBase::numberOfRxns
                << " forwardRxns: " << forwardRxnsSize << '\n';
      // forward reactions
      for (unsigned rxnItr{0}; rxnItr < forwardRxnsSize; ++rxnItr) {
        ...
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        std::cout << " itr: " << rxnItr
                  << " abs index, relindex: " << tmpRxn.absRxnIndex << ' '
                  << tmpRxn.relRxnIndex << '\n';
        int rxnType{-1};
        restartFile >> rxnType >> tmpRxn.isSymmetric >> tmpRxn.isOnMem >>
            tmpRxn.hasStateChange;
        std::cout << "Rxntype: " << rxnType << '\n';
        if (rxnType != -1) {
          tmpRxn.rxnType = static_cast<ReactionType>(
              rxnType);  // turn the int rxnType into ReactionType
        } else {
          std::cerr << "ERROR: Cannot parse reaction type for reaction "
                    << tmpRxn.absRxnIndex << ". Exiting.\n";
          exit(1);
        }
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        restartFile >> tmpRxn.isObserved;
        if (tmpRxn.isObserved) restartFile >> tmpRxn.observeLabel;
        ...
        restartFile >> tmpRxn.isReversible >> tmpRxn.conjBackRxnIndex >>
            ...
            tmpRxn.loopCoopFactor >> tmpRxn.length3Dto2D;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        //>> tmpRxn.assocAngles.phi1 >> tmpRxn.assocAngles.phi2 >>
        // tmpRxn.assocAngles.omega;
        std::cout << "RXN angles " << tmpRxn.bindRadius << ' '
                  << tmpRxn.assocAngles.theta1 << ' '
                  << tmpRxn.assocAngles.theta2 << ' ' << tmpRxn.assocAngles.phi1
                  << ' ' << tmpRxn.assocAngles.phi2 << ' '
                  << tmpRxn.assocAngles.omega << '\n';
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        std::cout << " norm 2: " << tmpRxn.norm2.x << ' ' << tmpRxn.norm2.y
                  << ' ' << tmpRxn.norm2.z << '\n';
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        if (tmpRxn.isCoupled) {
          ...
          restartFile >> tmpRxn.coupledRxn.absRxnIndex >>
              tmpRxn.coupledRxn.relRxnIndex >> rxnType >>
              tmpRxn.coupledRxn.label >> tmpRxn.coupledRxn.probCoupled;
          if (rxnType != -1) {
        ...
        for (unsigned itr{0}; itr < reactantListNewSize; ++itr) {
          ...
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
          restartFile >> oneReact.ifaceName >> oneReact.absIfaceIndex >>
              oneReact.relIfaceIndex;
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
          restartFile >> oneReact.requiresState >> oneReact.requiresInteraction;
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
          tmpRxn.reactantListNew.emplace_back(oneReact);
          std::cout << " requiresState, requiresInteraction: "
                    << oneReact.requiresState << ' '
                    << oneReact.requiresInteraction << '\n';
        }
        ...
        for (unsigned itr{0}; itr < productListNewSize; ++itr) {
          ...
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
          restartFile >> oneProd.ifaceName >> oneProd.absIfaceIndex >>
              oneProd.relIfaceIndex;
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        for (unsigned itr{0}; itr < rateListSize; ++itr) {
          ...
          for (unsigned listItr{0}; listItr < otherIfaceListsSize; ++listItr) {
            ...
            restartFile.ignore(std::numeric_limits<std::streamsize>::max(),
                               '\n');
            for (unsigned anccIfaceItr{0}; anccIfaceItr < oneListSize;
                 ++anccIfaceItr) {
              RxnIface otherIface{};
              restartFile >> otherIface.molTypeIndex >> otherIface.ifaceName >>
                  otherIface.absIfaceIndex >> otherIface.relIfaceIndex >>
                  otherIface.requiresState >> otherIface.requiresInteraction;
              restartFile.ignore(std::numeric_limits<std::streamsize>::max(),
                                 '\n');
      ...
      for (unsigned rxnItr{0}; rxnItr < createDestructRxnsSize; ++rxnItr) {
        ...
        for (unsigned itr{0}; itr < reactantMolListSize; ++itr) {
          ...
          restartFile >> oneMol.molTypeIndex >> oneMol.molName >>
              interfaceListSize;
          ...
          for (unsigned ifaceItr{0}; ifaceItr < interfaceListSize; ++ifaceItr) {
            ...
            restartFile.ignore(std::numeric_limits<std::streamsize>::max(),
                               '\n');
            restartFile >> tmpIface.ifaceName >> tmpIface.absIfaceIndex >>
                tmpIface.relIfaceIndex;
            restartFile.ignore(std::numeric_limits<std::streamsize>::max(),
                               '\n');
            restartFile >> tmpIface.requiresState >>
                tmpIface.requiresInteraction;
            restartFile.ignore(std::numeric_limits<std::streamsize>::max(),
                               '\n');
        ...
        for (unsigned itr{0}; itr < rateListSize; ++itr) {
          ...
          // for (unsigned ifaceItr { 0 }; ifaceItr < otherIfaceListSize;
          // ++ifaceItr) {
          //     RxnIface tmpIface {};
          //     restartFile >> tmpIface.molTypeIndex >> tmpIface.ifaceName >>
          //     tmpIface.absIfaceIndex
          //         >> tmpIface.relIfaceIndex >> tmpIface.requiresState >>
          //         tmpIface.requiresInteraction;
          //     restartFile.ignore(std::numeric_limits<std::streamsize>::max(),
          //     '\n');
          //     tmpRate.otherIfaceLists.emplace_back(std::vector<RxnIface> {
          //     tmpIface });
          // }
    ...
    // write Molecules
    {
      ...
      restartFile >> molListSize >> Molecule::numberOfMolecules >>
          Molecule::maxID;
      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
      std::cout << "Mol list size and molecule.numberofMolecules: "
                << molListSize << ' ' << Molecule::numberOfMolecules
                << std::endl;
      ...
      //   restartFile >> key >> value;
      //   Molecule::mapIdToIndex[key] = value;
      // }
      // restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
      for (unsigned molItr{0}; molItr < molListSize; ++molItr) {
        Molecule tmpMol{};
        restartFile >> tmpMol.index >> tmpMol.isEmpty >> tmpMol.myComIndex >>
            tmpMol.molTypeIndex >> tmpMol.mySubVolIndex;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        restartFile >> tmpMol.mass >> tmpMol.isLipid >>
            tmpMol.isImplicitLipid >> tmpMol.linksToSurface >> tmpMol.isEmpty >>
            tmpMol.id >> tmpMol.isLeftGhost >> tmpMol.isLeftEdge >> tmpMol.isRightEdge >> tmpMol.isRightGhost >>
            tmpMol.receivedFromNeighborRank >> tmpMol.complexId;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

        // center of mass
        restartFile >> std::fixed >> tmpMol.comCoord.x >> tmpMol.comCoord.y >>
            tmpMol.comCoord.z;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        for (unsigned itr{0}; itr < interfaceListSize; ++itr) {
          ...
          restartFile >> tmpIface.index >> tmpIface.relIndex >>
              tmpIface.molTypeIndex >> tmpIface.stateIndex >>
              tmpIface.stateIden >> tmpIface.isBound >> tmpIface.excludeVolume;
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
          restartFile >> std::fixed >> tmpIface.coord.x >> tmpIface.coord.y >>
              tmpIface.coord.z;
          restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

          if (tmpIface.isBound) {
            restartFile >> tmpIface.interaction.partnerIndex >>
                tmpIface.interaction.partnerId >>
                tmpIface.interaction.partnerIfaceIndex >>
        ...
        // std::cout <<"read in : "<<tmpMol.index<<" first interface z crd:
        // "<<tmpMol.comCoord.z<<std::endl;
      }
    ...
    // read Complexes
    {
      ...
      restartFile >> comListSize >> Complex::numberOfComplexes >>
          Complex::maxID;
      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
      std::cout << " Ncomplexes including empties: " << comListSize
                << " N actual complexes: " << Complex::numberOfComplexes
                << std::endl;
      // size_t mapSize = 0;
      // restartFile >> mapSize;
      // for (size_t i = 0; i < mapSize; ++i) {
      //   size_t key = 0, value = 0;
      //   restartFile >> key >> value;
      //   Complex::mapIdToIndex[key] = value;
      // }
      // restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
      for (unsigned comItr{0}; comItr < comListSize; ++comItr) {
        Complex tmpCom{};
        restartFile >> tmpCom.index >> tmpCom.isEmpty >> tmpCom.radius >>
            tmpCom.mass >> tmpCom.id >> tmpCom.isLeftGhost >> tmpCom.isLeftEdge >> tmpCom.isRightEdge >> tmpCom.isRightGhost >> tmpCom.ownerRank >> tmpCom.receivedFromNeighborRank;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        restartFile >> tmpCom.linksToSurface >> tmpCom.iLipidIndex >>
            tmpCom.OnSurface;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        restartFile >> tmpCom.comCoord.x >> tmpCom.comCoord.y >>
            tmpCom.comCoord.z;
        restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    ...
    // read counterArrays
    {
      restartFile >> counterArrays.nLoops >>
          counterArrays.nCancelOverlapPartner >>
          counterArrays.nCancelOverlapSystem >>
          counterArrays.nCancelDisplace2D >> counterArrays.nCancelDisplace3D >>
          counterArrays.nCancelDisplace3Dto2D >> counterArrays.nCancelSpanBox >>
          counterArrays.nAssocSuccess >> counterArrays.eventArraySize;
      restartFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
