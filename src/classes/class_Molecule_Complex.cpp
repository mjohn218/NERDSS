  ...
  if (membraneObject.isSphere) {
    ...
    if (molTemplate.isPromoter) {
      ...
      for (unsigned int ifaceItr{0}; ifaceItr < molTemplate.interfaceList.size(); ++ifaceItr) {
        ...
        // initialize the Interface::State to the default state (first
        // listed)
        /*For each physical molecule, initialize the interfaces using
       the interfaces defined for the molTemplate, which were defined in
       parse_molFile.cpp at lines 1038.  Since each interface can exist in
...
void Molecule::print(MpiContext &mpiContext) const {
  ...
  std::cout << "Complex Id: " << complexId << '\n';
  std::cout << "Molecule Type Index: " << molTypeIndex << '\n';
  std::cout << "mySubVolIndex: " << mySubVolIndex << '\n';
  std::cout << "Index: " << index << '\n';
  std::cout << "Id: " << id << '\n';
  std::cout << "isLipid: " << isLipid << '\n';
  std::cout << "isImplicitLipid: " << isImplicitLipid << '\n';
  std::cout << "isEmpty: " << isEmpty << '\n';
  std::cout << "isGhosted: " << isGhosted << '\n';
  std::cout << "isShared: " << isShared << '\n';
  std::cout << "receivedFromNeighborRank: " << receivedFromNeighborRank << '\n';
  std::cout << "trajStatus: " << static_cast<int>(trajStatus) << '\n';
  ...
  std::cout << "linksToSurface: " << linksToSurface << '\n';
...
void Complex::update_properties(
  ...
  for (auto &memMol : memberList) {
    ...
    if (moleculeList[memMol].isPromoter) onFiber = true;
    if (moleculeList[memMol].isLipid) OnSurface = true;
  }
  if (linksToSurface > 0){
    OnSurface = true;
  }
  ...
  */
  ...
  // if (D.z < 1E-50 && OnSurface == true){
  //   std::cout << "OnSurface is " << OnSurface << " with D.Z = " << D.z
  //             << ", and linksToSurface = " << linksToSurface << std::endl;
  //   for (auto &memMol : memberList) {
  //     std::cout << "isLipid? " << moleculeList[memMol].isLipid << std::endl;
  //     std::cout << "isImplicitLipid? " << moleculeList[memMol].isImplicitLipid
  //               << std::endl;
  //   }
  // }
  // if (D.z < 1E-50) {
