...
bool Observable::Constituent::Iface::operator==(
    const RxnIface& rxnIface) const {
  return (relIndex == rxnIface.relIfaceIndex) &&
         (isBound == rxnIface.relIfaceIndex);
}
...
bool Observable::operator==(const Molecule& mol) const {
  if (constituentList[0].molTypeIndex != mol.molTypeIndex) return false;
