...
/* RXNIFACE */
RxnIface::RxnIface(std::string ifaceName, int molTypeIndex, int absIfaceIndex,
...
std::ostream& operator<<(std::ostream& os, const RxnIface& rxnIface) {
  os << "[iface name: " << rxnIface.ifaceName
     << ", iface index: " << rxnIface.absIfaceIndex;
...
bool RxnIface::operator==(const Molecule::Iface& molIface) const {
  //    return this->molTypeIndex == molIface.molTypeIndex &&
  //    this->absIfaceIndex == molIface.index
  //        && this->requiresState == molIface.stateIden &&
  //        this->requiresInteraction == molIface.isBound;
  // this can't check absIfaceIndex, since bound interfaces in the reactants
  // can't have indexed bonds, yet
  ...
  // return this->molTypeIndex == molIface.molTypeIndex && this->relIfaceIndex
  // == molIface.relIndex
  //     && this->requiresState == molIface.stateIden &&
  //     this->requiresInteraction == molIface.isBound;
}
...
/* FORWARDRXN */
ForwardRxn::ForwardRxn(ParsedRxn& parsedRxn,
  ...
  for (auto& mol : parsedRxn.reactantList) {
    for (auto& iface : mol.interfaceList) {
      if (iface.ifaceRxnStatus == Involvement::ancillary) {
        ...
        //                    iface.ifaceName, mol.molTypeIndex, iface.absIndex,
        //                    iface.relIndex, iface.state, iface.isBound);
      } else if (iface.ifaceRxnStatus == Involvement::stateChange) {
        stateChangeIface.first =
            RxnIface(iface.ifaceName, mol.molTypeIndex, iface.absIndex,
                     iface.relIndex, iface.state, iface.isBound);
      }
    }
  }
  ...
  for (auto& reactant : parsedRxn.rxnReactants) {
    reactantListNew.emplace_back(reactant.ifaceName, reactant.molTypeIndex,
                                 reactant.absIndex, reactant.relIndex,
                                 reactant.state, reactant.isBound);
  }

  for (auto& product : parsedRxn.rxnProducts) {
    productListNew.emplace_back(product.ifaceName, product.molTypeIndex,
                                product.absIndex, product.relIndex,
  ...
  for (unsigned prodItr{0}; prodItr < productListNew.size(); ++prodItr) {
    if (productListNew[prodItr].relIfaceIndex == -1)
      productListNew[prodItr].relIfaceIndex =
          reactantListNew[prodItr].relIfaceIndex;
  }
}
  ...
  if (rxnType != ReactionType::uniMolStateChange) {
    ...
    if (isCoupled)
      std::cout << " coupledRxn Number: " << coupledRxn.absRxnIndex
                ...
                << " prob to perform coupled: " << coupledRxn.probCoupled
                << std::endl;
  }
}

void ForwardRxn::assoc_display(
    const std::vector<MolTemplate>& molTemplateList) const {
  std::cout << "\nReaction data:\n";
  // TODO: break this off into the constructor, and maybe put it onto RateState,
  // because each conditional rate will have different otherIfaceList and/or
  // stateChangeIface.
  std::cout << "Product: ";
  std::cout << molTemplateList[productListNew.front().molTypeIndex].molName
            << '(' << productListNew.front().ifaceName << "!1)";
  for (const auto& product : productListNew)
    std::cout << '.' << molTemplateList[product.molTypeIndex].molName << '('
              << product.ifaceName << "!1)";
  std::cout << "\nAngles:\n";
  assocAngles.display();
  std::cout << std::endl;
...
BackRxn::BackRxn(double offRatekb, ForwardRxn& forwardRxn) {
  ...
  stateChangeIface = std::pair<RxnIface, RxnIface>{
      forwardRxn.stateChangeIface.second, forwardRxn.stateChangeIface.first};
  ...
  //        if (intReactantList.size() > 1 && intReactantList[0] >
  //        intReactantList[1]) {
...
void BackRxn::display() const {
  ...
  if (!hasStateChange && rxnType == ReactionType::bimolecular) {
    std::cout << "Reactants:\n";
    for (auto& reactant : reactantListNew) std::cout << ' ' << reactant << '\n';
    std::cout << std::endl;
    std::cout << "Products:\n";
    for (auto& product : productListNew) std::cout << ' ' << product << '\n';
    std::cout << std::endl;
  } else if (rxnType == ReactionType::biMolStateChange) {
    std::cout << "Facilitator: ";
    for (auto& reactant : reactantListNew) {
...
CreateDestructRxn::CreateDestructRxn(
    ParsedRxn& parsedRxn, const std::vector<MolTemplate>& molTemplateList) {
  // set booleans
  isOnMem = parsedRxn.isOnMem;
  isSymmetric = parsedRxn.isSymmetric;
  hasStateChange = parsedRxn.hasStateChange;
  ...
  for (const auto& reactant : parsedRxn.reactantList) {
    ...
    for (const auto& tempIface :
         molTemplateList[reactant.molTypeIndex].interfaceList) {
      RxnIface tmpIface{};
  ...
  for (const auto& product : parsedRxn.productList) {
    ...
    for (const auto& tempIface :
         molTemplateList[product.molTypeIndex].interfaceList) {
      RxnIface tmpIface{};
      tmpIface.ifaceName = tempIface.name;
  ...
  for (const auto& reactant : parsedRxn.rxnReactants) {
    reactantMolList[reactant.speciesIndex]
        .interfaceList[reactant.relIndex]
        .requiresState = reactant.state;
    reactantMolList[reactant.speciesIndex]
        .interfaceList[reactant.relIndex]
        .requiresInteraction = reactant.isBound;
  }

  for (const auto& product : parsedRxn.rxnProducts) {
    productMolList[product.speciesIndex]
        .interfaceList[product.relIndex]
        .requiresState = product.state;
    productMolList[product.speciesIndex]
        .interfaceList[product.relIndex]
        .requiresInteraction = product.isBound;
  }
