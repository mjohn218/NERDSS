...
Vector::Vector(const Coord& coordEnd, const Coord& coordStart) {
  /// constructor which takes in two points and outputs a vector with the start
  /// point at the origin
...
Vector::Vector(std::array<double, 3>& arr) : Coord(arr) {
  x = arr[0];
  y = arr[1];
  z = arr[2];
}
...
Vector::Vector(std::vector<double> arr) {
  if (arr.size() != 3) {
    std::cerr
        << "ERROR: Attempting to create Vector from an array with size > 3.";
    exit(1);
  }
...
Coord operator+(const Vector& vec, const Coord& crd) {
...
double Vector::dot_theta(const Vector& vec) const {
  ...
  if (std::abs(cTheta - 1) < 1E-12) cTheta = 1;
  if (std::abs(-1 - cTheta) < 1E-12) cTheta = -1;

  if (this->magnitude < 1E-8 || vec.magnitude < 1E-8) {
    // std::cout << "WARNING: Attempted dot product with vector of magnitude
    // 0.\n";
    return 0.0;
  } else
    return acos(cTheta);
...
void Vector::normalize() {
  ...
  if (magnitude == 0) {
    ...
    magnitude = 1;
    //    std::cout <<"WARNING: NORMALIZING A VECTOR OF ZERO LENGTH
    //    "<<std::endl;
  }
  *this = *this /= magnitude;
  calc_magnitude();
...
void Vector::calc_magnitude() { this->magnitude = sqrt(x * x + y * y + z * z); }
