...
std::map<const std::string, ParamKeyword> parmKeywords = {
    {"nummoltypes", ParamKeyword::numMolTypes},
    {"numtotalspecies", ParamKeyword::numTotalSpecies},
    {"nitr", ParamKeyword::nItr},
    {"fromrestart", ParamKeyword::fromRestart},
    {"timewrite", ParamKeyword::timeWrite},
    {"trajwrite", ParamKeyword::trajWrite},
    {"timestep", ParamKeyword::timeStep},
    {"numtotalcomplex", ParamKeyword::numTotalComplex},
    {"mass", ParamKeyword::mass},
    {"restartwrite", ParamKeyword::restartWrite},
    {"pdbwrite", ParamKeyword::pdbWrite},
    {"overlapseplimit", ParamKeyword::overlapSepLimit},
    {"name", ParamKeyword::name},
    {"checkpoint", ParamKeyword::checkPoint},
    {"scalemaxdisplace", ParamKeyword::scaleMaxDisplace},
...
void Parameters::set_value(std::string value, ParamKeyword keywords) {
   ...
   * @param keywords the keyword parsed from the input file to match to the
  ...
  try {
    ...
    switch (key) {
      ...
      case 1:
        ...
        std::cout << "Read in numTotalSpecies: " << this->numTotalSpecies
                  << std::endl;
        break;
      case 2:
        nit = std::stod(value);
        this->nItr = (long long)(nit);  // std::stoi(value);
        std::cout << "Read in nItr: " << this->nItr << " timeSteps"
                  << std::endl;
        break;
      case 3:
        this->fromRestart = read_boolean(value);
        std::cout << "Read in fromRestart: " << std::boolalpha
                  << this->fromRestart << std::endl;
        break;
      case 4:
        this->timeWrite = std::stoi(value);
        std::cout << "Read in timeWrite: " << this->timeWrite << " timeSteps"
                  << std::endl;
        break;
      case 5:
        this->trajWrite = std::stoi(value);
        std::cout << "Read in trajWrite: " << this->trajWrite << " timeSteps"
                  << std::endl;
        break;
      case 6:
        this->timeStep = std::stod(value);
        std::cout << "Read in timeStep: " << this->timeStep << " us"
                  << std::endl;
        break;
      case 7:
        this->numTotalComplex = std::stoi(value);
        std::cout << "Read in numTotalComplex: " << this->numTotalComplex
                  << std::endl;
        break;
      case 8:
      ...
      case 10:
        ...
        std::cout << "Read in restartWrite: " << this->restartWrite
                  << " timeSteps" << std::endl;
        break;
      case 11:
        this->pdbWrite = std::stoi(value);
        std::cout << "Read in pdbWrite: " << this->pdbWrite << " timeSteps"
                  << std::endl;
        break;
      case 12:
        this->overlapSepLimit = std::stod(value);
        std::cout << "Read in overlapSepLimit: " << this->overlapSepLimit
                  << " nm" << std::endl;
        break;
      case 13:
        this->name = value;
      ...
      case 14:
        ...
        std::cout << "Read in checkPoint: " << this->checkPoint << " timeSteps"
                  << std::endl;
        break;
      case 15:
        this->scaleMaxDisplace = std::stod(value);
        std::cout << "Read in scaleMaxDisplace: " << this->scaleMaxDisplace
                  << std::endl;
        break;
      case 16:
        transitionit = std::stod(value);
        this->transitionWrite = (long long)(transitionit);
        std::cout << "Read in transitionWrite: " << this->transitionWrite
                  << " timeSteps" << std::endl;
        break;
      case 17:
        this->clusterOverlapCheck = read_boolean(value);
        std::cout << "Read in clusterOverlapCheck: " << std::boolalpha
                  << this->clusterOverlapCheck << std::endl;
        break;
      default:
    ...
    for (auto lineItr = line.begin(); lineItr != line.end(); ++lineItr) {
      if (std::isalnum(*lineItr))
        ...
        line.erase(line.begin(),
                   lineItr + 1);  // + 1 removes the '=' sign. could make this
                                  // erase(remove_if)
...
void Parameters::display() {
  ...
  std::cout << "Timestep log interval (timeWrite): " << timeWrite
            << " timesteps\n";
  std::cout << "Restart file write interval: " << restartWrite
            << " timesteps\n";
  std::cout << "Coordinate write interval (trajWrite): " << trajWrite
            << " timesteps\n";
  ...
  std::cout << "Transition matrix write interval: " << transitionWrite
            << " timesteps\n";
  ...
  std::cout << "Total number of unique interfaces and states, including "
               "product states: "
            << numTotalSpecies << '\n';
  std::cout << "Total number of complexes in system at start: "
            << numTotalComplex << '\n';
  std::cout
      << "Total number of units (molecules + interfaces) in system at start: "
      << numTotalUnits << '\n';
