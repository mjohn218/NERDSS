 ...
 * ### Purpose
 * ***
 * Contains class member functions for classes ParsedMol and ParsedRxn found in
 * bngl_parsing_class.hpp
 *
 * ### TODO List
 * ***
 *  - TODO (PRIORITY MED): Function to do final checking of ParsedRxn before
 * conversion into ForwardRxn(s)/CreateDestructRxn
 */
...
std::ostream& operator<<(std::ostream& os, const ReactionType& rxnType) {
  // scoped enumerations are not implicilty convertible. must be explicitly
  // converted
...
bool operator==(const std::vector<RxnIface>& rxnIfaceList,
                const std::vector<ParsedMol::IfaceInfo>& parsedIfaceList) {
  if (parsedIfaceList.size() != rxnIfaceList.size()) return false;

  unsigned numMatches{0};
  for (unsigned rxnItr{0}; rxnItr < rxnIfaceList.size(); ++rxnItr) {
    for (unsigned parsedItr{0}; parsedItr < parsedIfaceList.size();
         ++parsedItr) {
      if (rxnIfaceList[rxnItr].requiresInteraction ==
              parsedIfaceList[parsedItr].isBound &&
          rxnIfaceList[rxnItr].requiresState ==
              parsedIfaceList[parsedItr].state &&
          rxnIfaceList[rxnItr].molTypeIndex ==
              parsedIfaceList[parsedItr].molTypeIndex) {
        ++numMatches;
        break;
      }
    }
...
/* CONSTRUCTORS */
ParsedMol::IfaceInfo::IfaceInfo(std::string _ifaceName, char _state,
                                bool _isBound, Involvement _ifaceRxnStatus,
                                int speciesIndex)
...
ParsedMol::IfaceInfo::IfaceInfo(std::string _ifaceName, char _state,
                                bool _isBound, int _bondIndex,
                                Involvement _ifaceRxnStatus)
    : ifaceName(_ifaceName),
      state(_state),
      isBound(_isBound),
...
ParsedMol::IfaceInfo::IfaceInfo(std::string _ifaceName, char _state,
                                bool _isBound, int _bondIndex,
                                Involvement _ifaceRxnStatus, int speciesIndex)
    : ifaceName(_ifaceName),
      state(_state),
      isBound(_isBound),
      bondIndex(_bondIndex),
...
bool ParsedMol::IfaceInfo::operator!=(const ParsedMol::IfaceInfo& iface) const {
  return ((bool{this->absIndex != iface.absIndex}) ||
          (bool{this->state != iface.state}) ||
          (bool{this->isBound != iface.isBound}) ||
          (bool{this->bondIndex != iface.bondIndex}));
}
...
bool ParsedMol::IfaceInfo::operator==(const RxnIface& rxnIface) const {
  return (bool{this->absIndex == rxnIface.absIfaceIndex} &&
          bool{this->state == rxnIface.requiresState} &&
          bool{this->isBound == rxnIface.requiresInteraction});
}

std::ostream& operator<<(std::ostream& os,
                         const ParsedMol::IfaceInfo& oneInfo) {
  os << oneInfo.absIndex << ", ";
  if (oneInfo.state != '\0')
    os << oneInfo.state << ", ";
  else
    os << "NO STATE, ";
  os << std::boolalpha << oneInfo.isBound << ", " << oneInfo.bondIndex << ", "
     << oneInfo.ifaceRxnStatus;
  return os;
}

bool ParsedMol::IfaceInfo::operator<(const ParsedMol::IfaceInfo& rhs) const {
  return speciesIndex < rhs.speciesIndex;
}
...
void ParsedMol::IfaceInfo::change_ifaceRxnStatus(int newIndex,
                                                 Involvement newRxnStatus) {
...
bool ParsedMol::operator==(const ParsedMol& mol1) const {
  if ((this->molName != mol1.molName) ||
      (this->interfaceList.size() != mol1.interfaceList.size()))
    return false;
  for (auto& thisTargetIface : this->interfaceList) {
    ...
    auto ifacePos =
        std::find_if(mol1.interfaceList.begin(), mol1.interfaceList.end(),
                     [&](const ParsedMol::IfaceInfo& iface) {
                       return iface.ifaceName == thisTargetIface.ifaceName;
                     });
...
void ParsedMol::set_molTypeIndex(
  ...
  if (this->molTypeIndex == -1) {
    std::cerr << "Error, molecule " << this->molName
              << " not found. Exiting.\n";
    exit(1);
  }
}

void ParsedRxn::assemble_reactions(
    std::vector<ForwardRxn>& forwardRxns, std::vector<BackRxn>& backRxns,
    std::vector<CreateDestructRxn>& createDestructRxns,
    const std::vector<MolTemplate>& molTemplateList) {
  if (this->rxnType == ReactionType::bimolecular ||
      this->rxnType == ReactionType::uniMolStateChange ||
...
/* ParsedRxn */
std::pair<bool, std::string> ParsedRxn::isComplete(
    const std::vector<MolTemplate>& molTemplateList) {
  ...
  if (rxnType == ReactionType::bimolecular) {
    ...
    if (!molTemplateList[reactantList[0].molTypeIndex].isPoint &&
        !molTemplateList[reactantList[1].molTypeIndex].isPoint) {
      if (!molTemplateList.at(reactantList[0].molTypeIndex).isRod &&
          (norm1.magnitude == 0 || std::isnan(norm1.magnitude))) {
        return std::make_pair(
            false, "Norm1 is not set or is not a vector for reaction");
      }
      if (!molTemplateList.at(reactantList[1].molTypeIndex).isRod &&
          (norm2.magnitude == 0 || std::isnan(norm2.magnitude))) {
        return std::make_pair(
            false, "Norm2 is not set or is not a vector for reaction");
      }
    }
  }
...
void ParsedRxn::display_angles() const {
  std::cout << std::setw(10) << std::left << "Theta 1" << std::setw(7)
            << std::right << assocAngles.theta1 << '\n';
  std::cout << std::setw(10) << std::left << "Theta 2" << std::setw(7)
            << std::right << assocAngles.theta2 << '\n';
  std::cout << std::setw(10) << std::left << "Phi 1" << std::setw(7)
            << std::right << assocAngles.phi1 << '\n';
  std::cout << std::setw(10) << std::left << "Phi 2" << std::setw(7)
            << std::right << assocAngles.phi2 << '\n';
  std::cout << std::setw(10) << std::left << "Omega" << std::setw(7)
            << std::right << assocAngles.omega << '\n';
}
  ...
  try {
    switch (key) {
      case 0: {
        ...
        std::cout << "Read in value of onRate3Dka: " << onRate3Dka
                  << "nm^3us^-1\n";
        break;
      }
      case 1: {
        onRate3DMacro = std::stod(line);
        std::cout << "Read in value of onRate3DMacro: " << onRate3DMacro
                  << "uM^-1us^-1\n";
        break;
      }
      case 2: {
      ...
      case 3: {
        ...
        std::cout << "Read in value of offRateMacro: " << offRateMacro
                  << "s^-1\n";
        break;
      }
      case 4: {
      ...
      case 7: {
        ...
        std::cout << "Read in value of assocAngles: [theta1: "
                  << assocAngles.theta1 << ", theta2: " << assocAngles.theta2
                  << ", phi1: " << assocAngles.phi1
                  << ", phi2: " << assocAngles.phi2
                  << ", omega: " << assocAngles.omega << "]" << '\n';
        break;
      }
      case 8: {
      ...
      case 10: {
        ...
        std::cout << "Read in value of coupledRxn: absRxnIndex, "
                  << coupledRxn.absRxnIndex << '\n';
        break;
      }
      case 11: {
        isObserved = true;
      ...
      default: { throw std::invalid_argument("Not a valid keyword."); }
    }
...
void ParsedRxn::display() const {
  /*THIS ROUTINE IS DUPLICATED IN CLASS_RXNS>CPP, FORWARDRXN::DISPLAY, and OTHER
   * RXN TYPES, THIS IS NOT CALLED!*/
  ...
  if (hasStateChange) {
    std::cout << "Interface " << stateChangeIface.first.ifaceName
              << " on molecule " << stateChangeIface.first.molTypeIndex
              << " changes state from " << stateChangeIface.first.requiresState
              << " to " << stateChangeIface.second.requiresState << '\n';
  }
  ...
  if (isCoupled)
    std::cout << " coupledRxn Number: " << coupledRxn.absRxnIndex
              << " type: " << coupledRxn.rxnType << '\n';
  std::cout << "microRate3D: " << onRate3Dka << '\n';
  std::cout << "macroRate3D: " << onRate3DMacro << '\n';
  if (isReversible) std::cout << "micro Off Rate: " << offRatekb << '\n';
  std::cout << "macro Off Rate: " << offRateMacro << '\n';
}

void ParsedRxn::check_previous_bound_states(
    int& totSpecies, const std::vector<ForwardRxn>& forwardRxns,
    const std::vector<MolTemplate>& molTemplateList) {
    ...
    if (oneRxn.rxnType == ReactionType::bimolecular) {
      for (auto& oneProduct : oneRxn.productListNew) {
        if (oneProduct.requiresState == this->rxnReactants[0].state) {
          // add species for the biomolecular Product with other states of
          // oneProduct
          for (auto& oneState :
               molTemplateList[this->rxnReactants[0].molTypeIndex]
                   .interfaceList[this->rxnReactants[0].relIndex]
                   .stateList) {
  ...
  std::cout << "Warning: must declare association reaction before declaring a "
               "state change of its product.\n";
}
...
void ParsedRxn::determine_reactants() {
  ...
  if (rxnType == ReactionType::uniMolStateChange) {
    // - if it is stateChange, we already know the reactants. this could be done
    // earlier, but it's better for code legibility to have all the reactant
    // determination done in one place, I think
    int numberOfMatches{
        0};  // index to keep track of matches. should only be 1 at the end
    ...
    for (auto& reactant : reactantList) {
      for (auto& reactIface : reactant.interfaceList) {
        ...
        if (reactIface.ifaceRxnStatus == Involvement::stateChange) {
          ...
          // if the reactant is still listed as possible, look for a matching
          // possible iface in the product. if they're equal, they're ancillary
          for (auto& product : productList) {
            auto prodIfaceItr = std::find_if(
                product.interfaceList.begin(), product.interfaceList.end(),
                [&](const ParsedMol::IfaceInfo& prodIface) -> bool {
                  return reactIface == prodIface;
                });
    ...
    std::vector<int> speciesUsed;  // keep track of which reaction species
                                   // you've found a reactant on
    for (auto& product : productList) {
      for (auto& prodIface : product.interfaceList) {
        ...
        if (prodIface.ifaceRxnStatus == Involvement::interactionChange) {
          ...
          // make sure there's only one match. if two, throw to something to see
          // if it matters
          int numberOfMatches{0};
          for (auto& reactant : reactantList) {
            // only check if the interface lists are the same size
            if (reactant.specieIndex == product.specieIndex &&
                reactant.molTypeIndex == product.molTypeIndex &&
                reactant.interfaceList.size() == product.interfaceList.size()) {
              // find the interface with the same name and only on a different
              // species than we've already gotten a reactant from
              auto reactIfaceItr = std::find_if(
                  reactant.interfaceList.begin(), reactant.interfaceList.end(),
                  [&](const ParsedMol::IfaceInfo& reactIface) -> bool {
                    return reactIface.ifaceName == prodIface.ifaceName &&
                           std::find(speciesUsed.begin(), speciesUsed.end(),
                                     reactIface.speciesIndex) ==
                               speciesUsed.end();
                  });

              if (reactIfaceItr != reactant.interfaceList.end()) {
                std::cout << "Found corresponding interfaces: React: "
                          << reactant.molName << '(' << reactIfaceItr->ifaceName
                          << ')' << " -- Prod: " << product.molName << '('
                          << prodIface.ifaceName << ')' << '\n';

                // we found a matching interface in the reactants. Make it a
                // reactant
          ...
          for (auto& reactant : reactantList) {
            if ((prodIface.speciesIndex == reactant.specieIndex) &&
                (reactant.molTypeIndex == product.molTypeIndex) &&
                (reactant.interfaceList.size() ==
                 product.interfaceList.size())) {
              auto reactIfaceItr = std::find_if(
                  reactant.interfaceList.begin(), reactant.interfaceList.end(),
                  [&](const ParsedMol::IfaceInfo& reactIface) {
                    return reactIface.ifaceName == prodIface.ifaceName &&
                           std::find(speciesUsed.begin(), speciesUsed.end(),
                                     reactIface.speciesIndex) ==
                               speciesUsed.end();
    ...
    if (this->rxnType == ReactionType::biMolStateChange) {
      ...
      ParsedMol::IfaceInfo& facilIface =
          reactantList[facilIndex].interfaceList.at(0);
    ...
    if (rxnReactants.size() != 2 &&
        this->rxnType == ReactionType::bimolecular) {
      std::cerr << "Error determining reactants, check reaction file for "
                   "missing interfaces."
                << '\n';
...
void ParsedRxn::determine_creation_products(
    const std::vector<MolTemplate>& molTemplateList) {
  for (auto& reactant : reactantList) {
    try {
      auto tempNameItr =
          std::find_if(molTemplateList.begin(), molTemplateList.end(),
                       [&](const MolTemplate& oneTemp) -> bool {
                         return oneTemp.molName == reactant.molName;
                       });
  ...
  for (auto& product : productList) {
    try {
      auto tempNameItr =
          std::find_if(molTemplateList.begin(), molTemplateList.end(),
                       [&](const MolTemplate& oneTemp) -> bool {
                         return oneTemp.molName == product.molName;
                       });
...
void ParsedRxn::create_other_iface_lists(
    const std::vector<MolTemplate>& molTemplateList) {
  for (auto& reactant : reactantList) {
    std::vector<RxnIface> tmpIfaceVec;
    for (auto& reactIface : reactant.interfaceList) {
      // if the iface's status is ancillary, create a new element in the vector
      // holding those ifaces
      if (reactIface.ifaceRxnStatus == Involvement::ancillary) {
        int relIface{
            molTemplateList[reactant.molTypeIndex].find_relIndex_from_absIndex(
                reactIface.absIndex)};
        tmpIfaceVec.emplace_back(
            reactIface.ifaceName, reactant.molTypeIndex, reactIface.absIndex,
            relIface, (reactIface.state == '\0') ? '\0' : reactIface.state,
            reactIface.isBound);
      }
    }
...
bool ParsedRxn::check_for_conditional_rates(
    int& totSpecies, std::vector<ForwardRxn>& forwardRxns,
    std::vector<BackRxn>& backRxns) {
  // sort the reactants in order according to their iface index, to make
  // comparing reactions easier don't do this for bimolecular state changes,
  // since we put the facilitator first, regardless of its index
  //    if (rxnType != ReactionType::biMolStateChange) {
  //        if (intReactantList.size() == 2 && intReactantList[0] >
  //        intReactantList[1]) {
  ...
  for (auto& oneRxn : forwardRxns) {
    ...
    if (tmpVec1 == tmpVec2 && rxnType == oneRxn.rxnType) {
      // if the values had to be swapped, swap everything so the order is the
      // same as the parent reaction
      ...
      // ...give that ForwardRxn this on rate and list of ancillary
      // interfaces...
      oneRxn.rateList.emplace_back(onRate3Dka, otherIfaceLists);
      // ...and the conjugate BackRxn the off rate and list of ancillary
      // interfaces. and interfaces which change state (swapped, of course)
      if (oneRxn.isReversible) {
        backRxns[oneRxn.conjBackRxnIndex].rateList.emplace_back(
            this->offRatekb, otherIfaceLists);
      }
      --totSpecies;  // since it's not a new reaction, reduce the number of
                     // total species...
      std::cout << "Forward Reaction " << &oneRxn - &forwardRxns[0]
                << " has been updated with a new rate:\nRate:" << onRate3Dka
                << '\n';
      std::cout << "Reactant 1 requires interfaces:\n";
      for (auto& iface : otherIfaceLists[0]) std::cout << iface << '\n';
      if (rxnType == ReactionType::bimolecular ||
          rxnType == ReactionType::biMolStateChange) {
        std::cout << "Reactant 2 requires interfaces:\n";
        for (auto& iface : otherIfaceLists[1]) std::cout << iface << '\n';
      }
      return true;
    }
  }
...
ParsedRxn ParsedRxn::make_one_split_reaction(
    int& totSpecies, int parsedMolIndex, ParsedMol::IfaceInfo& reactIface,
    const Interface::State& tempState) {
  ++totSpecies;
  ParsedRxn newRxn(*this);

  auto newIfaceList = newRxn.reactantList[parsedMolIndex].interfaceList;
  // look through the list of interfaces for the interface which we need to make
  // the multiple reactions for
  for (auto& newIface : newIfaceList) {
    if (newIface.ifaceName == reactIface.ifaceName) {
      newIface.state = tempState.iden;
      // if the interface isn't bound, give it the index of the MolTemplate
      // interface state
      if (!newIface.isBound) newIface.absIndex = tempState.index;

      // update the reactant lists
      for (auto& oneReact : newRxn.rxnReactants) {
        // only need to update the IfaceInfo, the name should stay the same
        // (also it doesn't compile if you try to change the map key)
        if (oneReact.ifaceName == newIface.ifaceName) oneReact = newIface;
      }
      ...
      newRxn.intProductList =
          std::vector<int>(newRxn.intProductList.size(), totSpecies);
...
std::vector<ParsedRxn> ParsedRxn::split_into_multiple_reactions(
  ...
  for (auto& reactIface : noStateList) {
    --totSpecies;  // need to remove a species for each interface we split into
                   // multiple reactions
    ...
    auto tempNameItr =
        std::find_if(molTemplateList.begin(), molTemplateList.end(),
                     [&](const MolTemplate& oneTemp) -> bool {
                       return oneTemp.molName == thisMol.molName;
                     });
    // reactIface.first looks bad, but reactIface.second = the iface element of
    // noStateList, reactIface.first is that iface's name
    auto tempIfaceItr = std::find_if(
        tempNameItr->interfaceList.begin(), tempNameItr->interfaceList.end(),
        [&](const Interface& oneIface) -> bool {
          return oneIface.name == reactIface.second.ifaceName;
        });

    // TODO: error for out of range

    // if only one interface has no explicit state, we can just make the new
    // reactions immediately
    for (auto& tempState : tempIfaceItr->stateList)
      newRxns.emplace_back(make_one_split_reaction(
          totSpecies, reactIface.first, reactIface.second, tempState));
  }
