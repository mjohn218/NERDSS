...
// Constructors
Interface::State::State(int index) : index(index) {}

Interface::Interface(std::string name, const Coord& iCoord)
    : iCoord(iCoord), name(std::move(name)) {}

Interface::State::State(const std::string& ifaceAndStateName, char iden,
                        int index)
    : ifaceAndStateName(ifaceAndStateName), iden(iden), index(index) {}
Interface::State::State(const std::string& ifaceAndStateName, int index)
    : ifaceAndStateName(ifaceAndStateName), index(index) {}

Interface::Interface(std::string name, std::vector<Interface::State> states,
                     Coord iCoord)
    : iCoord(iCoord), name(name), stateList(states) {
...
// Constructors
MolTemplate::MolTemplate(std::string molName,
                         std::vector<Interface>& interfaceList)
    : molName(molName), interfaceList(interfaceList) {}

MolTemplate::MolTemplate(Coord& comCoord, std::vector<Interface>& Interfaces)
    : comCoord(comCoord), interfaceList(Interfaces) {}
  ...
  for (auto& iface : interfaceList) {
    if (iface.stateList.size() == 1) {
      ...
      for (auto& state : iface.stateList) {
        std::cout << ""
                  << "(identity: " << state.iden
                  << ", absolute index: " << state.index << ")\n";
        //                if (!state.myForwardRxns.empty()) {
        //                    std::cout << "Forward Reactions: ";
        //                    for (auto& rxn : state.myForwardRxns)
        //                        for (auto& rxn :
        //                        iface.stateList[0].myCreateDestructRxns)
        //                    std::cout << '\n';
        //                }
...
void MolTemplate::display(const std::string& name) const {
  ...
  std::cout << "Radius: " << radius << '\n';
  std::cout << comCoord << '\n';
  for (auto& iface : interfaceList) std::cout << iface.iCoord << '\n';
  std::cout << '\n';
}

int MolTemplate::find_relIndex_from_absIndex(int targStateIndex) const {
  /*!
   * \brief Finds the relative index of Interface given the absolute index of
   * the State
   */
...
int MolTemplate::find_absIndex_from_relIndex(int relIndex, char state) const {
  ...
  std::cerr << "Absolute index for state " << state
            << " cannot be found on interface " << relIndex << ".\n";
  exit(1);
}

void MolTemplate::set_value(std::string& line, MolKeyword molKeyword) {
  /*! \ingroup Parser
   * \brief sets the value of a MolTemplate's variable based on the keyword
   * parsed from the parameter input file
   */
  ...
  switch (key) {
    ...
    case 3: {
      ...
      std::cout << "Read in isLipid: " << std::boolalpha << isLipid
                << std::endl;
      break;
    }
    case 4: {
      D = Coord(parse_input_array(line));
      std::cout << "Read in D: [" << D.x << "um^2s^-1, " << D.y << "um^2s^-1, "
                << D.z << "um^2s^-1]" << std::endl;
      break;
    }
    case 5: {
      Dr = Coord(parse_input_array(line));
      std::cout << "Read in Dr: [" << Dr.x << "rad^2s^-1, " << Dr.y
                << "rad^2s^-1, " << Dr.z << "rad^2s^-1]" << std::endl;
      break;
    }
    case 8: {
      mass = std::stod(line);
    ...
    case 9: {
      ...
      std::cout << "Read in checkOverlap: " << std::boolalpha << checkOverlap
                << std::endl;
      break;
    }
    case 11: {
    ...
    case 12: {
      ...
      std::cout << "Read in isImplicitLipid: " << std::boolalpha
                << isImplicitLipid << std::endl;
      break;
    }
    case 13: {
      countTransition = read_boolean(line);
      std::cout << "Read in countTransition: " << std::boolalpha
                << countTransition << std::endl;
      break;
    }
    case 14: {
...
size_t find_molTypeIndex_from_ifaceIndex(
    const int& targIfaceIndex,
    const std::vector<MolTemplate>& molTemplateList) {
  /* this function should find the molTypeIndex of the parent MolTemplate of am
   * absolute ifaceIndex */
  try {
    for (auto& oneTemplate : molTemplateList) {
      for (auto& oneIface : oneTemplate.interfaceList) {
        for (auto& oneState : oneIface.stateList) {
          if (oneState.index == targIfaceIndex) return oneTemplate.molTypeIndex;
        }
      }
    }
    throw std::out_of_range(
        "Interface::State index cannot be found in list of MolTemplates...");
