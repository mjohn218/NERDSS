...
void determine_iface_indices(int specieIndex, int& totSpecies,
  ...
  for (auto tempItr = molTemplateList.begin(); tempItr != molTemplateList.end();
       ++tempItr) {
    if (tempItr->molName == targMol.molName) {
      ...
      for (auto targIfaceItr = targMol.interfaceList.begin();
           targIfaceItr != targMol.interfaceList.end(); ++targIfaceItr) {
        // iterate over the template ifaces
        for (auto tempIfaceItr = tempItr->interfaceList.begin();
             tempIfaceItr != tempItr->interfaceList.end(); ++tempIfaceItr) {
          if (tempIfaceItr->name == targIfaceItr->ifaceName) {
            ...
            // TODO: can split this up into several if statements to stop it
            // from being so nested
            if (targIfaceItr->absIndex == -1) {
              // if the Interface::states vector is larger than 1, the states
              // are named
              if (tempIfaceItr->stateList.size() == 1) {
                ...
                if (parsedRxn.rxnType != ReactionType::zerothOrderCreation &&
                  ...
                  std::cerr << "WARNING: Creation and Destruction reactions "
                               "must have states explicitly "
                               "stated. Using default states.\n";
                }
              } else {
                for (auto& tempState : tempIfaceItr->stateList) {
                  if (targIfaceItr->state == tempState.iden) {
                    // set the index and tell the parser that the index was
                    // found
              ...
              // if the iface is bound (and not a wildcard), we know it has to
              // be an iface that has changed interaction
              if (targIfaceItr->isBound && targIfaceItr->bondIndex != 0) {
                determine_bound_iface_index(totSpecies, *targIfaceItr,
                                            parsedRxn, forwardRxns,
                                            molTemplateList);
              }
            }
          }
