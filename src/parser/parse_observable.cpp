...
SpeciesTracker::Observable parse_observable(
    const std::string& line, const std::vector<MolTemplate>& molTemplateList,
  ...
  std::string specie;
  {
    ...
    std::transform(obsTypeStr.begin(), obsTypeStr.end(), obsTypeStr.begin(),
                   ::tolower);
    auto obsTypeItr = observableTypes.find(obsTypeStr);
    if (obsTypeItr == observableTypes.end()) {
      std::cerr << "FATAL ERROR: Observable type " << obsTypeStr
                << " unknown. Exiting.\n";
      exit(1);
    } else {
  ...
  // split into Molecules based on period (.)
  {
    ...
    molecules.emplace_back(tmpSpecie.substr(
        0, std::string::npos));  // so it actually includes the last molecule
  ...
  if (tmpObs.observableType == SpeciesTracker::ObservableType::complex) {
    for (auto& oneRxn : forwardRxns) {
      if (oneRxn.rxnType == ReactionType::bimolecular) {
        ...
        bool firstObsfirstProd{
            parsedMols[0].molTypeIndex ==
                oneRxn.productListNew[0].molTypeIndex &&
            parsedMols[0].interfaceList[0].relIndex ==
                oneRxn.productListNew[0].relIfaceIndex &&
            parsedMols[0].interfaceList[0].isBound ==
                oneRxn.productListNew[0].requiresInteraction};
        bool firstObsSecondProd{
            parsedMols[0].molTypeIndex ==
                oneRxn.productListNew[1].molTypeIndex &&
            parsedMols[0].interfaceList[0].relIndex ==
                oneRxn.productListNew[1].relIfaceIndex &&
            parsedMols[0].interfaceList[0].isBound ==
                oneRxn.productListNew[1].requiresInteraction};
        bool secondObsfirstProd{
            parsedMols[1].molTypeIndex ==
                oneRxn.productListNew[0].molTypeIndex &&
            parsedMols[1].interfaceList[0].relIndex ==
                oneRxn.productListNew[0].relIfaceIndex &&
            parsedMols[1].interfaceList[0].isBound ==
                oneRxn.productListNew[0].requiresInteraction};
        bool secondObsSecondProd{
            parsedMols[1].molTypeIndex ==
                oneRxn.productListNew[1].molTypeIndex &&
            parsedMols[1].interfaceList[0].relIndex ==
                oneRxn.productListNew[1].relIfaceIndex &&
        ...
        if (oneRxn.isSymmetric && firstObsfirstProd && firstObsSecondProd &&
            secondObsfirstProd && secondObsSecondProd) {
        }
      }
    }
  }
  ...
  for (auto& parsedMol : parsedMols) {
    ...
    SpeciesTracker::Observable::Constituent& newConst =
        tmpObs.constituentList.back();
    newConst.molTypeIndex = parsedMol.molTypeIndex;
    for (auto& parsedIface : parsedMol.interfaceList)
      newConst.interfaceList.emplace_back(
          parsedIface.relIndex, parsedIface.absIndex, parsedIface.state,
