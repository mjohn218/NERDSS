...
// this is so we can compare parsed keywords with the enumerations (we need them
// as strings)
std::map<const std::string, BoundaryKeyword> bcKeywords = {
    {"implicitlipid", BoundaryKeyword::implicitLipid},
    {"waterbox", BoundaryKeyword::waterBox},
    {"xbctype", BoundaryKeyword::xBCtype},
    {"ybctype", BoundaryKeyword::yBCtype},
    {"zbctype", BoundaryKeyword::zBCtype},
    {"issphere", BoundaryKeyword::isSphere},
    {"spherer", BoundaryKeyword::sphereR}};

void Membrane::set_value_BC(std::string value, BoundaryKeyword keywords) {
  try {
    auto key =
        static_cast<std::underlying_type<BoundaryKeyword>::type>(keywords);
    switch (key) {
      case 0:
      ...
      case 1:
        ...
        std::cout << "Read in waterBox: "
                  << "[" << waterBox.x << " nm, " << waterBox.y << " nm, "
                  << waterBox.z << " nm]" << std::endl;
        break;
      case 2:
        this->xBCtype = value;
        std::cout << "Read in xBCtype: " << value << std::endl;
        break;
      case 3:
        this->yBCtype = value;
        std::cout << "Read in yBCtype: " << value << std::endl;
        break;
      case 4:
        this->zBCtype = value;
        std::cout << "Read in zBCtype: " << value << std::endl;
        break;
      case 5:
        this->isSphere = read_boolean(value);
        std::cout << "Read in isSphere: " << std::boolalpha << this->isSphere
                  << std::endl;
        break;
      case 6:
...
void Membrane::display() {
  ...
  if (isBox == true) {
    ...
    std::cout << waterBox.x << ' ' << waterBox.y << ' ' << waterBox.z
              << std::endl;
  }
  std::cout << " hasImplicitLipid? " << std::boolalpha << implicitLipid
            << std::endl;
}
...
void parse_input(std::string& fileName, Parameters& params,
  ...
  // the start and end blocks are hardcoded since regex has proved itself
  // woefully non-portable, especially with the Intel compiler (icpc)
  while (!inputFile.eof()) {
    std::string line;
    ...
    if (skipLine(tmpLine)) {
          ...
          for (auto lineItr = line.begin(); lineItr != line.end(); ++lineItr) {
            if (std::isalnum(*lineItr))
              ...
              line.erase(line.begin(),
                         lineItr + 1);  // + 1 removes the '=' sign. could make
                                        // this erase(remove_if)
              // find the value type from the keyword and then set that
              // parameter
      ...
      while (getline(inputFile, line)) {
        ...
        if (skipLine(tmpLine))
          ...
          if (gotValue == false) {
            std::cout << "Please provide the copy number of each molecule in "
                         "INP file in this format-- molName:100"
                      << std::endl;
            exit(0);
          }
        }
      ...
      for (int providedMolIndex = 0; providedMolIndex < providedMols.size();
           providedMolIndex++) {
        molTemplateList.emplace_back(parse_molFile(
            providedMols[providedMolIndex]));  // do the actual parsing
        molTemplateList.back().molTypeIndex =
            &providedMols[providedMolIndex] - &providedMols[0];
        molTemplateList.back().copies = providedNums[providedMolIndex];
        molTemplateList.back().startingNumState =
            providedNumState[providedMolIndex];
        ...
        if (molTemplateList.back().isImplicitLipid == false) {
          ...
          params.numTotalUnits +=
              (molTemplateList.back().copies *
               (molTemplateList.back().interfaceList.size() + 1));
      ...
      // Give a buffer for the trajectory file (since VMD can't understand
      // trajectories with frames that have different numbers of atoms)
      // TODO: Think about the size of buffer
      //            params.numTotalUnits += params.numTotalUnits / 100;

      // Make sure the writing of restart file and timestep information is in
      // unison
      ...
      if (hasParsedMol == false) {
        std::cerr << "Error, molecule section must be before reaction section "
                     "in input file, exiting...\n";
        exit(1);
      }
      // index of the last template. should. starts out as number of interface
      // states
      int totSpecies{(molTemplateList.back()
                          .interfaceList.back()
                          .stateList.back()
                          .index)};  // last iface index
      ...
      while (getline(inputFile, line)) {
        tmpLine = create_tmp_line(line);
        // if the line is an ignored line, ignore it. if it's an end of block
        // line, break
        if (skipLine(tmpLine)) {
          ...
          parse_reaction(inputFile, totSpecies, numProvidedRxns,
                         molTemplateList, forwardRxns, backRxns,
      ...
      RxnBase::totRxnSpecies =
          totSpecies + 1;  // had to increment this by 1, it was not correct!

    } else if (tmpLine == "startobservables") {
      // TODO: make this use parse_molecule_bngl() and create a vector of
      // ParsedMol Need to change the function to allow for molecules with no
      // explicit interfaces
      std::cout << "Gathering observables...\n";
      while (getline(inputFile, line)) {
        tmpLine = create_tmp_line(line);
        tmpLine.erase(
            std::remove_if(tmpLine.begin(), tmpLine.end(),
                           [](unsigned char x) { return std::isspace(x); }),
            tmpLine.end());
        if (skipLine(tmpLine)) {
          ...
          remove_comment(line);
          providedObs.emplace_back(
              line);  // save these to parse AFTER reactions
        }
      }
    }
  }
    ...
    if (forwardRxns[rxnItr].isCoupled) {
      if (forwardRxns[rxnItr].coupledRxn.absRxnIndex != -1) {
        ...
        for (const auto& createDestructRxn : createDestructRxns) {
          if (createDestructRxn.absRxnIndex ==
              forwardRxns[rxnItr].coupledRxn.absRxnIndex) {
            forwardRxns[rxnItr].coupledRxn.rxnType = ReactionType::destruction;
            forwardRxns[rxnItr].coupledRxn.relRxnIndex =
                createDestructRxn.relRxnIndex;
            forwardRxns[rxnItr].coupledRxn.label = createDestructRxn.rxnLabel;
            backRxns[forwardRxns[rxnItr].conjBackRxnIndex].coupledRxn =
                forwardRxns[rxnItr].coupledRxn;
            break;
          }
        }
        ...
        for (const auto& uniStateChangeRxn : forwardRxns) {
          if (uniStateChangeRxn.rxnType == ReactionType::uniMolStateChange) {
            if (uniStateChangeRxn.absRxnIndex ==
                forwardRxns[rxnItr].coupledRxn.absRxnIndex) {
              forwardRxns[rxnItr].coupledRxn.rxnType =
                  ReactionType::uniMolStateChange;
              forwardRxns[rxnItr].coupledRxn.relRxnIndex =
                  uniStateChangeRxn.relRxnIndex;
        ...
        // for the case destruction coupled to disscociation
        for (const auto& createDestructRxn : createDestructRxns) {
          if (createDestructRxn.rxnLabel ==
              forwardRxns[rxnItr].coupledRxn.label) {
            forwardRxns[rxnItr].coupledRxn.rxnType = ReactionType::destruction;
            forwardRxns[rxnItr].coupledRxn.relRxnIndex =
                createDestructRxn.relRxnIndex;
            forwardRxns[rxnItr].coupledRxn.absRxnIndex =
                createDestructRxn.absRxnIndex;
            backRxns[forwardRxns[rxnItr].conjBackRxnIndex].coupledRxn =
                forwardRxns[rxnItr].coupledRxn;
            break;
          }
  ...
  populate_reaction_lists(forwardRxns, backRxns, createDestructRxns,
                          molTemplateList);
  for (auto& oneTemp : molTemplateList) {
    ...
    oneTemp.rxnPartners.erase(
        std::unique(oneTemp.rxnPartners.begin(), oneTemp.rxnPartners.end()),
        oneTemp.rxnPartners.end());
    ...
    for (auto& oneIface : oneTemp.interfaceList) {
      for (auto& oneState : oneIface.stateList) {
        ...
        oneState.rxnPartners.erase(std::unique(oneState.rxnPartners.begin(),
                                               oneState.rxnPartners.end()),
                                   oneState.rxnPartners.end());
      }
    }
  }
  ...
  params.numTotalSpecies =
      RxnBase::totRxnSpecies;  // total number of all species possible in the
                               // system (includes reactant and product states).
...
void parse_input_for_add(std::string& fileName, Parameters& params,
  ...
  bool hasParsedMol = false;
  int addForwardRxnNum{static_cast<int>(
      forwardRxns
          .size())};  // this is the origin number of forwardRxns for add case
  int addBackRxnNum{static_cast<int>(
      backRxns.size())};  // this is the origin number of backRxns for add case
  int addCreateDestructRxnNum{static_cast<int>(
      createDestructRxns.size())};  // this is the origin number of
                                    // createDestructRxns for add case
  ...
  while (!inputFile.eof()) {
    ...
    if (skipLine(tmpLine)) {
      ...
      for (int providedMolIndex = 0; providedMolIndex < providedMols.size();
        ...
        molTemplateList.emplace_back(parse_molFile(
            providedMols[providedMolIndex]));  // do the actual parsing
        molTemplateList.back().molTypeIndex = &providedMols[providedMolIndex] -
                                              &providedMols[0] +
                                              addMolTemplateListNum;
        molTemplateList.back().copies = providedNums[providedMolIndex];
        molTemplateList.back().startingNumState =
            providedNumState[providedMolIndex];
        ++MolTemplate::numMolTypes;
