...
void read_bonds(int numBonds, std::ifstream& molFile,
  ...
  for (unsigned bondItr{0}; bondItr < numBonds; ++bondItr) {
    ...
    if (atom1 == std::string{"com"}) tmpBond[0] = 0;
    if (atom2 == std::string{"com"}) tmpBond[1] = 0;
    ...
    for (unsigned ifaceItr{0}; ifaceItr < molTemplate.interfaceList.size();
      ...
      if (tmpBond[0] != -1 && tmpBond[1] != -1) break;
    }

    if (tmpBond[0] == -1 || tmpBond[1] == -1) {
      std::cout << "Invalid atom in " << molTemplate.molName
                << " mol file. Ignoring bonds...\n";
