...
MolTemplate parse_molFile(std::string& mol) {
  /* NOTE: need to edit both this and enum class MolKeyword if you want to add
   * keywords  */
  ...
  while (getline(molFile, line)) {
    ...
    for (auto lineItr = line.begin(); lineItr != line.end(); ++lineItr) {
      // if the line starts with com, it's the beginning of the coordinates
      // block
      if (std::isdigit(*lineItr) &&
          molKeywords.find(buffer)->second == MolKeyword::com) {
  ...
  for (auto& iface : tmpTemplate.interfaceList) {
    ...
    /*initialize values of the molecule templates interfaces, including if it
     can be in multiple states stored in
     molTemplate.interfaceList[].stateList[].index*/
    if (iface.stateList.empty()) {
      MolTemplate::absToRelIface.push_back(ifaceItr);
      iface.stateList.emplace_back(iface.name,
                                   Interface::State::totalNumOfStates);
      ++Interface::State::totalNumOfStates;
    } else {
      tmpTemplate.ifacesWithStates.push_back(&iface -
                                             &tmpTemplate.interfaceList[0]);
  ...
  std::cout << "radius calculated from the ifacesCoord: " << tmpTemplate.radius
            << " nm" << std::endl;
  // set mass to radius
  if (tmpTemplate.mass < 0) {
    tmpTemplate.mass = tmpTemplate.radius;
    std::cout << "mass auto calculated from the radius: " << tmpTemplate.mass
              << std::endl;
  }
