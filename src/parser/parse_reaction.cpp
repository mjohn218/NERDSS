...
void parse_reaction(std::ifstream& reactionFile, int& totSpecies,
  ...
  /* NOTE: need to edit both this and enum class RxnKeyword if you want to add
   * keywords  */
  std::map<const std::string, RxnKeyword> rxnKeywords = {
      {"onrate3dka", RxnKeyword::onRate3Dka},
      {"onrate3dmacro", RxnKeyword::onRate3DMacro},
      {"offratekb", RxnKeyword::offRatekb},
      {"offratemacro", RxnKeyword::offRateMacro},
      {"norm1", RxnKeyword::norm1},
      {"norm2", RxnKeyword::norm2},
      {"sigma", RxnKeyword::sigma},
      {"assocangles", RxnKeyword::assocAngles},
      {"onmem", RxnKeyword::onMem},
      {"rate", RxnKeyword::rate},
      {"iscoupled", RxnKeyword::isCoupled},
      {"isobserved", RxnKeyword::isObserved},
      {"observelabel", RxnKeyword::observeLabel},
      {"bindradsamecom", RxnKeyword::bindRadSameCom},
      {"irrevringclosure", RxnKeyword::irrevRingClosure},
      {"creationradius", RxnKeyword::creationRadius},
      {"loopcoopfactor", RxnKeyword::loopCoopFactor},
      {"length3dto2d", RxnKeyword::length3Dto2D},
      {"rxnlabel", RxnKeyword::rxnLabel},
      {"coupledrxnlabel", RxnKeyword::coupledRxnLabel},
      {"kcat", RxnKeyword::kcat},
      {"excludevolumebound", RxnKeyword::excludeVolumeBound}};
  ...
  {  // break into reactant and product side using <-> or ->
    ...
    if (reaction.find("<->") != std::string::npos) {
      ...
      productSide = reaction.substr(
          position + 3, std::string::npos);  // +1 is to remove the delimiter
      ...
      reactantSide = reaction.substr(0, position);
      productSide = reaction.substr(
          position + 2, std::string::npos);  // +2 is to remove delimiter
    } else
  ...
  {  // break into species based on '+'
    ...
    while ((position = reactantSide.find('+')) != std::string::npos) {
      reactantSpecies.emplace_back(reactantSide.substr(0, position),
                                   speciesIndex);
      reactantSide.erase(0, position + 1);
      ++speciesIndex;
    }
    // std::string::substr(position, position + length of substr). I think this
    // might accidentally work
    reactantSpecies.emplace_back(reactantSide.substr(0, std::string::npos),
                                 speciesIndex);

    speciesIndex = 0;
    while ((position = productSide.find('+')) != std::string::npos) {
      productSpecies.emplace_back(productSide.substr(0, position),
                                  speciesIndex);
      productSide.erase(0, position + 1);
      ++speciesIndex;
    }
    productSpecies.emplace_back(productSide.substr(0, std::string::npos),
                                speciesIndex);
  }
  ...
  if (reactantSpecies.size() > 2) {
    ...
    std::cerr << "Reaction " << reaction
              << " has more than two reacting molecules.\n";
    exit(1);
  }
  ...
  if ((reactantSpecies.size() == 1) && (productSpecies.size() == 1)) {
    ...
    std::transform(tmpLSide.begin(), tmpLSide.end(), tmpLSide.begin(),
                   ::tolower);
    std::transform(tmpRSide.begin(), tmpRSide.end(), tmpRSide.begin(),
                   ::tolower);
    ...
    if (!lside && !rside) {
      ...
      std::cerr << "FATAL ERROR: Ccannot determine reaction type. Please check "
                   "before moving on.\n";
      exit(1);
    }
  ...
  // the following two large blocks of text parsing can't be split off into one
  // subroutine, because of the use of the local structs ParsedMol and ParsedRxn
  if (parsedRxn.rxnType != ReactionType::zerothOrderCreation) {
    // TODO: need to pass some integer to tell ParsedMol which species the
    // molecule is in (via speciesIndex)
    ...
    for (auto& specie : reactantSpecies) {
      ...
      reactantMols.emplace_back(
          tmpSpecie.substr(0, std::string::npos),
          specie.second);  // so it actually includes the last molecule
      // if there was only one molecule, make sure it makes its way into the
      // vector
    ...
    for (auto& oneMol : reactantMols) {
      parsedRxn.reactantList.push_back(
          parse_molecule_bngl(totSpecies, false, oneMol));
  ...
  if (parsedRxn.rxnType != ReactionType::destruction) {
    ...
    for (auto& specie : productSpecies) {
      ...
      if (tmpSpecie.find('.') != std::string::npos) {
        ...
        productMols.emplace_back(
            tmpSpecie.substr(0, std::string::npos),
            speciesIndex);  // so it actually includes the last molecule
      } else {
        // this is to account for bimolecular state changes that have no bound
        // species
        productMols.emplace_back(
            tmpSpecie.substr(0, std::string::npos),
            speciesIndex);  // so it actually includes the last molecule
    ...
    for (auto& oneMol : productMols) {
      parsedRxn.productList.push_back(
          parse_molecule_bngl(totSpecies, true, oneMol));
  ...
  std::string line;
  std::string tmpLine{
      line};  // to avoid altering the original line, just in case
  while (line != "endreactions") {
    ...
    if (line[0] == '#') {
      ...
      // if the next line contains a reaction, seek back to where you started in
      // the file and stop looking for parameter keywords
      reactionFile.seekg(initialPos);
      std::cout
          << "Finished reading reaction parameters, constructing reaction.\n";
      break;
    }
    ...
    for (auto lineItr = line.begin(); lineItr != line.end(); ++lineItr) {
      if (std::isalnum(*lineItr))
        ...
        if (keyFind == rxnKeywords.end()) {
          std::cerr << buffer +
                           " is an invalid argument for the reactions block. "
                           "Exiting...";
          exit(1);
        }

        line.erase(line.begin(),
                   lineItr + 1);  // hard coded for character length '='
  ...
  for (auto& reactant : parsedRxn.reactantList) {
    determine_iface_indices(&reactant - &parsedRxn.reactantList[0], totSpecies,
                            reactant, parsedRxn, forwardRxns, molTemplateList);
    check_for_valid_states(&reactant - &parsedRxn.reactantList[0], reactant,
                           parsedRxn, molTemplateList);
  }

  for (auto& product : parsedRxn.productList)
    determine_iface_indices(&product - &parsedRxn.productList[0], totSpecies,
                            product, parsedRxn, forwardRxns, molTemplateList);

  // determine the indices for the reactants and products of the real reaction
  // here
  if (parsedRxn.rxnType == ReactionType::bimolecular) {
    parsedRxn.determine_reactants();

    if (parsedRxn.rxnProducts[0].speciesIndex != 0 &&
        parsedRxn.rxnProducts[1].speciesIndex != 1) {
  ...
  if (parsedRxn.isObserved) {
    if (parsedRxn.rxnType == ReactionType::zerothOrderCreation) {
      int startNum{
          molTemplateList[parsedRxn.rxnProducts[0].molTypeIndex].copies};
      observablesList.emplace(std::make_pair(parsedRxn.observeLabel, startNum));
    } else if (parsedRxn.rxnType == ReactionType::uniMolCreation) {
      int startNum{
          molTemplateList[parsedRxn.rxnProducts[1].molTypeIndex].copies};
      observablesList.emplace(std::make_pair(parsedRxn.observeLabel, startNum));
    } else if (parsedRxn.rxnType == ReactionType::destruction) {
      int startNum{
          molTemplateList[parsedRxn.rxnReactants[0].molTypeIndex].copies};
      observablesList.emplace(std::make_pair(parsedRxn.observeLabel, startNum));
    } else if (parsedRxn.rxnType == ReactionType::uniMolStateChange) {
      int molTypeIndex{parsedRxn.rxnReactants[0].molTypeIndex};
      int relIfaceIndex{parsedRxn.rxnReactants[0].relIndex};
      if (parsedRxn.rxnProducts[0].state == molTemplateList[molTypeIndex]
                                                .interfaceList[relIfaceIndex]
                                                .stateList[0]
                                                .iden) {
        int startNum{molTemplateList[molTypeIndex].copies};
        observablesList.emplace(
            std::make_pair(parsedRxn.observeLabel, startNum));
      } else
  ...
  if (parsedRxn.rxnType == ReactionType::bimolecular) {
    ...
    std::string react1{molTemplateList[react1ProType].molName + '(' +
                       parsedRxn.rxnReactants[0].ifaceAndStateName + "!1)"};
    int react2ProType{parsedRxn.rxnReactants[1].molTypeIndex};
    std::string react2{molTemplateList[react2ProType].molName + '(' +
                       parsedRxn.rxnReactants[1].ifaceAndStateName + "!1)"};
  ...
  if (parsedRxn.rxnType == ReactionType::bimolecular ||
      parsedRxn.rxnType == ReactionType::biMolStateChange) {
    /*for all bimolecular reactions, they might take place in 2D, need to assign
      3Dto2D length. By default, if it is not read in, it will be set to
      2*sigma*/
  ...
  if (parsedRxn.rxnType == ReactionType::bimolecular ||
    ...
    // for 3D reaction, Dz != 0 for each reactant; for 3D->2D, Dz == 0 for one
    // reactant. determine this first
    bool to2D = false;
    bool is2D = true;
    ...
    if (isSelf == true) {  // Self reaction
      if (std::isnan(parsedRxn.onRate3Dka) == true) {
        if (std::isnan(parsedRxn.onRate3DMacro) == false) {
          // convert macro rate to micro rate, uM-1s-1 = 1/0.602214076 nm3/us,
          // get Dtot first
          double Dtot{0.0};
          for (auto& reactant : parsedRxn.reactantList) {
            Dtot += (1 / 3.0 * molTemplateList[reactant.molTypeIndex].D.x +
                     1 / 3.0 * molTemplateList[reactant.molTypeIndex].D.y +
                     1 / 3.0 * molTemplateList[reactant.molTypeIndex].D.z);
          }
          if (std::abs(parsedRxn.onRate3DMacro - 0.0) > 1E-15) {
            parsedRxn.onRate3Dka =
                0.5 / (1.0 / (2.0 * parsedRxn.onRate3DMacro / 0.602214076) -
                       1.0 / (4.0 * M_PI * parsedRxn.bindRadius * Dtot));
          } else {
          ...
          if (std::isnan(parsedRxn.offRateMacro) == false) {
            if (std::abs(parsedRxn.onRate3DMacro - 0.0) > 1E-15) {
              parsedRxn.offRatekb = parsedRxn.offRateMacro *
                                    parsedRxn.onRate3Dka * 0.602214706 /
                                    parsedRxn.onRate3DMacro;
            } else {
              parsedRxn.offRatekb = parsedRxn.offRateMacro;
            }
          }
      ...
      if (is2D == true) {  // 2D reaction
        if (std::isnan(parsedRxn.onRate3Dka) == true) {
            if (std::isnan(parsedRxn.onRate3DMacro) == false) {
              // convert macro rate to micro rate, uM-1s-1 = 1/0.602214076
              // nm3/us, get Dtot first
              double Dtot{0.0};
              for (auto& reactant : parsedRxn.reactantList) {
                Dtot += (1 / 2.0 * molTemplateList[reactant.molTypeIndex].D.x +
                         1 / 2.0 * molTemplateList[reactant.molTypeIndex].D.y);
              }
              if (std::abs(parsedRxn.onRate3DMacro - 0.0) > 1E-15) {
                parsedRxn.onRate3Dka =
                    1.0 / (1.0 / (parsedRxn.onRate3DMacro / 0.602214076) -
                           1.0 / (4.0 * M_PI * parsedRxn.bindRadius * Dtot));
              } else {
                parsedRxn.onRate3Dka = 0;
              }
  ...
  if (parsedRxn.isCoupled == true) {
    // create a probability for this reaction to be Michaelis Menten, if kcat
    // exists and kb exists. Otherwise prob=1.
    if (std::isnan(parsedRxn.kcat) == false) {
      double kcat = parsedRxn.kcat;  // kcat is not used after this function in
                                     // the code, only probCoupled is.
      if (std::isnan(parsedRxn.offRatekb) == true)
        parsedRxn.offRatekb =
            0;  // set this to zero, so dissociation occurs with rate kcat, and
                // coupled Rxn always occurs.
      parsedRxn.coupledRxn.probCoupled = kcat / (kcat + parsedRxn.offRatekb);
      parsedRxn.offRatekb +=
          kcat;  // the rate that the bound complex undergoes reaction is due to
                 // both catalysis and unbinding. Will then choose which one to
                 // perform based on probCoupled.
      std::cout << "Allow Michaelis-Menten for this reaction. Probability of "
                   "catalysis: kcat/(kcat+offRatekb): "
                << parsedRxn.coupledRxn.probCoupled
                << " total rate: " << parsedRxn.offRatekb << std::endl;
    } else {
      std::cout << " No kcat rate specified, will NOT perform coupled "
                   "reaction! Must specify rate using kcat "
                << std::endl;
    }
  }
  // Done parsing, set up the reaction(s) and place into their correct vectors
  if (parsedRxn.willBeMultipleRxns) {
    // if there is an interface that is missing a state (on purpose), we need to
    // create a different forward reaction for each of those missing states
    auto newRxns = parsedRxn.split_into_multiple_reactions(
        totSpecies, forwardRxns, molTemplateList);
    ...
    for (auto& oneRxn : newRxns) {
      ...
      if (!oneRxn.check_for_conditional_rates(totSpecies, forwardRxns,
                                              backRxns))
        oneRxn.assemble_reactions(forwardRxns, backRxns, createDestructRxns,
                                  molTemplateList);
    }
  } else {
    ...
    // check if this is a repeat reaction and create new RateState if so
    if (!parsedRxn.check_for_conditional_rates(totSpecies, forwardRxns,
                                               backRxns)) {
      ...
      if (!status.first) {
        ...
        std::cout << status.second << " [" << reaction << "].\n";
      parsedRxn.assemble_reactions(forwardRxns, backRxns, createDestructRxns,
                                   molTemplateList);
    }
  }
  ...
  if (parsedRxn.rxnType == ReactionType::bimolecular ||
      parsedRxn.rxnType == ReactionType::biMolStateChange) {
    MolTemplate& reactTemp1 =
        molTemplateList[forwardRxns.back().reactantListNew.at(0).molTypeIndex];
    MolTemplate& reactTemp2 =
        molTemplateList[forwardRxns.back().reactantListNew.at(1).molTypeIndex];
