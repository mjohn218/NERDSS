...
void check_for_valid_states(size_t parsedMolIndex, ParsedMol& targMol,
  ...
  auto tempNameItr =
      std::find_if(molTemplateList.begin(), molTemplateList.end(),
                   [&](const MolTemplate& oneTemp) -> bool {
                     return oneTemp.molName == targMol.molName;
                   });

  if (tempNameItr == molTemplateList.end()) {
    std::cerr << "Error, target molecule's type cannot be found in list of "
                 "templates.";
    exit(1);
  }
  ...
  for (auto ifaceItr = targMol.interfaceList.begin();
       ifaceItr != targMol.interfaceList.end(); ++ifaceItr) {
    ...
    auto tempIfaceItr = std::find_if(
        ...
        [&](const Interface& oneTempIface) -> bool {
          return oneTempIface.name == ifaceItr->ifaceName;
        });

    if (tempIfaceItr == tempNameItr->interfaceList.end()) {
      std::cout << ifaceItr->ifaceName
                << " is not a valid interface for molecule template "
                << tempNameItr->molName << '\n';
      exit(120);
    }
    ...
    if (tempIfaceItr->stateList.size() != 1 && ifaceItr->state == '\0') {
      // if no state is explicitly provided, but the iface has specific states,
      // we need to create separate reactions for each state
      std::cout
          << "No interface state provided for reactant "
      ...
      tempStateItr = std::find_if(
          tempIfaceItr->stateList.begin(), tempIfaceItr->stateList.end(),
          [&](const Interface::State& oneTempState) -> bool {
            return oneTempState.iden == ifaceItr->state;
          });
      ...
      if (parsedRxn.rxnType != ReactionType::destruction) {
        ...
        std::cout << "Found state for reactant "
                  << write_mol_iface(targMol.molName, ifaceItr->ifaceName)
                  << '\n';
    }
  }
