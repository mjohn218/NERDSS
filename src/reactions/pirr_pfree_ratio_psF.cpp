...
double pirr_pfree_ratio_psF(double rCurr, double r0, double tCurr, double Dtot,
  ...
  term1 = -0.5 * fDt * exp(-ndist * ndist / fDt) -
          0.5 * sq_fDt * sq_P * r0 * erf(-ndist / sq_fDt);
  term2 = 0.5 * fDt * exp(-nadist * nadist / fDt) +
          0.5 * sq_fDt * sq_P * r0 * erf(nadist / sq_fDt);
  double pnorm =
      1.0 -
      c1 * (term1 + term2);  // this is then the normlization from sigma->inf
  ...
  if (std::abs(pirr - pfree * ps_prev) < rtol)
    ...
    //    cout <<"pirr: "<<pirr<<" pfree*ps: "<<pfree*ps_prev<<" ratio:
    //    "<<ratio<<endl;
  }
