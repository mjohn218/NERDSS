...
void associate_sphere(long long int iter, int ifaceIndex1, int ifaceIndex2,
  ...
  if (reactCom1.index == reactCom2.index) {
    ...
    if (reactMol2.isImplicitLipid == true ||
        reactMol1.isImplicitLipid == true) {
      std::cout << "WRONG: implicit-lipid binding involves wrong function "
                   "file. associate_ImplicitLipid_sphere should be called "
                   "instead of associate_sphere!"
                << std::endl;
      exit(1);
    }
    ...
    /* MOVE PROTEIN TO SIGMA */
    {
      ...
      if (DzSum < 1E-14) {
        isOnMembrane = true;
        currRxn.bindRadius2D =
            calc_bindRadius2D(currRxn.bindRadius, reactIface1);
        ...
        sigmaMag = sqrt((sigma.x * sigma.x) + (sigma.y * sigma.y) +
                        (sigma.z * sigma.z));
      ...
      // now, calculate the transVec for each complex and move these two
      // complexes
      Coord target1Pos;
      Coord target2Pos;
      ...
      if (isOnMembrane == true) {
        reactCom1.update_association_coords_sphere(moleculeList, reactIface1,
                                                   target1Pos);
        reactCom2.update_association_coords_sphere(moleculeList, reactIface2,
                                                   target2Pos);
      } else {
    ...
    if (isOnMembrane == true || transitionToSurface == true) {
      ...
      if (reactCom1.D.z < reactCom2.D.z) {
        set_memProtein_sphere(
            reactCom1, memProtein, moleculeList,
            membraneObject);  // rotate relative to the slower protein.
        find_Lipid_sphere(reactCom1, Lipid, moleculeList, membraneObject);
      } else {
        set_memProtein_sphere(
            reactCom2, memProtein, moleculeList,
            membraneObject);  // rotate relative to the slower protein.
        find_Lipid_sphere(reactCom2, Lipid, moleculeList, membraneObject);
      }
      Quat memRot = save_mem_orientation(memProtein, Lipid,
                                         molTemplateList[Lipid.molTypeIndex]);
    ...
    com_of_two_tmp_complexes(
        reactCom1, reactCom2, finalCOM,
        moleculeList);  // com of c1+c2 (final (tmp) coordinates).
    // Force finalCOM to startCOM if in solution, while on sphere or
    // transitionToSurface make sure lipids are still on sphere
    if (isOnMembrane == true || transitionToSurface == true) {
      ...
      complexes for (auto& mp : reactCom1.memberList)
          moleculeList[mp].update_association_coords(dtrans);
      for (auto& mp : reactCom2.memberList)
          moleculeList[mp].update_association_coords(dtrans);
    */
    ...
    // std::cout << " FINAL COORDS PRIOR TO OVERLAP CHECK  AND REFLECT OFF
    // SPHERE: "
    //           << std::endl;
    // reactMol1.display_assoc_icoords("mol1");
    // reactMol2.display_assoc_icoords("mol2");
...
*/
    ...
    if (cancelAssoc == false) {
      check_if_spans_sphere(cancelAssoc, params, reactCom1, reactCom2,
