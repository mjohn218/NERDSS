...
bool determine_parent_complex(int pro1Index, int pro2Index, int newComIndex,
  ...
  for (auto& mp : origlist) {
    ...
    if (std::find(tmppartvec.begin(), tmppartvec.end(), pro1Index) !=
            tmppartvec.end() &&
        std::find(tmppartvec.begin(), tmppartvec.end(), pro2Index) !=
            tmppartvec.end()) {
      // std::cout << "Molecule " << mp << " is bound to both dissociating
      // molecules. Keeping as one complex.\n";
      boundTwice = true;
      break;
    }
  ...
  if (boundTwice) {
    ...
    // Now check each protein bound to a dissociating protein to determine which
    // complex they'll belong to after dissociation
    std::vector<int> recheck;
    for (auto memMol :
         complexList[moleculeList[pro1Index].myComIndex].memberList) {
      if (memMol != pro1Index && memMol != pro2Index) {
        ...
        if (com_flag != 1) recheck.push_back(memMol);
      }
    }
    ...
    // what this SHOULD do is take each protein in the bndlist of the protein to
    // recheck (a) and check each of those proteins' (b) bndlists (c) for target
    // proteins for the dissociating proteins if they're not found, it goes in
    // another layer and looks at the bndlists of (c), and so on
    for (auto i = recheck.begin(); i != recheck.end();) {
      ...
      std::vector<int>
          checkedList;  // these are the moleculeList that have been checked
      // for(int pp=0;pp<testList.size();pp++)
      // 	      std::cout <<testList[pp]<<'\t';
      // 	    std::cout<<std::endl;

      while (!doneChecking) {
        testList.erase(std::remove(testList.begin(), testList.end(), -1),
                       testList.end());
          ...
          for (auto& ppart2 : moleculeList[ppart].bndpartner) {
            ...
            if (ppart != molIndex) {
              if (ppart2 == pro1Index) {
                ...
                // std::cout << "base " << molIndex << " belongs to complex c1."
                // << std::endl;
                com_flag++;
              } else if (ppart2 == pro2Index) {
                tmpc2.push_back(molIndex);
                // std::cout << "base " << molIndex << " belongs to complex c2."
                // << std::endl;
                com_flag++;
              }
            }
            if (com_flag > 1) {
              // if both pro1Index and pro2Index are found, com_flag > 1 and its
              // a closed loop
              moleculeList[pro1Index].myComIndex = c1;
              moleculeList[pro2Index].myComIndex = c1;
              return true;
            } else if (com_flag == 0) {
              // neither pro1Index or pro2Index found, move to the next set of
              // test subjects
              // std::cout <<" did not find either for prote
              // "<<molIndex<<std::endl;
              no++;
            } else {
              no = 0;
        ...
        if (no > 0) {
          // the new test subject list is all the binding partners of the
          // previous set of test subjects
          testList.clear();
          for (auto& mp : testlist2) {
            if (mp != *i && find(checkedList.begin(), checkedList.end(), mp) ==
                                checkedList.end()) {
              testList.push_back(mp);
            }
          }
          std::sort(testList.begin(), testList.end());
          testList.erase(std::unique(testList.begin(), testList.end()),
                         testList.end());
    ...
    if (tmpc1.size() + tmpc2.size() != complexList[c1].memberList.size()) {
      std::cout << " complex sizes don't match the parent! " << tmpc1.size()
                << ' ' << tmpc2.size()
                << " original size: " << complexList[c1].memberList.size()
                << std::endl;
      std::cout << " pros in c1: " << std::endl;
      for (auto memMol : tmpc1) std::cout << memMol << '\t';
      std::cout << " pros in c2: " << std::endl;
      for (auto memMol : tmpc2) std::cout << memMol << '\t';
      std::cout << "display molecules " << std::endl;
      for (auto mp : tmpc1) moleculeList[mp].display_all();
      std::cout << " pros in c2: " << std::endl;
      for (auto mp : tmpc2) moleculeList[mp].display_all();

      std::cerr << "ERROR: Combined size of dissociated complexes does not "
                   "match the parent complex. Exiting...\n";
      exit(1);
    }
    ...
    std::sort(complexList[c1].memberList.begin(),
              complexList[c1].memberList.end());
    std::sort(complexList[newComIndex].memberList.begin(),
              complexList[newComIndex].memberList.end());
