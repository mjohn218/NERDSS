...
double calculate_omega(Coord reactIface1, int reactIface2, Vector& sigma,
  ...
  if (areSameAngle(currRxn.assocAngles.theta1, M_PI) ||
      areSameAngle(currRxn.assocAngles.theta2, M_PI)) {
    v1 = determine_normal(currRxn.norm1,
                          molTemplateList[reactMol1.molTypeIndex], reactMol1);
    v2 = determine_normal(currRxn.norm2,
                          molTemplateList[reactMol2.molTypeIndex], reactMol2);
  } else {
  ...
  if (test.z > 0 and std::abs(omega) > tol and
      (M_PI - std::abs(omega)) > tol)  // positive z, flip theta sign
    omega = -omega;
  return omega;

  //(projVec1.cross(projVec2).z < 0) ? projVec1.dot_theta(projVec2) :
  //-projVec1.dot_theta(projVec2);
  ...
  //    projVec2.calc_magnitude();
  //
  //    return (angleSignIsCorrect(projVec1, projVec2)) ?
  //    projVec1.dot_theta(projVec2) : -projVec1.dot_theta(projVec2);
}
