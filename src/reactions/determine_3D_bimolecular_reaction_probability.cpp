...
void determine_3D_bimolecular_reaction_probability(
  ...
  if (moleculeList[biMolData.pro1Index].isDissociated == false &&
    ...
    if (withinRmax && forwardRxns[rxnIndex].rateList[rateIndex].rate > 0) {
      ...
      */
      double kdiff{4 * M_PI * biMolData.Dtot *
                   forwardRxns[rxnIndex].bindRadius};
      double kact{forwardRxns[rxnIndex].rateList[rateIndex].rate};

      if (std::abs(complexList[biMolData.com1Index].D.z - 0) < 1E-10 ||
          std::abs(complexList[biMolData.com2Index].D.z - 0) < 1E-10)
        kact *= 2.0;
      ...
      double alpha{fact * sqrt(biMolData.Dtot) /
                   forwardRxns[rxnIndex].bindRadius};
      ...
      */
      ...
      if (biMolData.com1Index != biMolData.com2Index) {
        ...
        // <<
        // '\n';
        rxnProb = passocF(R1, params.timeStep, biMolData.Dtot,
        ...
        */
        if (sep < forwardRxns[rxnIndex].bindRadius) rxnProb = 1.0;
      }
      if (rxnProb > 1.000001) {
        std::cerr << "WARNING: prob of reaction is: " << rxnProb
      ...
      moleculeList[biMolData.pro2Index].probvec.back() =
          moleculeList[biMolData.pro1Index].probvec.back();
