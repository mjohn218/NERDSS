...
Vector determine_normal(Vector normal, const MolTemplate& molTemplate,
  ...
  for (auto& coord : oneMol.tmpICoords) coord -= oneMol.tmpComCoord;
  ...
  /*  int numUnmatched { 0 };
  for (unsigned ifaceItr { 0 }; ifaceItr < oneMol.interfaceList.size();
  ++ifaceItr) { Vector diffVec {oneMol.tmpICoords[ifaceItr] -
  molTemplate.interfaceList[ifaceItr].iCoord}; diffVec.calc_magnitude(); if
  (std::abs(diffVec.magnitude) > 1E-8)
          ++numUnmatched;
          }*/
  //    if (numUnmatched != 0) {
  ...
  {  // check to make sure the rotations were successful
    for (unsigned ifaceIndex{0}; ifaceIndex < oneMol.tmpICoords.size();
         ifaceIndex++) {
      ...
      if (oneMol.tmpICoords[ifaceIndex] != tmpMol.tmpICoords[ifaceIndex]) {
        // std::cout << "Backwards rotation unsuccessful on interface " <<
        // ifaceIndex << std::endl;
        return {0, 0, 0};
      }
    }
