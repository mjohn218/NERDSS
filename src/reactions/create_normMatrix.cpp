...
void create_normMatrix(gsl_matrix*& normMatrix, double bindRadius, double Dtot,
  ...
  //    for (double Rindex = intParams.a; Rindex <= Rmax + RStepSize; Rindex +=
  //    RStepSize) {
  while (RIndex <= comRMax + RStepSize) {
    ...
    double result{};
    double error{};
    gsl_integration_qagiu(&F, xLowB, epsAbs, epsRel, 10000000, w, &result,
                          &error);
  ...
  //    std::cout << "ELEMENT 318: " << gsl_matrix_get(normMatrix, 0, 318) << '
  //    ' << gsl_matrix_get(normMatrix, 1, 318)
  //              << '\n';
  //    std::cout << "ELEMENT 319: " << gsl_matrix_get(normMatrix, 0, 319) << '
  //    ' << gsl_matrix_get(normMatrix, 1, 319)
  //              << '\n';
}
