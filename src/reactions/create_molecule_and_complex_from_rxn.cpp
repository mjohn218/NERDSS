...
bool moleculeOverlaps(const Parameters& params, SimulVolume& simulVolume,
                      Molecule& createdMol, std::vector<Molecule>& moleculeList,
  ...
  int xItr{int((createdMol.comCoord.x + membraneObject.waterBox.x / 2) /
               simulVolume.subCellSize.x)};
  int yItr{int((createdMol.comCoord.y + membraneObject.waterBox.y / 2) /
               simulVolume.subCellSize.y)};
  int zItr{
      int(-(createdMol.comCoord.z + 1E-6 - membraneObject.waterBox.z / 2.0) /
  ...
  if (molTemplateList[createdMol.molTypeIndex].D.z == 0 &&
      std::abs(createdMol.comCoord.z) -
              std::abs((membraneObject.waterBox.z / 2)) >
          1E-6) {
    // std::cerr << "Molecule " << createdMol.index << " of type " <<
    // molTemplateList[createdMol.molTypeIndex].molName
    //           << " is off the membrane. Writing coordinates and exiting.\n";
    return true;
  }

  // Now make sure the Molecule is still inside the box in all dimensions
  if (createdMol.comCoord.z > (membraneObject.waterBox.z / 2) ||
      createdMol.comCoord.z + 1E-6 < -(membraneObject.waterBox.z / 2)) {
    // std::cout << "Molecule " << createdMol.index
    //           << " is outside simulation volume in the z-dimension, with
    //           center of mass coordinates ["
    //           << createdMol.comCoord << "]. Attempting to fit back into
    //           box.\n";
    return true;
    ...
    // std::cout << "Molecule " << createdMol.index
    //           << " is outside simulation volume with center of mass
    //           coordinates [" << createdMol.comCoord
    ...
    for (auto memMol : simulVolume.subCellList[currBin].memberMolList) {
      ...
      // memMol maybe not belong to this subCellList anymore, check its status
      if (moleculeList[memMol].myComIndex == -1) continue;
      if (moleculeList[memMol].myComIndex >= complexList.size()) continue;

      const Complex& oneCom =
          complexList[moleculeList[memMol].myComIndex];  // legibility

      // check bounding sphere
      Vector tmpVec{createdMol.comCoord - oneCom.comCoord};
      tmpVec.calc_magnitude();
      if (tmpVec.magnitude >
          (molTemplateList[createdMol.molTypeIndex].radius + oneCom.radius))
        return false;
      ...
      for (auto& comMemMol : oneCom.memberList) {
        ...
        for (const auto& oneRxn : forwardRxns) {
          for (unsigned iface1Itr{0};
               iface1Itr < createdMol.interfaceList.size(); ++iface1Itr) {
            for (unsigned iface2Itr{0};
                 iface2Itr < partMol.interfaceList.size(); ++iface2Itr) {
              if (isReactant(createdMol.interfaceList[iface1Itr], createdMol,
                             oneRxn.reactantListNew[0]) &&
                  isReactant(partMol.interfaceList[iface2Itr], partMol,
                             oneRxn.reactantListNew[1])) {
                // if they're reactants, check if they're within the binding
                // radius
                Vector ifaceVec{createdMol.interfaceList[iface1Itr].coord -
                                partMol.interfaceList[iface2Itr].coord};
                ifaceVec.calc_magnitude();

                if (ifaceVec.magnitude > oneRxn.bindRadius) return true;
              } else if (isReactant(createdMol.interfaceList[iface1Itr],
                                    createdMol, oneRxn.reactantListNew[1]) &&
                         isReactant(partMol.interfaceList[iface2Itr], partMol,
                                    oneRxn.reactantListNew[0])) {
                // if they're reactants, check if they're within the binding
                // radius
...
void create_molecule_and_complex_from_rxn(
    int parentMolIndex, int& newMolIndex, int& newComIndex,
    bool createInVicinity, MolTemplate& createdMolTemp, Parameters& params,
  ...
  //         // Check to make sure the object pointed to by the last element in
  //         the each list is actually empty
  //         // If it isn't delete it from the list and move on until you find
  //         one that is
  // //TODO: OPTIMIZE AT THE END: no need to try-catch, but rather loop while
  // there are elements, and after check wheter empty
  //         try {
  ...
  //             // if there's an available empty Complex spot, make the new
  //             Complex in place newComIndex = Complex::emptyComList.back();
  //             Complex::emptyComList.pop_back(); // remove the empty Complex
  //             spot index from the list
  ...
  if (createInVicinity) {
    ...
    while (needsResampling) {
      moleculeList[newMolIndex] = initialize_molecule_after_uni_reaction(
          newMolIndex, moleculeList[parentMolIndex], params, createdMolTemp,
          currRxn);
      needsResampling = moleculeOverlaps(
          params, simulVolume, moleculeList[newMolIndex], moleculeList,
          complexList, forwardRxns, molTemplateList, membraneObject);
    }
  } else {
    bool needsResampling{true};
    while (needsResampling) {
      moleculeList[newMolIndex] = initialize_molecule_after_zeroth_reaction(
          newMolIndex, params, createdMolTemp, currRxn, membraneObject);
      needsResampling = moleculeOverlaps(
          params, simulVolume, moleculeList[newMolIndex], moleculeList,
          complexList, forwardRxns, molTemplateList, membraneObject);
    }
