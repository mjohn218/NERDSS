...
void determine_2D_implicitlipid_reaction_probability(
    ...
    const std::vector<BackRxn>& backRxns, std::vector<double>& IL2DbindingVec,
    std::vector<double>& IL2DUnbindingVec, Membrane& membraneObject,
  ...
  if (moleculeList[biMolData.pro1Index].isDissociated == false) {
    ...
    if (forwardRxns[rxnIndex].rateList[rateIndex].rate > 0) {
      ...
      if (forwardRxns[rxnIndex].isReversible == true) {
        kb = backRxns[forwardRxns[rxnIndex].conjBackRxnIndex]
                 .rateList[rateIndex]
                 .rate;
      }

      for (int l = 0; l < IL2DbindingVec.size(); ++l) {
        if (std::abs(ILTableIDs[l * 3] - ktemp) < 1e-8 &&
            std::abs(ILTableIDs[l * 3 + 1] - biMolData.Dtot) < 1E-4 &&
            std::abs(ILTableIDs[l * 3 + 2] - kb) < 1e-8) {
          probValExists = true;
          probMatrixIndex = l;
          break;
        }
      ...
      if (!probValExists) {
        ...
        params2D.Na =
            membraneObject.numberOfProteinEachState
                [relStateIndex];  // the initial number of protein's interfaces
      ...
      // std::cout <<" IN DETERMIN 2D BINDING, RHO: "<<rho<<" N free lipids:
      // "<<membraneObject.No_free_lipids<<" Binding PROB: "<<rxnProb<<" ";
      moleculeList[biMolData.pro1Index].probvec.back() = rxnProb * currnorm;
      // std::cout <<" SIZE OF PROBVEC FOR MOL: "<<biMolData.pro1Index<<" size:"
      // <<moleculeList[biMolData.pro1Index].probvec.size()<<std::endl;
      moleculeList[proA].crossbase.push_back(proB);
      moleculeList[proA].mycrossint.push_back(ifaceA);
      moleculeList[proA].crossrxn.push_back(
