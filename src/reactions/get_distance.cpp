...
bool get_distance(int pro1, int pro2, int iface1, int iface2, int rxnIndex,
  ...
  bool is2D = false;
  if (complexList[moleculeList[pro1].myComIndex].D.z < 1E-8 &&
      complexList[moleculeList[pro2].myComIndex].D.z < 1E-8)
    is2D = true;
  ...
  if (membraneObject.isSphere == true && is2D == true) {
    ...
    double theta = acos((iface11.x * iface22.x + iface11.y * iface22.y +
                         iface11.z * iface22.z) /
                        r1 / r2);
    R1 = r * theta;
    ...
    double dz{
        (std::abs(complexList[moleculeList[pro1].myComIndex].D.z - 0) < 1E-10 &&
