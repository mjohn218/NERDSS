...
void associate_implicitlipid_box(
  ...
  for (int RS3Di = 0; RS3Di < 100; RS3Di++) {
    if ((std::abs(membraneObject.RS3Dvect[RS3Di] - currRxn.bindRadius) <
      ...
      RS3D =
          membraneObject
              .RS3Dvect[RS3Di + 300];  // look up value of the RS3D in a table.
      break;
    }
  }
  ...
  if (reactMol2.isImplicitLipid == true) {
    reactMol2.create_position_implicit_lipid(
        reactMol1, ifaceIndex2, currRxn.bindRadius, membraneObject);
    reactCom2.comCoord = reactMol2.comCoord;
  } else {
    reactMol1.create_position_implicit_lipid(
        reactMol2, ifaceIndex1, currRxn.bindRadius, membraneObject);
...
*/
    ...
    if (reactCom1.D.x < reactCom2.D.x) {
      ...
      memProtein =
          reactMol1;  // rotate relative to the slower protein. Clat + IL, IL is
                      // slower; Clat-IL + IL, Clat-IL is slower
    }
    ...
    // MOVE PROTEIN TO THE MEMBRANE.
    {
      ...
      Vector transVec1 =
          Vector(-(sigmaMag - currRxn.bindRadius - RS3D) / sigmaMag * sigma);
      ...
      // if (reactMol2.isImplicitLipid == true)
      //     std::cout << " transVec1 to sigma: " << transVec1.x << ' ' <<
      //     transVec1.y << ' ' << transVec1.z << std::endl;
      // if (reactMol1.isImplicitLipid == true)
      //     std::cout << " transVec2 to sigma: " << transVec2.x << ' ' <<
      //     transVec2.y << ' ' << transVec2.z << std::endl;
    ...
    if ((molTemplateList[reactMol1.molTypeIndex].isPoint &&
         reactMol2.isImplicitLipid == true) ||
      ...
      // std::cout << " Move point particle to implicit lipid NO ORIENTATION."
      // << std::endl;
    } else {  // not point.
      /* THETA */
      ...
      if (isOnMembrane == true || transitionToSurface == true) dtrans.z = 0.0;
      for (auto& mp : reactCom1.memberList)
      ...
      if (isOnMembrane == true || transitionToSurface == true) {
        /* For the impicit binding, we need to move the impicit lipid to the
         * bottom of box */
        dtrans.x = 0;
        dtrans.y = 0;
        for (auto& mp : reactCom1.memberList) {
          if (moleculeList[mp].isImplicitLipid == true) {
            ...
            if (moleculeList[mp].tmpComCoord.z - 0.01 >
              ...
              double ztmp = (-membraneObject.waterBox.z / 2.0) -
                            moleculeList[mp]
                                .tmpComCoord.z;  // lipid COM is ABOVE box
                                                 // bottom, here ztmp is negtive
              // std::cout << "WARNING, during associate, implicit LIPID IS
              // ABOVE MEMBRANE BY " << -ztmp << '\n'; move the lipid back to
              // the bottom
              if (-ztmp > zchg) {
                zchg = -ztmp;
                isAboveBottom = true;
        ...
        if (isBelowBottom == true) {
          dtrans.z = zchg;
          // std::cout << "Implicit Lipid is below membrane, shift up by: " <<
          // zchg << std::endl;
        }
        if (isAboveBottom == true) {
          dtrans.z = -zchg;
          // std::cout << "Implicit Lipid is above membrane, shift down by: " <<
          // zchg << std::endl;
        }
    ...
    check_if_spans_box(cancelAssoc, params, reactCom1, reactCom2, moleculeList,
                       membraneObject);
  ...
  if (reactMol2.isImplicitLipid == true) {
    ...
    // update complexes
    reactCom1.iLipidIndex =
        reactMol2.index;  // index in molecule list of the implicit lipid.
    ...
    // Interfaces on IL: " << moleculeList[reactMol2.index].interfaceList.size()
    // << std::endl;
    reactCom2.update_properties(
        moleculeList,
        molTemplateList);  // recalculate the properties of the second complex
    ...
    // update complexes
    reactCom2.iLipidIndex =
        reactMol1.index;  // index in molecule list of the implicit lipid.
    ...
    // std::cout << " Size of IL's complex:" << reactCom1.memberList.size() << "
    // Interfaces on IL: " << moleculeList[reactMol1.index].interfaceList.size()
    // << std::endl;
    reactCom1.update_properties(
        moleculeList,
        molTemplateList);  // recalculate the properties of the second complex
