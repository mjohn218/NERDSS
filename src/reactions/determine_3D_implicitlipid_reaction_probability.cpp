...
void determine_3D_implicitlipid_reaction_probability(
  ...
  if (std::abs(complexList[biMolData.com1Index].D.z - 0) < 1E-15) {
    ...
    double cf = cos(
        sqrt(4.0 * complexList[biMolData.com1Index].Dr.z * params.timeStep));
    Dr1 = 2.0 * biMolData.magMol1 * (1.0 - cf);
  ...
  if (std::abs(complexList[biMolData.com2Index].D.z - 0) < 1E-15) {
    ...
    double cf = cos(
        sqrt(4.0 * complexList[biMolData.com2Index].Dr.z * params.timeStep));
    Dr2 = 2.0 * biMolData.magMol2 * (1.0 - cf);
    biMolData.Dtot += Dr2 / (6.0 * params.timeStep);
  }

  double Rmax{3.0 * sqrt(6.0 * biMolData.Dtot * params.timeStep) +
  ...
  double sep{};
  double R1{};
  bool withinRmax = get_distance_to_surface(
  ...
  if (moleculeList[biMolData.pro1Index].isDissociated == false) {
    ...
    if (withinRmax && forwardRxns[rxnIndex].rateList[rateIndex].rate > 0) {
      ...
      // Evaluate probability of reaction, implicit-lipid method doesn't need
      // reweighting
      if (sep < 0) {
        // std::cout <<
        // "*****************************************************\n"
        ...
        // << " ON MOLECULE "
        //           << biMolData.pro1Index << " AND THE MEMBRANE SURFACE IS
        //           LESS THAN 0\n";
      ...
      // std::cout << "bind prob: " << std::setprecision(20) << rxnProb <<
      // std::endl; std::cout << "rho: " << std::setprecision(20) << rho <<
      // std::endl; std::cout << "z: " << std::setprecision(20) << z <<
      // std::endl; std::cout << "params3D.sigma: " << std::setprecision(20) <<
      // params3D.sigma << std::endl; std::cout << "params3D.Dtot: " <<
      // std::setprecision(20) << params3D.Dtot << std::endl; std::cout <<
      // "params3D.ka: " << std::setprecision(20) << params3D.ka << std::endl;
      // std::cout << "params3D.area: " << std::setprecision(20) <<
      // params3D.area << std::endl; std::cout << "params3D.dt: " <<
      // std::setprecision(20) << params3D.dt << std::endl; exit(1);
