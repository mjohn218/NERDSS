...
void perform_implicitlipid_state_change_sphere(
  ...
  // need to adjust Lipid's orientation, make it verticle to sphere
  {
    Molecule memProtein;
    Molecule Lipid;
    set_memProtein_sphere(stateChangeCom, memProtein, moleculeList,
                          membraneObject);
    ...
    double lamda = (membraneObject.sphereR - reactIface1.get_magnitude()) /
                   reactIface1.get_magnitude();
    Vector dtrans = Vector{lamda * reactIface1};
    // std::cout << " In ImplicitLipid model, protein interface is off spherical
    // membrane, shift up by: " << dtrans.x << " " << dtrans.y << " " <<
    // dtrans.z
    //           << std::endl;
