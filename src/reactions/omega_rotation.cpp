...
void omega_rotation(Coord& reactIface1, Coord& reactIface2, int ifaceIndex1,
  ...
  double currOmega{calculate_omega(reactIface1, ifaceIndex2, rotAxis, currRxn,
                                   reactMol1, reactMol2, molTemplateList)};

  // double omega {dotproduct(norm1, norm2)};
  // std::cout << "Desired omega: " << targOmega << " current omega: " <<
  // currOmega << std::endl;

  if (std::abs(targOmega - currOmega) < 1E-8) {
    // std::cout << "No omega rotation needed" << std::endl;
    return;
  } else if ((areSameAngle(targOmega, M_PI) || areSameAngle(targOmega, 0)) &&
             areSameAngle(targOmega, currOmega)) {
    // std::cout << "No omega rotation needed" << std::endl;
  } else {
    ...
    determine_rotation_angles(targOmega, currOmega, posOmegaRotAngle,
                              negOmegaRotAngle, reactCom1, reactCom2);
    // std::cout << "Positive half angle: " << posOmegaRotAngle << "\nNegative
    // half angle: " << negOmegaRotAngle
    //           << std::endl;

    // Create the quaternions
    Quat omegaPos(cos(posOmegaRotAngle / 2),
                  sin(posOmegaRotAngle / 2) * rotAxis.x,
                  sin(posOmegaRotAngle / 2) * rotAxis.y,
                  sin(posOmegaRotAngle / 2) * rotAxis.z);
    Quat omegaNeg(cos(negOmegaRotAngle / 2),
                  sin(negOmegaRotAngle / 2) * rotAxis.x,
                  sin(negOmegaRotAngle / 2) * rotAxis.y,
                  sin(negOmegaRotAngle / 2) * rotAxis.z);
    ...
    currOmega = calculate_omega(reactIface1, ifaceIndex2, rotAxis, currRxn,
                                reactMol1, reactMol2, molTemplateList);

    if ((areSameAngle(targOmega, M_PI) || areSameAngle(targOmega, 0)) &&
        areSameAngle(targOmega, currOmega)) {
    ...
    if (std::abs(currOmega - targOmega) > tol) {
      // std::cout << "Rotated to omega in the wrong direction. " << currOmega
      // << " Reversing and rotating the other way." << '\n';
      reverse_rotation(reactIface1, reactMol1, reactMol2, reactCom1, reactCom2,
                       omegaPos, omegaNeg, moleculeList);
      ...
      determine_rotation_angles(targOmega, currOmega, posOmegaRotAngle,
                                negOmegaRotAngle, reactCom2, reactCom1);
      ...
      //           << std::endl;
      omegaPos =
          Quat(cos(posOmegaRotAngle / 2), sin(posOmegaRotAngle / 2) * rotAxis.x,
               sin(posOmegaRotAngle / 2) * rotAxis.y,
               sin(posOmegaRotAngle / 2) * rotAxis.z);
      omegaNeg =
          Quat(cos(negOmegaRotAngle / 2), sin(negOmegaRotAngle / 2) * rotAxis.x,
               sin(negOmegaRotAngle / 2) * rotAxis.y,
               sin(negOmegaRotAngle / 2) * rotAxis.z);
      omegaPos = omegaPos.unit();
      omegaNeg = omegaNeg.unit();
      ...
      if ((areSameAngle(targOmega, M_PI) || areSameAngle(targOmega, 0)) &&
        ...
        // std::cout << "WARNING: OMEGA NOT CONVERGED TO CORRECT VALUE, CURR
        // VALUE: " << currOmega << std::endl;
      }
    }
  }
}
