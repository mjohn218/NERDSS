...
void theta_rotation(int ifaceIndex1, int ifaceIndex2, Coord& reactIface1,
  ...
  // std::cout << "Desired theta: " << targAngle << " Current theta: " <<
  // currTheta << std::endl;
  ...
  if (std::abs(targAngle - currTheta) < 1E-8) {
    ...
    // if sigma and v1 are parallel, rotation axis can't be determined by the
    // cross product so we need to find an arbitrary vector orthogonal to
    // either. Choose either x or y axis,
    // whichever one is not parallel.
    Vector rotAxis = (areParallel(currTheta)) ? create_arbitrary_vector(v1)
                                              : sigma.cross(v1);
    rotAxis.normalize();

    // Determine how far to rotate each Molecule, based on their respective
    // rotation diffusion constants
    double rotAngPos{};
    double rotAngNeg{};
    determine_rotation_angles(targAngle, currTheta, rotAngPos, rotAngNeg,
                              reactCom1, reactCom2);

    // std::cout << "Positive half angle: " << rotAngPos << "\nNegative half
    // angle: " << rotAngNeg << std::endl;

    // Create rotation quaternions
    Quat rotQuatPos{cos(rotAngPos / 2), sin(rotAngPos / 2) * rotAxis.x,
                    sin(rotAngPos / 2) * rotAxis.y,
                    sin(rotAngPos / 2) * rotAxis.z};
    Quat rotQuatNeg{cos(rotAngNeg / 2), sin(rotAngNeg / 2) * rotAxis.x,
    ...
    if ((areSameAngle(targAngle, M_PI) || areSameAngle(targAngle, 0)) &&
        areSameAngle(currTheta, targAngle)) {
