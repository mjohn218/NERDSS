...
void perform_bimolecular_state_change_sphere(
  ...
  // } //end moving to sigma
  {
    ...
    if (DzSum < 1E-14) {
      ...
      if (stateChangeCom.D.z < tol || facilitatorCom.D.z < tol) {
        transitionToSurface =
            true;  // both can't be less than tol, or would not be in this loop.
    ...
    if (isOnMembrane == true) {  // both complexes are on the sphere
      target1Pos = find_position_after_association(
          arc1Move, reactIface1, reactIface2, sigmaMag, currRxn.bindRadius2D);
      target2Pos = find_position_after_association(
          arc2Move, reactIface2, reactIface1, sigmaMag, currRxn.bindRadius2D);
    ...
    if (isOnMembrane == true) {
      stateChangeCom.update_association_coords_sphere(moleculeList, reactIface1,
                                                      target1Pos);
      facilitatorCom.update_association_coords_sphere(moleculeList, reactIface2,
                                                      target2Pos);
    } else {
      for (auto& mp : stateChangeCom.memberList)
        moleculeList[mp].update_association_coords(transVec1);
  ...
  if (molTemplateList[facilitatorMol.molTypeIndex].isPoint &&
      molTemplateList[stateChangeMol.molTypeIndex].isPoint) {
  ...
  //         memRot = save_mem_orientation(memProtein, facilitatorMol,
  //         molTemplateList[facilitatorMol.molTypeIndex]); pivot =
  //         facilitatorMol.tmpComCoord;

  //     } else {
  //         //stateChangeMol.display_assoc_icoords("CURRORIENTATION_TOROTATE2");
  //         memRot = save_mem_orientation(memProtein, stateChangeMol,
  //         molTemplateList[stateChangeMol.molTypeIndex]); pivot =
  //         stateChangeMol.tmpComCoord;
  //     }
  ...
  //     //	    std::cout <<"CRDS after forcing back to membrane bound
  //     orientation: \n";*/
  // }
  // //Coord finalCOM;//=new double[3];
  // com_of_two_tmp_complexes(facilitatorCom, stateChangeCom, finalCOM,
  // moleculeList); //com of c1+c2 (final (tmp) coordinates).
  ...
  // coordinates for both complexes for (auto& mp : facilitatorCom.memberList)
  ...
  // double zchg = 0;
  // if (isOnMembrane == 1) {
  //     /*RECHECK HERE IF ANY OF THE LIPIDS ARE SLIGHTLY BELOW THE MEMBRANE.
  ...
  //             if (moleculeList[mp].tmpComCoord.z < -membraneObject.waterBox.z
  //             / 2.0) {
  ...
  //                 double ztmp = (-membraneObject.waterBox.z / 2.0)
  //                     - moleculeList[mp].tmpComCoord.z; // lipid COM is ABOVE
  //                     box bottom

  //                 std::cout << "WARNING, during state-change associate, LIPID
  //                 IS ABOVE MEMBRANE BY " << -ztmp << '\n';
  //             }
  //         }
  //     }
  //     for (auto& mp : stateChangeCom.memberList) {
  ...
  if (isOnMembrane == true || transitionToSurface == true) {
    ...
    if (membraneObject.implicitLipid == false) {
      ...
      for (auto& mp : stateChangeCom.memberList) {
        if (moleculeList[mp].isLipid == true) {  // this is a lipid
          ...
          double drtmp = std::abs(
              rtmp -
              membraneObject.sphereR);  // lipid COM is off spherical membrane
          if (drtmp > zchg) {
            zchg = drtmp;
            targ = moleculeList[mp]
                       .tmpComCoord;  // farthest point outside membrane
          }
        }  // this is a lipid
      }
      for (auto& mp : facilitatorCom.memberList) {
      ...
      if (targ.get_magnitude() > 1E-8) {
        dtrans = Vector{(membraneObject.sphereR - targ.get_magnitude()) /
                        targ.get_magnitude() * targ};
      }
    } else {
      Coord targ{0.0, 0.0, 0.0};
      double dr = 0;
      for (auto& mp : stateChangeCom.memberList) {
        for (int i = 0; i < moleculeList[mp].interfaceList.size(); i++) {
          if (moleculeList[mp].interfaceList[i].isBound == true) {
            int index =
                moleculeList[mp].interfaceList[i].interaction.partnerIndex;
            if (moleculeList[index].isImplicitLipid == true) {
              double drtmp = moleculeList[mp].tmpICoords[i].get_magnitude() -
    ...
    // std::cout << " Lipid is off spherical membrane, shift up by: " <<
    ...
    //           << dtrans.x << ' ' << dtrans.y << ' ' << dtrans.z
    //           << std::endl; // update the temporary coordinates for both
    //           complexes
    for (auto& mp : stateChangeCom.memberList)
      moleculeList[mp].update_association_coords(dtrans);
    for (auto& mp : facilitatorCom.memberList)
  ...
  if (cancelAssoc == false)
    check_if_spans_sphere(cancelAssoc, params, facilitatorCom, stateChangeCom,
