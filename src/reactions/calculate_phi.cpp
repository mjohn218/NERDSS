...
double calculate_phi(Coord reactIface1, int ifaceIndex2, Molecule reactMol1,
  ...
  Vector vec1{reactIface1 -
              reactMol2.tmpICoords[ifaceIndex2]};  // iface1-iface2= sigma
  Vector vec2{determine_normal(normal, molTemplateList[reactMol1.molTypeIndex],
                               reactMol1)};
  ...
  /*angle between sigma and normal is phi.
    ...
    since phi here is between 0 and pi, use defined convention to decide sign
    flip.
  */
  ...
  */
  ...
  if (test.z > 0 && std::abs(phi) > 1E-12 &&
      (M_PI - std::abs(phi)) > 1E-12)  // positive z, flip phi
    phi = -phi;
