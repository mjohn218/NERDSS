...
{
    // First rotate all points in complex 1 around the reacting interface of protein p1, and translate them by vector
    for (auto& mol : targCom.memberList) {
        ...
        moleculeList[mol].tmpComCoord
            = Coord(comVec.x, comVec.y, comVec.z) + Coord(rotOrigin.x, rotOrigin.y, rotOrigin.z);
