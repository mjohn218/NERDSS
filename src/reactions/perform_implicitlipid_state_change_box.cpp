...
void perform_implicitlipid_state_change_box(
    int stateChangeIface, int facilitatorIface, std::array<int, 3>& rxnItr,
    Molecule& stateChangeMol, Molecule& facilitatorMol, Complex& stateChangeCom,
    Complex& facilitatorCom, copyCounters& counterArrays,
  ...
  // TRACE();
  const auto& stateList = molTemplateList[stateChangeMol.molTypeIndex]
                              .interfaceList[stateChangeIface]
                              .stateList;
  double bindRadius{0};
  ...
  for (int RS3Di = 0; RS3Di < 100; RS3Di++) {
    if ((std::abs(membraneObject.RS3Dvect[RS3Di] - currRxn.bindRadius) <
        ...
        std::abs(
            ...
            (1.0 / 3.0 *
                 (molTemplateList[currRxn.reactantListNew[0].molTypeIndex].D.x +
                  molTemplateList[currRxn.reactantListNew[1].molTypeIndex]
                      .D.x) +
             1.0 / 3.0 *
                 (molTemplateList[currRxn.reactantListNew[0].molTypeIndex].D.y +
                  molTemplateList[currRxn.reactantListNew[1].molTypeIndex]
                      .D.y) +
             1.0 / 3.0 *
                 (molTemplateList[currRxn.reactantListNew[0].molTypeIndex].D.z +
                  molTemplateList[currRxn.reactantListNew[1].molTypeIndex]
                      .D.z))) < 1E-15) {
  ...
  // stateChangeMol is implicit-lipid, then we need to set its temporary
  // position according to facilitatorMol. Coord displace =
  // stateChangeMol.interfaceList[stateChangeIface].coord -
  // stateChangeMol.comCoord; double shift = 0.1; //do not put right underneath,
  // so that sigma starts at non-zero. stateChangeMol.comCoord.x =
  // facilitatorMol.comCoord.x + shift; stateChangeMol.comCoord.y =
  // facilitatorMol.comCoord.y - shift; stateChangeMol.comCoord.z =
  // -membraneObject.waterBox.z / 2.0 + RS3D;

  // stateChangeMol.interfaceList[stateChangeIface].coord =
  // stateChangeMol.comCoord + displace;

  // stateChangeCom.comCoord = stateChangeMol.comCoord;

  stateChangeMol.create_position_implicit_lipid(
      facilitatorMol, stateChangeIface, currRxn.bindRadius, membraneObject);
  stateChangeCom.comCoord = stateChangeMol.comCoord;
   ...
   * complex after should be close to this                              Here, we
   * will force it back, as rotation can cause large displacements*/
  Coord startCOM;  //=new double[3];

  com_of_two_tmp_complexes(
      facilitatorCom, stateChangeCom, startCOM,
      moleculeList);  // com of c1+c2 (original coordinates).
  //    std::cout <<"INITIAL COMPLEX PAIR COM: "<<startCOM.x<<' '<<startCOM.y<<'
  //    '<<startCOM.z<<std::endl;
  ...
  //     Vector sigma { reactIface1 - reactIface2 };
  //     //        std::cout<<" sigma: "<<sigma.x<<' '<<sigma.y<<'
  //     '<<sigma.z<<std::endl;
  //     //std::cout <<" Dsum and components: "<<DxSum<<' '<<DySum<<'
  //     '<<DzSum<<std::endl;
  ...
  //             memProtein = stateChangeMol; //rotate relative to the slower
  //             protein.
  //         } else {
  ...
  //             displaceFrac = 1;
  //         } else {
  //             double sigmaMag = sqrt((sigma.x * sigma.x) + (sigma.y *
  //             sigma.y)); displaceFrac = (sigmaMag - bindRadius) / sigmaMag;
  //         }
  //     } else {
  ...
  //             would not be in this loop. std::cout << "TRANSITIONING FROM
  //             3D->2D " << std::endl;
  //         }
  //     }
  ...
  //     //std::cout<<" translation1 to sigma: "<<transVec1.x<<'
  //     '<<transVec1.y<<' '<<transVec1.z<<std::endl;
  ...
  if (facilitatorCom.D.z < 1E-15 && stateChangeCom.D.z < 1E-15) {
    isOnMembrane = true;
    // std::cout << " IMPLICIT LIPID 2D ! Value of isOnMembrane: " <<
    // isOnMembrane << std::endl;
  }
  bool transitionToSurface = false;
  ...
  // MOVE PROTEIN TO THE MEMBRANE, and place IL at sigma.
  {
    ...
    Vector transVec1 =
        Vector(-(sigmaMag - currRxn.bindRadius) / sigmaMag * sigma);
  ...
  if (molTemplateList[facilitatorMol.molTypeIndex].isPoint) {
    ...
    theta_rotation(facilitatorIface, stateChangeIface, reactIface1, reactIface2,
                   facilitatorMol, stateChangeMol, assocAngles.theta1,
                   facilitatorCom, stateChangeCom, moleculeList);

    //        write_xyz_assoc_cout( stateChangeCom, facilitatorCom,
    //        moleculeList);

    // std::cout << std::setw(30) << std::setfill('-') << ' ' << std::setfill('
    // ') << std::endl; std::cout << "THETA 2" << std::endl
    //           << std::setw(8) << std::setfill('-') << ' ' << std::setfill('
    //           ') << std::endl;
    theta_rotation(stateChangeIface, facilitatorIface, reactIface2, reactIface1,
                   stateChangeMol, facilitatorMol, assocAngles.theta2,
                   stateChangeCom, facilitatorCom, moleculeList);
    ...
    if (!std::isnan(currRxn.assocAngles.omega)) {
      omega_rotation(reactIface1, reactIface2, facilitatorIface,
                     stateChangeIface, facilitatorMol, stateChangeMol,
                     facilitatorCom, stateChangeCom, assocAngles.omega, currRxn,
    ...
    // std::cout << "P1 or P2 is a rod-type protein, no dihedral for associated
    // complex." << std::endl;
    ...
    if (!std::isnan(assocAngles.phi1)) {
      phi_rotation(reactIface1, reactIface2, facilitatorIface, stateChangeIface,
                   facilitatorMol, stateChangeMol, facilitatorCom,
                   stateChangeCom, currRxn.norm1, assocAngles.phi1, currRxn,
                   moleculeList, molTemplateList);
    ...
    if (!std::isnan(assocAngles.phi2)) {
      phi_rotation(reactIface2, reactIface1, stateChangeIface, facilitatorIface,
                   stateChangeMol, facilitatorMol, stateChangeCom,
                   facilitatorCom, currRxn.norm2, assocAngles.phi2, currRxn,
                   moleculeList, molTemplateList);
  ...
  com_of_two_tmp_complexes(
      facilitatorCom, stateChangeCom, finalCOM,
  ...
  //   '<<finalCOM.y<<' '<<finalCOM.z<<std::endl;
  {
    ...
    if (slowPro == facilitatorMol.index) {
      ...
      memRot =
          save_mem_orientation(memProtein, facilitatorMol,
                               molTemplateList[facilitatorMol.molTypeIndex]);
      pivot = facilitatorMol.tmpComCoord;

    } else {
      // stateChangeMol.display_assoc_icoords("CURRORIENTATION_TOROTATE2");
      memRot =
          save_mem_orientation(memProtein, stateChangeMol,
                               molTemplateList[stateChangeMol.molTypeIndex]);
  ...
  com_of_two_tmp_complexes(
      facilitatorCom, stateChangeCom, finalCOM,
      moleculeList);  // com of c1+c2 (final (tmp) coordinates).
  // std::cout <<"FINAL COMPLEX PAIR COM: "<<finalCOM.x<<' '<<finalCOM.y<<'
  // '<<finalCOM.z<<std::endl;
  ...
  // std::cout << "TRANSLATE COMPLEX PAIR TO ORIG COM BY SHIFTING: " << dtrans.x
  // << ' ' << dtrans.y << ' ' << dtrans.z << std::endl; // update the temporary
  // coordinates for both complexes
  for (auto& mp : facilitatorCom.memberList)
  ...
  double zchg = 0;
  {
    /*RECHECK HERE IF ANY OF THE LIPIDS ARE SLIGHTLY BELOW THE MEMBRANE. THIS
       CAN HAPPEN DUE TO PRECISION ISSUES always use tmpCoords in this associate
       routine.
         */
    dtrans.x = 0;
    dtrans.y = 0;
    for (auto& mp : facilitatorCom.memberList) {
      if (moleculeList[mp].isLipid == true) {  // this is a lipid
        if (moleculeList[mp].tmpComCoord.z < -membraneObject.waterBox.z / 2.0) {
          ...
          if (ztmp > zchg) zchg = ztmp;        // largest dip below membrane
        }
        if (moleculeList[mp].tmpComCoord.z - 0.01 >
          ...
          // std::cout << "WARNING, during state-change associate, LIPID IS
          // ABOVE MEMBRANE BY " << -ztmp << '\n';
        }
      }
    }
    for (auto& mp : stateChangeCom.memberList) {
  ...
  // //write_xyz_assoc("final.xyz", facilitatorCom, stateChangeCom,
  // moleculeList); write_xyz_assoc_cout(facilitatorCom, stateChangeCom,
  // moleculeList);
  ...
  reflect_traj_tmp_crds(params, moleculeList, facilitatorCom, traj,
                        membraneObject,
                        RS3D);  // uses tmpCoords to calculate traj.
  // reflect_traj_tmp_crds(params, moleculeList, stateChangeCom, traj,
  // membraneObject);
  ...
  check_for_structure_overlap(cancelAssoc, facilitatorCom, stateChangeCom,
                              moleculeList, params, molTemplateList);

  if (cancelAssoc == false)
    check_if_spans_box(cancelAssoc, params, facilitatorCom, stateChangeCom,
                       moleculeList, membraneObject);
  if (cancelAssoc == false)
    check_for_structure_overlap_system(
        cancelAssoc, facilitatorCom, stateChangeCom, moleculeList, params,
  ...
  // counterArrays.copyNumSpecies[currRxn.reactantListNew[0].absIfaceIndex] -=
  // 1; counterArrays.copyNumSpecies[currRxn.reactantListNew[1].absIfaceIndex]
  // -= 1; counterArrays.copyNumSpecies[currRxn.productListNew[0].absIfaceIndex]
  // += 1; counterArrays.copyNumSpecies[currRxn.productListNew[1].absIfaceIndex]
  // += 1;
  ...
  if (molTemplateList[currRxn.productListNew[1].molTypeIndex].isImplicitLipid ==
      true) {
  ...
  //     moleculeList[memMol].interfaceList.size(); ++i)
  //         moleculeList[memMol].interfaceList[i].coord =
  //         moleculeList[memMol].tmpICoords[i];
  //     moleculeList[memMol].clear_tmp_association_coords();
  //     moleculeList[memMol].trajStatus = TrajStatus::propagated;
  ...
  // for (unsigned crossItr { 0 }; crossItr < stateChangeMol.crossbase.size();
  // ++crossItr) {
  //     int skipMol { stateChangeMol.crossbase[crossItr] };
  //     for (unsigned crossItr2 { 0 }; crossItr2 <
  //     moleculeList[skipMol].crossbase.size(); ++crossItr2) {
  //         if (moleculeList[skipMol].crossbase[crossItr2] ==
  //         stateChangeMol.index)
  ...
  for (unsigned crossItr{0}; crossItr < facilitatorMol.crossbase.size();
       ++crossItr) {
  ...
  if (!isStateChangeBackRxn && forwardRxns[rxnIndex].isObserved) {
    ...
    if (observeItr == observablesList.end()) {
      ...
      // forwardRxns[rxnIndex].observeLabel << " not defined.\n";
    } else {
      ++observeItr->second;
    }
    ...
    if (observeItr == observablesList.end()) {
      // std::cerr << "WARNING: Observable " << backRxns[rxnIndex].observeLabel
      // << " not defined.\n";
    } else {
      --observeItr->second;
    }
  ...
  // counterArrays.copyNumSpecies[stateChangeMol.interfaceList[stateChangeIface].stateIndex]++;
