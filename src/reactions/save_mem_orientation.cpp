...
/*Calculate rotation matrix for orienting one molecule to itself (at another
  timepoint, e.g.) One of these molecules (the first one) is true coords:
  baseTarget, the other is temp coords (base1).
 */
Quat save_mem_orientation(Molecule baseTarget, Molecule baseTmp,
                          MolTemplate onePro) {
  ...
  for (int i = 0; i < baseTarget.interfaceList.size(); i++) {
    copy.interfaceList.emplace_back(
        iface, Coord{baseTarget.interfaceList[i].coord -
                     baseTarget.comCoord});  // subtract off com if non-zero
  }

  for (int i = 0; i < baseTmp.interfaceList.size(); i++) {
    baseTmp.tmpICoords[i] =
        baseTmp.tmpICoords[i] - baseTmp.tmpComCoord;  // subtract off com
  }
  ...
  Quat qRot = orient_crds_to_template(
      copy, baseTmp);  // first input is a MolTemplate, second is a Molecule
  // std::cout<<" QUAT: "<<qRot.x<<' '<<qRot.y<<' '<<qRot.z<<'
  // '<<qRot.w<<std::endl; cout <<"Reoriented to template Proten. "<<endl;
