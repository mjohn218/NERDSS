...
void print_association_events(copyCounters& counterArrays,
  ...
  int arraySize =
      counterArrays
          .eventArraySize;  // final array element contains all additions above
                            // (arraySize-maxSingles)*spacing
  outfile << "time (s): "
    ...
    if (counterArrays.events3D[i] > 0) {
      if (i == 0)
        ...
        if (i == arraySize - 1)
          ...
          text = "3D n=" + std::to_string(n) + " to " +
                 std::to_string(n + spacing - 1);
      }
  ...
  for (int i = 0; i < arraySize; i++) {
    if (counterArrays.events3Dto2D[i] > 0) {
      if (i == 0)
        ...
        if (i == arraySize - 1)
          text =
              "3Dto2D n>" + std::to_string(n);  // this is the max index stored.
        else
          text = "3Dto2D n=" + std::to_string(n) + " to " +
                 std::to_string(n + spacing - 1);
      }
