...
int find_reaction_rate_state(int simItr, int relIfaceIndex1, int relIfaceIndex2,
  ...
  for (int reactItr{0}; reactItr < backRxn.reactantListNew.size(); ++reactItr) {
    if (reactMol1.molTypeIndex ==
            backRxn.reactantListNew[reactItr].molTypeIndex &&
  ...
  if (reactIndex1 == -1 || reactIndex2 == -1) {
    std::cerr << llinebreak;
    std::cerr
        << "ERROR: Association product has incorrect dissociation reaction.\n";
  ...
  for (auto& oneRate : backRxn.rateList) {
    if (hasIntangibles(reactIndex1, reactIndex2, reactMol1, reactMol2,
      ...
      //            &backRxn.rateList[0]), (oneRate.otherIfaceLists[0].size() +
      //            oneRate.otherIfaceLists[1].size()));
      std::array<int, 2> tmpArr{
          {static_cast<int>(&oneRate - &backRxn.rateList[0]),
  ...
  if (matches == 1) {
    ...
    //        std::cerr << "Error, no matching dissociation rate state was found
    //        for product " << exit(1);
  } else if (matches == 0)
    return -1;
  else {
    // if there are multiple matches, see which fits best, e.g. the rate with
    // the most required ancillary interfaces
    int bestFitIndex{0};
    int numAnccIfaces{0};
