...
bool determine_parent_complex_IL(int pro1Index, int pro2Index, int newComIndex,
  ...
  complexList[c1].memberList.insert(complexList[c1].memberList.end(),
                                    doneSet.begin(), doneSet.end());
  complexList[newComIndex].memberList.clear();
  for (auto& m : origlist) {
    if (doneSet.find(m) == doneSet.end())
      complexList[newComIndex].memberList.push_back(m);
  }
