...
Coord find_spherical_coords(
    Coord mol)  // mol: cardesian coords, output spherical coords
{
  Coord angles;
  double R = radius(mol);
  if (mol.z == R) {
...
Coord find_cardesian_coords(
    Coord mol)  // mol: spherical coords, output cardesian coords
{
  Coord xyz;
  double theta = mol.x;
  double phi = mol.y;
...
Coord find_position_after_association(double arc1, Coord Iface1, Coord Iface2,
                                      double arc_total, double bindRadius) {
  double x1 = Iface1.x;
  double y1 = Iface1.y;
  double z1 = Iface1.z;
  double x2 = Iface2.x;
  ...
  double distance2 =
      sqrt(pow(X2 - x2, 2.0) + pow(Y2 - y2, 2.0) + pow(Z2 - z2, 2.0));
  ...
  if (std::isnan(new_position1.x) || std::isnan(new_position1.y)) {
    std::cout << "WRONG: non position is generated in "
                 "'find_position_after_association'...EXIT! "
              << std::endl;
    exit(1);
  }
...
std::array<double, 9> inner_coord_set(Coord com, Coord comnew) {
  ...
  crdset[8] = 0;
  Vector i, j, k, v;
  if (sqrt(pow(com.x - comnew.x, 2.0) + pow(com.y - comnew.y, 2.0) +
           pow(com.z - comnew.z, 2.0)) < 1E-8) {
    // std::cout<<"During the tranlsation on sphere, the complex doesn't move,
    // then use the default inner_coord_set"<<std::endl;
...
std::array<double, 9> inner_coord_set_new(Coord com, Coord comnew) {
  ...
  if (sqrt(pow(com.x - comnew.x, 2.0) + pow(com.y - comnew.y, 2.0) +
           pow(com.z - comnew.z, 2.0)) < 1E-8) {
    // std::cout<<"During the tranlsation on sphere, the complex doesn't move,
    // then use the default inner_coord_set_new"<<std::endl;
...
std::array<double, 3> calculate_inner_coord_coefficients(
    Coord TARG, Coord COM, std::array<double, 9> crdset) {
  ...
  // check whether targ vector is perpenticular to any of the coords
  ...
  if (std::abs(Targ.dot(i)) < 1E-8) {  // targ is perpenticular to i
    ...
    } else if (std::abs(Targ.dot(k)) <
               1E-8) {  // targ is also perpenticular to k
      gama = 0.0;
      beta = targ.magnitude;
      if (Targ.dot(j) < 0.0) {
        beta = -beta;
...
Coord translate_on_sphere(Coord targ, Coord COM, Coord COMnew,
                          std::array<double, 9> crdset,
                          std::array<double, 9> crdsetnew) {
  ...
  std::array<double, 3> M =
      calculate_inner_coord_coefficients(targ, COM, crdset);
  double alpha = M[0];
  double beta = M[1];
  double gama = M[2];
...
// input and output are cardesian coords
Coord rotate_on_sphere(Coord Targ, Coord COM, std::array<double, 9> crdset,
                       double dangle) {
  Coord targnew;

  // targ will rotate along O-COM line, i.e. i axis. so its projection along i
  // is not change
  Vector i = Vector{crdset[0], crdset[1], crdset[2]};
  Vector j = Vector{crdset[3], crdset[4], crdset[5]};
  Vector k = Vector{crdset[6], crdset[7], crdset[8]};
  ...
  if (targjk.magnitude < 1E-8 || std::abs(targi.magnitude - 1.0) < 1E-8) {
    ...
    phi = phi + dangle;
    targjk =
        j * (targjk.magnitude * cos(phi)) + k * (targjk.magnitude * sin(phi));
    targ = targi + targjk;
  ...
  if (std::isnan(targnew.x)) {
    if (targjk.magnitude < 1E-8 || std::abs(targi.magnitude - 1.0) < 1E-8) {
      ...
      std::cout
          << "WRONG! NON is generated after the rotation on sphere! EXIT..."
          << std::endl;
      exit(1);
    }
  }
...
void set_memProtein_sphere(Complex reactCom, Molecule& memProtein,
  ...
  // if (membraneObject.implicitLipid == false){ //for explicit lipid model;
  // lipid is a member of reactCom
  double r = 0.0;
  for (auto mol : reactCom.memberList) {
    if ((moleculeList[mol].isLipid == true ||
         moleculeList[mol].isImplicitLipid == true) &&
        moleculeList[mol].tmpComCoord.get_magnitude() > r) {
      memProtein = moleculeList[mol];
      r = moleculeList[mol].tmpComCoord.get_magnitude();
    }
  }
  memProtein.comCoord = memProtein.tmpComCoord;
  for (int i = 0; i < memProtein.interfaceList.size();
       i++) {  // here memProtein is an Lipid, has only one interface
  ...
  // for implicit lipid model, on 2D->2D case, reactCom has no implicitlipid
  // member.
  if (memProtein.isLipid == false && memProtein.isImplicitLipid == false &&
    ...
    for (auto mol : reactCom.memberList) {
      for (int i = 0; i < moleculeList[mol].interfaceList.size(); i++) {
        if (moleculeList[mol].interfaceList[i].isBound == true) {
          int index =
              moleculeList[mol].interfaceList[i].interaction.partnerIndex;
          if (moleculeList[index].isImplicitLipid == true &&
              moleculeList[mol].tmpICoords[i].get_magnitude() > r) {
    ...
    memProtein.comCoord =
        iface;  // here targ is an ImplicitLipid, has only one interface
  ...
  if (memProtein.isLipid == false && memProtein.isImplicitLipid == false) {
    std::cout << "WRONG: failed to create memProtein, in the step to adjust "
                 "complex's orientation on sphere. Exit..."
              << std::endl;
    exit(1);
  }
  // memProtein.comCoord =  (memProtein.comCoord.get_magnitude() + 0.1) /
  // memProtein.comCoord.get_magnitude()  * memProtein.comCoord;
  // memProtein.interfaceList[0].coord =
  // (memProtein.interfaceList[0].coord.get_magnitude() +
  // 0.1)/memProtein.interfaceList[0].coord.get_magnitude() *
  // memProtein.interfaceList[0].coord;
}
void find_Lipid_sphere(Complex reactCom, Molecule& Lipid,
                       std::vector<Molecule> moleculeList,
                       const Membrane membraneObject) {
  // if (membraneObject.implicitLipid == false){ //for explicit lipid model;
  // lipid is a member of reactCom
  ...
  // Lipid.comCoord = ( Lipid.comCoord.get_magnitude()+
  // 0.1)/Lipid.comCoord.get_magnitude() * Lipid.comCoord; Lipid.tmpComCoord = (
  // Lipid.tmpComCoord.get_magnitude()+ 0.1)/Lipid.tmpComCoord.get_magnitude() *
  // Lipid.tmpComCoord; Lipid.interfaceList[0].coord = (
  // Lipid.interfaceList[0].coord.get_magnitude()+
  // 0.1)/Lipid.interfaceList[0].coord.get_magnitude() *
  // Lipid.interfaceList[0].coord; Lipid.tmpICoords[0] = (
  // Lipid.tmpICoords[0].get_magnitude()+
  // 0.1)/Lipid.tmpICoords[0].get_magnitude() * Lipid.tmpICoords[0];
}