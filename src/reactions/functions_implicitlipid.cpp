...
// h is the time-step; sigma is the bind_radius, Na is the number of proteins in
// solution, Nlipid is the number of lipids on the membrane surface, A is the
...
double dissociate2D(paramsIL& parameters2D) {
  ...
  double kon = 1.0 / (1.0 / ka + 1.0 / (8.0 * M_PI * D) *
                                     (4.0 * log(b / sigma) /
                                          pow(1.0 - pow(sigma / b, 2.0), 2.0) -
                                      2.0 / (1.0 - pow(sigma / b, 2.0)) - 1.0));
  double koff = kon * KD;
...
double function2D(double u, void* parameter) {
  ...
  alpha =
      H * u * gsl_sf_bessel_Y1(sigma * u) + ka * gsl_sf_bessel_Y0(sigma * u);
  ...
  double out = 1.0 / pow(u, 3.0) * (exp(-D * u * u * h) - 1.0) /
               (alpha * alpha + peta * peta) * (alpha * a - peta * b);
  return out;
}
...
double integral_for_blockdistance2D(paramsIL& parameters2D) {
  ...
  int status =
      gsl_integration_qagiu(&F, 0, eps1, eps2, 1000000, w, &result, &error);
  if (status != GSL_SUCCESS) {
    ...
    while (status != GSL_SUCCESS) {
      status = gsl_integration_qags(&F, u1, u2, eps1, eps1, 1000000, w, &result,
                                    &error);
      u2 = u2 * 0.9;
    }
  }
...
///////////////////////////////////////////////////////////////
// 3D
double dissociate3D(double h, double D, double sigma, double ka,
                    double kbsecond) {
...
double pimplicitlipid_3D(double z, paramsIL& parameters3D) {
  ...
  if (z > sigma) {
    ...
    double conf = 2.0 * M_PI * sigma * sigma * ka * (4.0 * M_PI * sigma * D) /
                  (ka + 4.0 * M_PI * sigma * D) / (ka + 4.0 * M_PI * sigma * D);
    double a = (z - sigma) / sqrt(4.0 * D * h);
    double b = alpha * sqrt(h);
    if (std::isinf(exp(2.0 * a * b + b * b))) {
      out = conf * (exp(-a * a) / sqrt(M_PI) / (a + b) -
                    (2.0 * a * b + 1.0) * erfc(a) +
                    2.0 * alpha * sqrt(h / M_PI) * exp(-a * a));
    } else {
      out = conf * (exp(2.0 * a * b + b * b) * erfc(a + b) -
                    (2.0 * a * b + 1.0) * erfc(a) +
                    2.0 * alpha * sqrt(h / M_PI) * exp(-a * a));
    }
  } else {
    z = sigma;
    double alpha = sqrt(D) / sigma * (1.0 + ka / (4.0 * M_PI * sigma * D));
    double conf = 2.0 * M_PI * sigma * ka * sqrt(D) / alpha /
                  (ka + 4.0 * M_PI * sigma * D);
    double a = 0;
    double b = alpha * sqrt(h);
