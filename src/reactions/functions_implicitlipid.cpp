...
double dissociate2D(paramsIL& parameters2D)
{
    ...
    double KD = kb/ka; 
    double a = (Na-Nlipid-KD*A);
    double na = 0.5*( a + sqrt(a*a+4*KD*A*Na) );
    double nlipid = Nlipid-(Na-na);
    
    double b = 2.0*sqrt( A/M_PI/nlipid + sigma*sigma );
...
double integral_for_blockdistance2D(paramsIL& parameters2D)
{
    ...
    if (status != GSL_SUCCESS) 
    {
        double u1=0;
        double u2=1.0e4;
        while (abs(function2D(u2,F.params)) > 1.0e-5) {   u2=u2*1.5; }
