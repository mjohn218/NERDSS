				  ...
				  std::map<std::string, int>& observablesList, copyCounters& counterArrays, std::vector<Complex>& complexList, const Membrane &membraneObject, const std::vector<ForwardRxn> &forwardRxns, const std::vector<BackRxn> &backRxns)
{
  ...
  if(reactMol2.isImplicitLipid==true){
    ...
    reactMol2.comCoord.z = -membraneObject.waterBox.z/2.0+membraneObject.RS3D;
    ...
    reactMol1.comCoord.z = -membraneObject.waterBox.z/2.0+membraneObject.RS3D;
	...
	bool transitionToSurface = false;
        if (isOnMembrane == false){ transitionToSurface = true; }
        Molecule memProtein;
	...
	// write_xyz_assoc_cout(reactCom1, reactCom2, moleculeList);
        ...
        */
        ...
        reflect_traj_tmp_crds(params, moleculeList, reactCom1, traj, membraneObject); // uses tmpCoords to calculate traj.
	...
	if(reactMol2.isImplicitLipid==true){
	  ...
	  reflect_complex_rad_rot(membraneObject, reactCom1, moleculeList);
	  ...
	  //Enforce boundary conditions
	  reflect_complex_rad_rot(membraneObject, reactCom2, moleculeList);
