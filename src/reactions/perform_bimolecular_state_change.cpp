				      ...
				      std::vector<MolTemplate>& molTemplateList, std::map<std::string, int>& observablesList, const Membrane &membraneObject)
{
	...
	*/
	...
	update_complex_tmp_com_crds(stateChangeCom, moleculeList);
        ...
        reflect_traj_tmp_crds(params, moleculeList, facilitatorCom, traj, membraneObject);//uses tmpCoords to calculate traj.
    ...
    reflect_complex_rad_rot(membraneObject, facilitatorCom, moleculeList);
