...
void find_which_state_change_reaction(
  ...
  for (auto rxnItr : currState.stateChangeRxns) {
    const ForwardRxn& oneRxn =
        forwardRxns[rxnItr.first];  // rxnItr.first is the reaction's index in
                                    // forwardRxns
    if (oneRxn.rxnType == ReactionType::uniMolStateChange) {
      if (reactMol.interfaceList[ifaceIndex].index ==
          oneRxn.reactantListNew[0].absIfaceIndex) {
        int matches{0};
        ...
        if (matches == 0) {
          // TODO: either an error or the molecule doesn't match all the
          // requirements. do what?
        } else if (matches > 1) {
      ...
      } else if (reactMol.interfaceList[ifaceIndex].index ==
                 oneRxn.productListNew[0].absIfaceIndex) {
        int matches{0};
        for (const auto& oneRate : backRxns[oneRxn.conjBackRxnIndex].rateList) {
          if (hasIntangibles(0, reactMol, oneRate)) {
            ++matches;
            rateIndex = static_cast<int>(
                &oneRate - &backRxns[oneRxn.conjBackRxnIndex].rateList[0]);
          }
        }
