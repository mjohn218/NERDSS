...
// only works for sphere system, and reactCom1 binds to surface by 3D->2D or
// 2D->2D.
//
void associate_implicitlipid_sphere(
  ...
  // mol2 is implicit-lipid, then we need to set its temporary position
  // according to mol1.
  reactMol2.create_position_implicit_lipid(reactMol1, ifaceIndex2,
                                           currRxn.bindRadius, membraneObject);
  ...
  // std::cout << "ImplicitLipid model, need to rotate the complex, and make it
  // stick on the sphere.  \n"; std::cout << "Pre-MEMBRANE ROT, COORDS: " <<
  ...
  if (molTemplateList[reactMol1.molTypeIndex].isPoint) {
    ...
    theta_rotation(ifaceIndex1, ifaceIndex2, reactIface1, reactIface2,
                   reactMol1, reactMol2, currRxn.assocAngles.theta1, reactCom1,
    ...
    theta_rotation(ifaceIndex2, ifaceIndex1, reactIface2, reactIface1,
                   reactMol2, reactMol1, currRxn.assocAngles.theta2, reactCom2,
...
*/
  ...
  check_if_spans_sphere(cancelAssoc, params, reactCom1, reactCom2, moleculeList,
                        membraneObject);
  ...
  // if (reactMol2.isImplicitLipid == true)
  {
    ...
    reactCom1.D.z =
        0.0;  // maybe redundant, since we can use linksToSurface > 0
    reactCom1.OnSurface =
        true;  // maybe redundant, since we can use linksToSurface > 0
    ...
    // For the sphere system, many times of reflections may need to move the
    // complex back inside the sphere!!
