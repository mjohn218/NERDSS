...
void create_survMatrix(gsl_matrix*& survMatrix, double bindRadius, double Dtot,
  ...
  while (RIndex <= comRMax + RStepSize) {
    //    for (double RIndex = intParams.a; RIndex <= com + RStepSize; RIndex +=
    //    RStepSize) {
    intParams.r0 = RIndex;
    ...
    if (kr < 1.0 / 0.0) {
      double result{integrator(F, intParams, w, RIndex, bindRadius, Dtot, kr,
                               params.timeStep, funcID, survival_function)};
