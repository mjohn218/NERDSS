...
void check_dissociation_implicitlipid(
    unsigned int simItr, const Parameters& params, SimulVolume& simulVolume,
    std::vector<MolTemplate>& molTemplateList,
    std::map<std::string, int>& observablesList, unsigned int molItr,
    ...
    std::vector<double>& IL2DbindingVec, std::vector<double>& IL2DUnbindingVec,
    std::vector<double>& ILTableIDs) {
  // TRACE();
  double Dtot;
  for (int relIface1Itr{0}; relIface1Itr < moleculeList[molItr].bndlist.size();
       ++relIface1Itr) {
    int relIface1{moleculeList[molItr].bndlist[relIface1Itr]};
    // std::cout<<"moleculeList[molItr].interfaceList[relIface1].isBound:
    // "<<moleculeList[molItr].interfaceList[relIface1].isBound<<std::endl;
    if (moleculeList[molItr]
            .interfaceList[relIface1]
            .isBound) {  // make sure it's actually bound
      int pro2Index{moleculeList[molItr]
                        .interfaceList[relIface1]
                        .interaction.partnerIndex};
      int relIface2{moleculeList[molItr]
                        .interfaceList[relIface1]
      ...
      if (complexList[moleculeList[molItr].myComIndex].OnSurface == false ||
          moleculeList[pro2Index].isImplicitLipid == false)
        continue;

      int mu{moleculeList[molItr]
                 .interfaceList[relIface1]
                 .interaction.conjBackRxn};
      // if the reaction is irreversible, the conjBackRxn index will be -1, so
      // continue
      if (mu == -1) continue;

      int rateItr{find_reaction_rate_state(
          simItr, relIface1, relIface2, moleculeList[molItr],
          moleculeList[pro2Index], backRxns[mu], molTemplateList)};
      if (rateItr == -1) continue;

      double kb{backRxns[mu].rateList[rateItr].rate};  // <- kr[mu]
      int kfIndex =
          backRxns[mu].conjForwardRxnIndex;  //!< the index of this reaction's
                                             //!< ForwardRxn counterpart
      double ka = forwardRxns[kfIndex].rateList[rateItr].rate;
      double prob;

      /*Calculate Dtot for the binding reaction that gave rise to this complex.
        Assume Dcomplex=(1/D1+1/D2)-1 (two components)
        D1=molTemplate[implicitLipidIndex].D, and Dcomplex is current D, solve
        for D2, and Dtot=D1+D2.
       */
      // std::cout <<" In check implicit lipid Unbinding Complex D:
      // "<<Dcomplex_x<<' '<<Dcomplex_y<<' '<<Dcomplex_z<<" And DIL:" <<DIL_x<<'
      // '<<DIL_y<<' '<<DIL_z<<std::endl; std::cout <<" In check implicit lipid
      // Unbinding, calculated Dtot of protein bound to IL: "<<D2_x<<'
      // '<<D2_y<<' '<<D2_z<<" And Dtot:" <<Dtot<<std::endl;
      //  the expression of unbinding probability varies as implicit-lipid model
      //  (2D/3D) and explicit-lipid model if the complex has only one
      //  protein-implicit.lipid bond, then the unbinding will make the complex
      //  into solution, 2D->3D, then the reflect-surface RS3D needs to be
      //  introduced; otherwise, it is just 2D->2D, we donnot need use RS3D.
      //  std::cout<<"complexList[moleculeList[molItr].myComIndex].linksToSurface:
      //  "<<complexList[moleculeList[molItr].myComIndex].linksToSurface<<std::endl;
      if (complexList[moleculeList[molItr].myComIndex].linksToSurface == 1 &&
          membraneObject.TwoD == false) {
        // 2D->3D
        double Dcomplex_x = complexList[moleculeList[molItr].myComIndex].D.x;
        double DIL_x =
            molTemplateList[moleculeList[pro2Index].molTypeIndex].D.x;
        double D2_x = 1.0 / (1.0 / Dcomplex_x - 1.0 / DIL_x);
        // If it is a protein bound to a lipid only, then DIL might be==
        // Dcomplex due to forced rounding
        ...
        double DIL_y =
            molTemplateList[moleculeList[pro2Index].molTypeIndex].D.y;
        ...
        double DIL_z =
            molTemplateList[moleculeList[pro2Index].molTypeIndex].D.z;
        double D2_z = molTemplateList[moleculeList[molItr].molTypeIndex].D.z;

        Dtot = 1.0 / 3.0 * (D2_x + DIL_x) + 1.0 / 3.0 * (D2_y + DIL_y) +
               1.0 / 3.0 * (D2_z + DIL_z);

        double sigma = forwardRxns[kfIndex].bindRadius;
        double h = params.timeStep;
        prob = dissociate3D(h, Dtot, sigma, 2.0 * ka,
                            kb);  // membraneObject.Punbinding3D;

        // make sure that the time step is resonable according to the prob of
        // reaction
        if (prob > 1.000001) {
          std::cerr << "Error: prob of reaction > 1. Avoid this using a "
                       "smaller time step."
                    << std::endl;
          //  exit(1);
        }
        if (prob > 0.5) {
          // std::cout << "WARNING: prob of reaction > 0.5. If this is a
          // reaction for a bimolecular binding with multiple binding sites,
          // please use a smaller time step." << std::endl;
        }
      } else {
        // 2D->2D
        ...
        int proteinIndex{-1};
        if (molTemplateList
                [forwardRxns[kfIndex].reactantListNew[0].molTypeIndex]
        ...
        for (int p1 = 0; p1 < moleculeList.size(); p1++) {
          if (moleculeList[p1].isLeftGhost == true || moleculeList[p1].isRightGhost) continue;
        ...
        if (std::abs(Dcomplex_x - DIL_x) < 1E-10)
          D2_x =
              DIL_x;  // molTemplateList[moleculeList[molItr].molTypeIndex].D.x;
        ...
        if (std::abs(Dcomplex_y - DIL_y) < 1E-10)
          D2_y =
              DIL_y;  // molTemplateList[moleculeList[molItr].molTypeIndex].D.y;
        // For z, both are currently on the membrane, so Dcomplex_z and DIL_z==0
        double Dcomplex_z = complexList[moleculeList[molItr].myComIndex].D.z;
        double DIL_z =
            molTemplateList[moleculeList[pro2Index].molTypeIndex].D.z;
        double D2_z = 0;
        // If Links>1, then it is stuck in 2D even if this link breaks, so
        // Dz=0.//otherwise, use the bound protein's Dz
        ...
        params2D.Nlipid =
            membraneObject
                .numberOfFreeLipidsEachState[relStateIndex];  // free lipids
      ...
      if (params.debugParams.forceDissoc) prob = 1.0;
      ...
      // std::cout<<"dissociate prob: "<<prob<<std::endl;
      if (prob > rnum) {
        // std::cout << "Dissociation at iteration: " << simItr << " protein: "
        // << molItr
        //           << " from the membrane, with probability: " << prob <<
        //           '\n';
        // std::cout << "Complex " << moleculeList[molItr].myComIndex << ",
        // composed of "
        //           <<
        ...
        /*std::cout << "Complex members:";
                for (const auto& memMol :
           complexList[moleculeList[molItr].myComIndex].memberList) std::cout <<
           ' ' << memMol; std::cout << '\n';
                */

        // 'break_interaction' frees certain protein and its interface
        break_interaction_implicitlipid(
            relIface1, relIface2, moleculeList[molItr], moleculeList[pro2Index],
            backRxns[mu], moleculeList, complexList, molTemplateList, params);

        // Change the number of bound pairs in the system.
        update_Nboundpairs(moleculeList[molItr].molTypeIndex,
                           moleculeList[pro2Index].molTypeIndex, -1, params,
                           counterArrays);
        // Update species copy numbers
        // std::cout <<"Add new to index:"
        // <<backRxns[mu].productListNew[0].absIfaceIndex<<'
        // '<<backRxns[mu].productListNew[1].absIfaceIndex; std::cout
        // <<"Subtract from index:
        // "<<backRxns[mu].reactantListNew[0].absIfaceIndex<<std::endl;
        ...
        //  += 1;
        // counterArrays.copyNumSpecies[moleculeList[molItr].interfaceList[relIface1].index]
        // += 1;
        // counterArrays.copyNumSpecies[moleculeList[pro2Index].interfaceList[relIface2].index]
        // += 1;
        ...
        const auto& implicitLipidStateList =
            molTemplateList[moleculeList[membraneObject.implicitlipidIndex]
                ...
                .interfaceList[0]
                .stateList;
        if (molTemplateList[currRxn.productListNew[1].molTypeIndex]
                .isImplicitLipid == true) {
        ...
        // update the number of bonds that this complex has connected to the
        // membrane surface.
        ...
        if (complexList[moleculeList[molItr].myComIndex].linksToSurface < 1 &&
          ...
          if (membraneObject.isSphere) {
            ...
            transVec1.z =
                ...
                moleculeList[molItr]
                    .interfaceList[relIface1]
                    .coord.z;  // move this interface to the RS3D position in z.
            // std::cout << " unbinding POSITION UPDATE IN Z: " << transVec1.z
            // << std::endl;
          }
          complexList[moleculeList[molItr].myComIndex].OnSurface =
              false;  // the complex is not bound to the membrane anymore.
        } else {
          transVec1.x = 0;
          transVec1.y = 0;
          transVec1.z = 0;
        ...
        // BINDING CURRENT PUTS PROTEINS AT RS3D, SO NO DISPLACEMENT AFTER
        // DISSOCIATIONS
        //  update the temporary coordinates. If binding places it at RS, do not
        //  move here!!
        ...
        for (auto memMol :
          ...
          for (unsigned int i{0}; i < moleculeList[memMol].interfaceList.size();
               ++i)
            moleculeList[memMol].interfaceList[i].coord =
                moleculeList[memMol].tmpICoords[i];
          moleculeList[memMol].clear_tmp_association_coords();
        }
        //*/
        complexList[moleculeList[molItr].myComIndex].update_properties(
            moleculeList, molTemplateList);  // recalculate the properties of
                                             // the first complex
        ...
        // complexList[moleculeList[molItr].myComIndex].trajStatus =
        ...
        complexList[moleculeList[molItr].myComIndex].trajStatus =
            TrajStatus::propagated;
        moleculeList[molItr].isDissociated = true;

        // TODO: Temporary implementation for destruction coupled to
        // dissociation
        /*Must be a C->A+B
        ...
        if (backRxns[mu].isCoupled) {
          if (backRxns[mu].coupledRxn.rxnType == ReactionType::destruction) {
            ...
            const CreateDestructRxn& coupledRxn = createDestructRxns
                [backRxns[mu].coupledRxn.relRxnIndex];  // which reaction is
                                                        // being
            // performed.
            ...
            if (moleculeList[destroyProIndex].isImplicitLipid == true) {
              int indexIlState = coupledRxn.reactantMolList.at(0)
                                     .interfaceList.at(0)
                                     .absIfaceIndex;
              ...
              // for (auto& memMol :
              // complexList[moleculeList[destroyProIndex].myComIndex].memberList)
              // {
              //     for (auto& iface : moleculeList[memMol].interfaceList) {
              //         --counterArrays.copyNumSpecies[iface.index];
              //     }
              // }
              complexList[moleculeList[destroyProIndex].myComIndex].destroy(
                  moleculeList,
                  complexList);  // destroying the entire complex that this
              ...
              for (auto itr = simulVolume
                                  .subCellList[moleculeList[destroyProIndex]
                                                   .mySubVolIndex]
                                  .memberMolList.begin();
                   itr !=
                   simulVolume
                       .subCellList[moleculeList[destroyProIndex].mySubVolIndex]
                       .memberMolList.end();
                   ++itr) {
          ...
          if (backRxns[mu].coupledRxn.rxnType ==
              ReactionType::uniMolStateChange) {
            int stateChangeProIndex{-1};
            int relIndex{-1};
            const ForwardRxn& coupledRxn =
                forwardRxns[backRxns[mu]
                                .coupledRxn.relRxnIndex];  // which reaction is
                                                           // being performed.
            if (molTemplateList[coupledRxn.reactantListNew[0].molTypeIndex]
                    .isImplicitLipid == true) {
              int indexIlState = coupledRxn.reactantListNew[0].absIfaceIndex;
              int indexIlStateNew = coupledRxn.productListNew[0].absIfaceIndex;

              // std::cout << "Performing coupled IL uniMolStateChange
              // reaction.\n";
              ...
              // make sure the molecule iface have the same state with the
              // uniMolStateChange reaction's reactant
              ...
              if (stateChangeProIndex == -1) {
                std::cerr << "The products of the disscociation do not match "
                             "the corresponding uniMolStateChange reactant."
                          << std::endl;
                exit(1);
              }
              ...
              moleculeList[stateChangeProIndex].trajStatus =
                  TrajStatus::propagated;
              complexList[moleculeList[stateChangeProIndex].myComIndex]
        ...
        if (backRxns[mu].isObserved) {
          ...
          if (obsItr != observablesList.end()) --obsItr->second;
        }
      }
    }
  }
