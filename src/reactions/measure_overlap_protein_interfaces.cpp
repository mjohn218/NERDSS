...
void measure_overlap_protein_interfaces(Molecule base1, Molecule baseTmp,
  ...
  for (unsigned i{0}; i < base1.interfaceList.size(); i++) {
    ...
    for (int m = 0; m < baseTmp.interfaceList.size(); m++) {
      ...
      double d2;
      double bindrad2 = 1.0;  // THIS SHOULD BE AN ACTUAL BINDING RADIUS squared
                              // BETWEEN INTERFACES WITHIN THIS PROTEIN
      ...
      if (d2 < bindrad2) {
        // std::cout << " WARNING: Cancel Association, overlap with INTERFACES
        // in SYSTEM ! " << i << ' ' << m
        //           << " separation : " << sqrt(d2) << " from proteins: " <<
        //           '\t';
        // cout <<" UNBOUND PROTEINSs "<<mp<<' '<<mp2 <<" Interfaces:
        // "<<myIfaceIndex<<' '<<m<<  " WITHIN A COMPLEX ARE CLOSE TOGETHER:
        // "<<d<<endl;
        flagCancel = true;
      }
