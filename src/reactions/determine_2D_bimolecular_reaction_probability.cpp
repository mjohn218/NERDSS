...
void determine_2D_bimolecular_reaction_probability(
    int simItr, int rxnIndex, int rateIndex, bool isStateChangeBackRxn,
    unsigned& DDTableIndex, double* tableIDs, BiMolData& biMolData,
  ...
  {
    double cf{cos(
        sqrt(2.0 * complexList[biMolData.com1Index].Dr.z * params.timeStep))};
  ...
  {
    double cf = cos(
        sqrt(2.0 * complexList[biMolData.com2Index].Dr.z * params.timeStep));
    Dr2 = 2.0 * biMolData.magMol2 * (1.0 - cf);
  }

  biMolData.Dtot +=
      (Dr1 + Dr2) /
      (4.0 * params.timeStep);  // add in contributions from rotation

  {
    // Only allow 2D. diffusion at certain intervals, to avoid generating too
    // many 2D. Tables Keep only one sig fig for <0.1, 2 for 0.1<d<10, 3 for
    // 10<d<100, etc
    double dtmp;
  ...
  double RMax{3.5 * sqrt(4.0 * biMolData.Dtot * params.timeStep) +
  ...
  bool withinRmax{get_distance(
      ...
      RMax, complexList, forwardRxns[rxnIndex], moleculeList, membraneObject)};
  if (withinRmax) {
  ...
  if (moleculeList[biMolData.pro1Index].isDissociated == false &&
      moleculeList[biMolData.pro2Index].isDissociated == false) {
    ...
    if (withinRmax && forwardRxns[rxnIndex].rateList[rateIndex].rate > 0) {
      ...
      double ktemp{forwardRxns[rxnIndex].rateList[rateIndex].rate /
      ...
      for (int l = 0; l < DDTableIndex; ++l) {
        if (std::abs(tableIDs[l] - ktemp) < 1e-8 &&
            std::abs(tableIDs[params.max2DRxns + l] - biMolData.Dtot) < 1E-4) {
          probValExists = true;
          probMatrixIndex = l;
          break;
        }
      ...
      if (!probValExists) {
        ...
        size_t veclen{size_lookup(forwardRxns[rxnIndex].bindRadius,
                                  biMolData.Dtot, params, RMax)};
        // std::cout << "Create new 2D table: " << ktemp << ", Dtot: " <<
        // biMolData.Dtot << " size: " << veclen
        //           << '\n';
        ...
        create_DDMatrices(survMatrices[DDTableIndex],
                          normMatrices[DDTableIndex], pirMatrices[DDTableIndex],
                          forwardRxns[rxnIndex].bindRadius, biMolData.Dtot,
                          RMax, ktemp, params);
      ...
      if (sep < 0) {
        if (biMolData.com1Index != biMolData.com2Index) {
          ...
          // std::cout << "SEPARATION BETWEEN INTERFACE " << biMolData.relIface1
          // << " ON MOLECULE "
          //           << biMolData.pro1Index << " AND INTERFACE " <<
          //           biMolData.relIface2 << " ON MOLECULE "
          //           << biMolData.pro2Index << " IS LESS THAN 0\n";
          // std::cout << "separation: " << sep << " r1: " << R1 << " p1: " <<
          // biMolData.pro1Index
          //           << " p2: " << biMolData.pro2Index << " it " << simItr <<
          //           " i1: " << biMolData.absIface1
          //           << " i2: " << biMolData.absIface2 << '\n';
          // std::cout << "MOL1 COM: " <<
          // moleculeList[biMolData.pro1Index].comCoord
          //           << " freelist.size(): " <<
          //           moleculeList[biMolData.pro1Index].freelist.size() <<
          //           '\n';
          // std::cout << "IFACE1: "
          //           <<
          ...
          //           << '\n';
          // std::cout << "MOL2 COM: " <<
          // moleculeList[biMolData.pro2Index].comCoord
          //           << " freelist.size(): " <<
          //           moleculeList[biMolData.pro2Index].freelist.size() <<
          //           '\n';
          // std::cout << "IFACE2: "
          //           <<
          ...
          // "
          //              "Separation <0: "
          //           << sep << " r1 " << R1 << " p1: " << biMolData.pro1Index
          //           << " p2: " << biMolData.pro2Index
        ...
        if (moleculeList[proA].prevlist[s] == proB &&
          ...
          if (moleculeList[proA].prevsep[s] >= RMax) {
            ...
            p0_ratio = DDpirr_pfree_ratio_ps(
                pirMatrices[probMatrixIndex], survMatrices[probMatrixIndex],
                normMatrices[probMatrixIndex], R1, biMolData.Dtot,
                params.timeStep, moleculeList[proA].prevsep[s],
      ...
      rxnProb =
          get_prevSurv(survMatrices[probMatrixIndex], biMolData.Dtot,
                       params.timeStep, R1, forwardRxns[rxnIndex].bindRadius);
      ...
      if (rxnProb > 0.5) {
        ...
        // for a bimolecular binding with multiple binding sites, please use a
        // smaller time step." << std::endl;
      }
      ...
      //         prob = get_prevSurv(
      //             survMatrices[probMatrixIndex], biMolData.Dtot,
      //             params.timeStep, z, forwardRxns[rxnIndex].bindRadius);
