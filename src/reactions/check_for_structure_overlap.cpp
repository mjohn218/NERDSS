    ...
    if (molTemplateList[moleculeList[memMol].molTypeIndex].checkOverlap) {
      for (int memMol2 : reactCom2.memberList) {
        if (molTemplateList[moleculeList[memMol2].molTypeIndex].checkOverlap) {
          double dx = moleculeList[memMol].tmpComCoord.x -
                      moleculeList[memMol2].tmpComCoord.x;
          double dy = moleculeList[memMol].tmpComCoord.y -
                      moleculeList[memMol2].tmpComCoord.y;
          double dz = moleculeList[memMol].tmpComCoord.z -
                      moleculeList[memMol2].tmpComCoord.z;
          double r2 = dx * dx + dy * dy + dz * dz;
          if (r2 < (overlapTolerance)) {
            //   std::cout << "WARNING: Canceling association, complexes
            //   overlap. Distance COM-COM: " <<sqrt(r2)<<" mol1: "<<memMol<<"
            //   mol2 "<<memMol2<<"\n";
            cancelAssoc = true;
            return;
          }
