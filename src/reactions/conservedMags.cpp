...
bool conservedMags(const Complex& targCom,
  ...
  for (auto& memMol : targCom.memberList) {
    for (unsigned i{0}; i < moleculeList[memMol].tmpICoords.size(); ++i) {
      Vector tmpVec1{moleculeList[memMol].interfaceList[i].coord -
                     moleculeList[memMol].comCoord};
      Vector tmpVec2{moleculeList[memMol].tmpICoords[i] -
      ...
      if (roundv(tmpVec2.magnitude) != roundv(tmpVec1.magnitude)) {
        ...
        //           << " did not conserve its magnitude. Exiting..." <<
        //           std::endl;
        // std::cerr << "Mag tempVec2: " << roundv(tmpVec2.magnitude)
        //           << " Mag tempVec1: " << roundv(tmpVec1.magnitude) <<
        //           std::endl;
