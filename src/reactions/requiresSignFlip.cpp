...
bool requiresSignFlip(Vector axis, Vector v1, Vector v2) {
  ...
  // rotate
  Quat rot(cos(theta / 2), sin(theta / 2) * u.x, sin(theta / 2) * u.y,
           sin(theta / 2) * u.z);
  rot = rot.unit();
  rot.rotate(v1);
  rot.rotate(v2);
  ...
  if ((zAxis.dot_theta(axis) > 0.01 && !useXAxis) ||
      (useXAxis && xAxis.dot_theta(axis) < 0.01)) {
    rot = rot.inverse();
    rot.rotate(v1);
    rot.rotate(v2);
    rot = Quat(cos(-theta / 2), sin(-theta / 2) * u.x, sin(-theta / 2) * u.y,
               sin(-theta / 2) * u.z);
    rot.rotate(v1);
    rot.rotate(v2);
  }
