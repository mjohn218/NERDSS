...
Quat orient_crds_to_template(const MolTemplate& oneTemplate,
                             Molecule& targMol) {
  // TRACE();
  Quat firstRot{};
  Quat secondRot{};
  {  // first rotation
    // determine the interface-center of mass vector (v0, v1), the rotation
    // vector (u), and the angle to rotate (angle) arbitrarily the center of
    // mass to first interface of the MolTemplate
    ...
    firstRot =
        Quat{cos(angle / 2), sa * rotAxis.x, sa * rotAxis.y, sa * rotAxis.z};
    firstRot = firstRot.unit();
    {
      Vector tmpVec{targMol.tmpICoords[0] - targMol.tmpComCoord};
      firstRot.rotate(tmpVec);
      if (std::abs(tmpVec.x - oneTemplate.interfaceList[0].iCoord.x) > 1E-8 ||
          std::abs(tmpVec.y - oneTemplate.interfaceList[0].iCoord.y) > 1E-8 ||
          std::abs(tmpVec.z - oneTemplate.interfaceList[0].iCoord.z) > 1E-8) {
        angle = -angle;
        sa = sin(angle / 2);
  ...
  if (targMol.interfaceList.size() > 1) {
    // if the protein has more than one interface, use a second one to make sure
    // all the interfaces line up First check to make sure they're not in a
    // line. If so, use the last interface
    size_t ifaceIndex{1};
    {
      Vector ifaceVec1{oneTemplate.interfaceList[0].iCoord -
                       oneTemplate.comCoord};
      Vector ifaceVec2{oneTemplate.interfaceList[1].iCoord -
                       oneTemplate.comCoord};
      ifaceVec1.calc_magnitude();
      ifaceVec2.calc_magnitude();

      double ang1{ifaceVec1.dot_theta(ifaceVec2)};
      if ((ang1 == 0 || ang1 == M_PI) && !oneTemplate.isRod &&
          oneTemplate.interfaceList.size() > 2) {
        size_t tmpIndex{oneTemplate.interfaceList.size() - 1};
        Vector ifaceVec3{oneTemplate.interfaceList[tmpIndex].iCoord -
                         oneTemplate.comCoord};
    ...
    Vector v0{oneTemplate.interfaceList[ifaceIndex].iCoord -
              oneTemplate.comCoord};
    ...
    if (requiresSignFlip(rotAxis, projVec0, projVec1)) angle = -angle;

    double sa{std::sin(angle / 2)};
    secondRot = Quat(cos(angle / 2), sin(angle / 2) * rotAxis.x,
                     sin(angle / 2) * rotAxis.y, sin(angle / 2) * rotAxis.z);
    secondRot = secondRot.unit();

    //        {
    //            Vector tmpVec { targMol.tmpICoords[ifaceIndex] -
    //            targMol.tmpComCoord }; firstRot.rotate(tmpVec);
    //            if(std::abs(tmpVec.x -
    //            oneTemplate.interfaceList[ifaceIndex].iCoord.x) > 1E-8 ||
    //            std::abs(tmpVec.y -
    //            oneTemplate.interfaceList[ifaceIndex].iCoord.y) > 1E-8 ||
    //            std::abs(tmpVec.z -
    //            oneTemplate.interfaceList[ifaceIndex].iCoord.z) > 1E-8) {
    //                angle = -angle;
    //                sa = sin(angle/2);
    //                secondRot = Quat { cos(angle / 2), sa * rotAxis.x, sa *
    //                rotAxis.y,
    //                                  sa * rotAxis.z };
    //                secondRot = secondRot.unit();
    //            }
