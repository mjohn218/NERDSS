...
double calc_pirr(gsl_matrix* pirMatrix, gsl_matrix* survMatrix,
  ...
  // TRACE();
  // This is a 2D lookup table, decide the value of an arbitrary point (r,r0)
  // using 2D linear interpolation
  ...
  if (indexr0 == indexr) {
    ...
    // find the prob of coord (r0,r) that falls inside the square
    // (Ar0,Ar)-(Br0,Br)-(Cr0,Cr)-(Dr0,Dr)
