/**
 * @file associate_box.cpp
 * @author your name (you@domain.com)
 * @brief Perform the association event for each rank without the consideration
 * of the effect of other ranks. Some associations are unfeasible because the
 * restrait of other ranks. The solution is that make the associations are
 * temporary by storing the information of the association event in the
 * interface, including the id of the interface of both binding molecule, the id
 * of the molecule of its binding partner and the change of the coordinates for
 * the molecule and its complex.
 * @version 0.1
 * @date 2022-06-30
 *
...
/**
 * @brief Associates two interfaces.
 *
 * @param iter iteration steps.
 * @param ifaceIndex1 relative index of the first interface.
 * @param ifaceIndex2 relative index of the second interface.
 * @param reactMol1 first molecule.
 * @param reactMol2 second molecule.
 * @param reactCom1 first complex.
 * @param reactCom2 second complex.
 * @param params system parameters.
 * @param currRxn current binding reaction.
 * @param moleculeList list of all molecules.
 * @param molTemplateList list of all molecules templates.
 * @param observablesList list of all observables.
 * @param counterArrays struct used for the counting.
 * @param complexList list of all complex.
 * @param membraneObject struct storing the membrane information.
 * @param forwardRxns list of all forward reaction.
 * @param backRxns list of all backward reaction.
 */
void associate_box(long long int iter, int ifaceIndex1, int ifaceIndex2,
  ...
  if (reactCom1.index == reactCom2.index) {
    ...
    reactMol1.isAssociated = true;
    reactMol2.isAssociated = true;
    ...
    /* MOVE PROTEIN TO SIGMA */
    {
      ...
      if (DzSum < 1E-14) {
        isOnMembrane = true;
        /*Store coordinates of one protein to recover membrane-bound
         * orientation*/
        ...
        if (reactCom1.D.z < tol || reactCom2.D.z < tol) {
          transitionToSurface = true;  // both can't be less than tol, or would
                                       // not be in this loop.
      ...
      // std::cout << "Initial Position before any association movements: " <<
      // std::endl; reactMol1.display_assoc_icoords("mol1");
    ...
    com_of_two_tmp_complexes(reactCom1, reactCom1, afterSigmaCOM1,
                             moleculeList);
    Coord afterSigmaCOM2;
    com_of_two_tmp_complexes(reactCom2, reactCom2, afterSigmaCOM2,
                             moleculeList);

    // std::cout << "Diplace during pushing to sigma: " << std::endl;
    // std::cout << "COM1: " << std::endl;
    // std::cout << sqrt((afterSigmaCOM1.x - startCOM1.x) * (afterSigmaCOM1.x -
    // startCOM1.x) + (afterSigmaCOM1.y - startCOM1.y) * (afterSigmaCOM1.y -
    // startCOM1.y) + (afterSigmaCOM1.z - startCOM1.z) * (afterSigmaCOM1.z -
    // startCOM1.z)) << std::endl; std::cout << "COM2: " << std::endl; std::cout
    // << sqrt((afterSigmaCOM2.x - startCOM2.x) * (afterSigmaCOM2.x -
    // startCOM2.x) + (afterSigmaCOM2.y - startCOM2.y) * (afterSigmaCOM2.y -
    // startCOM2.y) + (afterSigmaCOM2.z - startCOM2.z) * (afterSigmaCOM2.z -
    // startCOM2.z)) << std::endl;

    if (molTemplateList[reactMol1.molTypeIndex].isPoint &&
        molTemplateList[reactMol2.molTypeIndex].isPoint) {
      ...
      if (!std::isnan(currRxn.assocAngles.theta1))
        theta_rotation(ifaceIndex1, ifaceIndex2, reactIface1, reactIface2,
                       reactMol1, reactMol2, currRxn.assocAngles.theta1,
      ...
      if (!std::isnan(currRxn.assocAngles.theta2))
        theta_rotation(ifaceIndex2, ifaceIndex1, reactIface2, reactIface1,
                       reactMol2, reactMol1, currRxn.assocAngles.theta2,
                       reactCom2, reactCom1, moleculeList);
      // else
      ...
      if (!std::isnan(currRxn.assocAngles.omega)) {
        omega_rotation(reactIface1, reactIface2, ifaceIndex1, ifaceIndex2,
                       reactMol1, reactMol2, reactCom1, reactCom2,
      ...
      if (!std::isnan(currRxn.assocAngles.phi1)) {
        phi_rotation(reactIface1, reactIface2, ifaceIndex1, ifaceIndex2,
                     reactMol1, reactMol2, reactCom1, reactCom2, currRxn.norm1,
      ...
      if (!std::isnan(currRxn.assocAngles.phi2)) {
        phi_rotation(reactIface2, reactIface1, ifaceIndex2, ifaceIndex1,
                     reactMol2, reactMol1, reactCom2, reactCom1, currRxn.norm2,
    ...
    com_of_two_tmp_complexes(reactCom1, reactCom1, afterRotateCOM1,
                             moleculeList);
    Coord afterRotateCOM2;
    com_of_two_tmp_complexes(reactCom2, reactCom2, afterRotateCOM2,
                             moleculeList);
    ...
    Coord finalCOM;
    com_of_two_tmp_complexes(
        reactCom1, reactCom2, finalCOM,
        moleculeList);  // com of c1+c2 (final (tmp) coordinates).
    ...
    if (isOnMembrane == true || transitionToSurface == true) {
      ...
      if (slowPro == reactMol1.index) {
        memRot = save_mem_orientation(memProtein, reactMol1,
                                      molTemplateList[reactMol1.molTypeIndex]);
        pivot = reactMol1.tmpComCoord;
        preCOM = afterSigmaCOM1;
      } else {
        memRot = save_mem_orientation(memProtein, reactMol2,
                                      molTemplateList[reactMol2.molTypeIndex]);
    ...
    Vector dtrans{};
    // std::cout << "POST-MEMBRANE ROT: COMPLEX PAIR COM: " << finalCOM.x << ' '
    // << finalCOM.y << ' ' << finalCOM.z << std::endl;
    //     reactMol1.display_assoc_icoords("mol1");
    //     reactMol2.display_assoc_icoords("mol2");

    Coord afterOrientationCOM1;
    com_of_two_tmp_complexes(reactCom1, reactCom1, afterOrientationCOM1,
                             moleculeList);
    Coord afterOrientationCOM2;
    com_of_two_tmp_complexes(reactCom2, reactCom2, afterOrientationCOM2,
                             moleculeList);
    ...
    */
      ...
      com_of_two_tmp_complexes(reactCom1, reactCom1, afterTranShiftCOM1,
                               moleculeList);
      Coord afterTranShiftCOM2;
      com_of_two_tmp_complexes(reactCom2, reactCom2, afterTranShiftCOM2,
                               moleculeList);
    ...
    */
      ...
      if (slowPro == reactMol1.index) {
        dispAngle = calc_one_angular_displacement(
            ifaceIndex2, reactMol2, reactCom2);  // pro2 is the one that moved.
        determine_rotation_angles(dispAngle, 0, rotAng, skip, reactCom1,
                                  reactCom2);  // positive angle

      } else {
        dispAngle = calc_one_angular_displacement(
            ifaceIndex1, reactMol1, reactCom1);  // pro1 is the one that moved
        determine_rotation_angles(0, dispAngle, skip, rotAng, reactCom1,
                                  reactCom2);  // positive angle
      }
      Coord origin{0.5 * (reactIface1 +
                          reactIface2)};  // halfway along the sigma vector, or
      ...
      Quat rotQuatPos(cos(rotAng / 2), sin(rotAng / 2) * rotAxis.x,
                      sin(rotAng / 2) * rotAxis.y, sin(rotAng / 2) * rotAxis.z);
      rotQuatPos = rotQuatPos.unit();

      // rotate the two complexes, using same angle here, so no orientations
      // should change!
      rotate(origin, rotQuatPos, reactCom1, moleculeList);
      rotate(origin, rotQuatPos, reactCom2, moleculeList);
      Coord afterRotShiftCOM1;
      com_of_two_tmp_complexes(reactCom1, reactCom1, afterRotShiftCOM1,
                               moleculeList);
      Coord afterRotShiftCOM2;
      com_of_two_tmp_complexes(reactCom2, reactCom2, afterRotShiftCOM2,
                               moleculeList);
    ...
    if (isOnMembrane == true || transitionToSurface == true) {
        ...
        if (moleculeList[mp].isLipid == true) {
          if (moleculeList[mp].tmpComCoord.z <
            ...
            double ztmp =
                ...
                moleculeList[mp]
                    .tmpComCoord
                    .z;  // lipid COM is below box bottom, here ztmp is positive
            if (ztmp > zchg) {
      ...
      if (reactCom1.D.z <
          tol) {  // for the implicit case, we need to compare the z-value of
                  // the 2D complex to keep it unchanged
        ...
        if (moleculeList[mp].isLipid == true) {
          if (moleculeList[mp].tmpComCoord.z <
            ...
            double ztmp = (-membraneObject.waterBox.z / 2.0) -
                          moleculeList[mp]
                              .tmpComCoord.z;  // lipid COM is below box bottom
            if (ztmp > zchg) {
              zchg = ztmp;  // largest dip below membrane
              isBelowBottom = true;
            }
    ...
    for (int mm = 0; mm < 3; mm++) traj[mm] = 0;
    ...
    reflect_traj_tmp_crds(params, moleculeList, reactCom1, traj, membraneObject,
                          0.0);  // uses tmpCoords to calculate traj.
    ...
    if (cancelAssoc == false) {
      ...
      if (cancelAssoc == true) counterArrays.nCancelSpanBox++;
    } else
      counterArrays
          .nCancelOverlapPartner++;  // true for structure overlap check.
    if (cancelAssoc == false) {
      ...
      if (cancelAssoc == true) counterArrays.nCancelOverlapSystem++;
    }
    if (cancelAssoc == false) {
    ...
    track_association_events(reactCom1, reactCom2, transitionToSurface,
                             isOnMembrane, counterArrays);
    ...
    for (auto memMol : reactCom2.memberList) {
      ...
      if (currRxn.rxnType != ReactionType::biMolStateChange) {
        moleculeList[memMol].myComIndex =
            reactCom1.index;  // update their complex index
    ...
    for (unsigned index = 0; index < molTemplateList.size(); index++) {
      if (molTemplateList[index].countTransition == true) {
        ...
        if (numEachMolPrevious2[index] - 1 >= 0)
          molTemplateList[index]
              .lifeTime[numEachMolPrevious2[index] - 1]
              .emplace_back(
                  (iter - lastNumberUpdateItrEachMolPrevious2[index]) *
                  Parameters::dt / 1E6);
      }
    }

    reactCom2.memberList
        .clear();  // clear the member list so the molecules don't get destroyed
    reactCom2.destroy(moleculeList, complexList);  // destroy the complex
    reactCom1.update_properties(
        moleculeList,
        molTemplateList);  // recalculate the properties of the first complex
    ...
    for (unsigned index = 0; index < molTemplateList.size(); index++) {
      if (molTemplateList[index].countTransition == true &&
        ...
        if (numEachMolPrevious1[index] - 1 >= 0) {
          molTemplateList[index]
              .lifeTime[numEachMolPrevious1[index] - 1]
              .emplace_back(
                  (iter - lastNumberUpdateItrEachMolPrevious1[index]) *
                  Parameters::dt / 1E6);
          complexList[reactMol1.myComIndex].lastNumberUpdateItrEachMol[index] =
              iter;
        }
    ...
    for (unsigned index = 0; index < molTemplateList.size(); index++) {
      if (molTemplateList[index].countTransition == true &&
        ...
        if (numEachMolPrevious1[index] - 1 >= 0)
          molTemplateList[index]
              .transitionMatrix[numEachMolPrevious1[index] - 1]
                               [numEachMolPrevious1[index] - 1] +=
              iter - Parameters::lastUpdateTransition[index] - 1;
        if (numEachMolPrevious2[index] - 1 >= 0)
          molTemplateList[index]
              .transitionMatrix[numEachMolPrevious2[index] - 1]
                               [numEachMolPrevious2[index] - 1] +=
              iter - Parameters::lastUpdateTransition[index] - 1;
        ...
        for (unsigned indexCom = 0; indexCom < complexList.size(); indexCom++) {
          if (indexCom != reactMol1.myComIndex) {
            if (complexList[indexCom].numEachMol[index] - 1 >= 0) {
              molTemplateList[index]
                  .transitionMatrix[complexList[indexCom].numEachMol[index] - 1]
                                   [complexList[indexCom].numEachMol[index] -
                                    1] +=
  ...
  reactMol1.interfaceList[ifaceIndex1].interaction.partnerIndex =
      reactMol2.index;
  reactMol2.interfaceList[ifaceIndex2].interaction.partnerIndex =
      reactMol1.index;
  reactMol1.interfaceList[ifaceIndex1].interaction.partnerId = reactMol2.id;
  reactMol2.interfaceList[ifaceIndex2].interaction.partnerId = reactMol1.id;
  reactMol1.interfaceList[ifaceIndex1].interaction.partnerIfaceIndex =
      ifaceIndex2;
  reactMol2.interfaceList[ifaceIndex2].interaction.partnerIfaceIndex =
      ifaceIndex1;
  if (currRxn.isReversible) {
    reactMol1.interfaceList[ifaceIndex1].interaction.conjBackRxn =
        currRxn.conjBackRxnIndex;
    reactMol2.interfaceList[ifaceIndex2].interaction.conjBackRxn =
        currRxn.conjBackRxnIndex;
  }

  reactMol1.interfaceList[ifaceIndex1].isBound = true;
  reactMol1.interfaceList[ifaceIndex1].index =
      currRxn.productListNew[0].absIfaceIndex;
  reactMol2.interfaceList[ifaceIndex2].isBound = true;
  reactMol2.interfaceList[ifaceIndex2].index =
      currRxn.productListNew[0].absIfaceIndex;
...

  {
    ...
    for (unsigned i{0}; i < reactMol1.freelist.size(); ++i) {
      if (reactMol1.freelist[i] == ifaceIndex1) tmpItr = i;
    }
  ...
  }
  {
    ...
    for (unsigned i{0}; i < reactMol2.freelist.size(); ++i) {
      if (reactMol2.freelist[i] == ifaceIndex2) tmpItr = i;
    }
  ...
  // Set probability of this protein to zero in all reactions so it doesn't try
  // to react again but the partners still will avoid overlapping.
  for (unsigned crossItr{0}; crossItr < reactMol2.crossbase.size();
       ++crossItr) {
