...
void generate_coordinates_for_restart(
    ...
    int numMolTemplateBeforeAdd, int numForwardRxnBdeforeAdd) {
  for (int molTemplateListIndex = numMolTemplateBeforeAdd;
       molTemplateListIndex < molTemplateList.size(); molTemplateListIndex++) {
    MolTemplate oneTemp{};
    oneTemp = molTemplateList[molTemplateListIndex];
    for (unsigned itr{0}; itr < oneTemp.copies; ++itr) {
      create_molecule_and_complex_for_restart(oneTemp, params, moleculeList,
...
void create_molecule_and_complex_for_restart(
  ...
  while (needsResampling) {
    moleculeList[newMolIndex] = initialize_molecule_for_restart(
        newMolIndex, params, createdMolTemp, membraneObject);
    needsResampling = moleculeOverlapsForRestart(
        params, moleculeList[newMolIndex], moleculeList, complexList,
...
Molecule initialize_molecule_for_restart(int index, Parameters& params,
  ...
  for (auto memMol : moleculeList) {
    ...
    for (auto& comMemMol : oneCom.memberList) {
      ...
      for (const auto& oneRxn : forwardRxns) {
        for (unsigned iface1Itr{0}; iface1Itr < createdMol.interfaceList.size();
             ++iface1Itr) {
          for (unsigned iface2Itr{0}; iface2Itr < partMol.interfaceList.size();
               ++iface2Itr) {
