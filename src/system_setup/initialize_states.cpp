...
void initialize_states(std::vector<Molecule>& moleculeList,
  ...
  for (auto& tmpMol : molTemplateList) {
    ...
    for (int i = 0; i < tmpLength; i++) {
      // std::cout << tmpMol.startingNumState.nameEachState[i] << ": " <<
      // tmpMol.startingNumState.numberEachState[i] << std::endl;
    }
    ...
    if (tmpLength > 0) {
      if (tmpMol.isImplicitLipid == true) {
        ...
        for (int i = 0; i < tmpLength; i++) {
          // parse the nameEachState, for the implicit case, only one interface
          // can exist
          std::string oneLine{tmpMol.startingNumState.nameEachState[i]};
          std::string buffer;
          tmpCopyNumbers.emplace_back(
              tmpMol.startingNumState.numberEachState[i]);
          ...
          for (auto molIterator = oneLine.begin(); molIterator != oneLine.end();
            ...
            if (isalnum(*molIterator))
              ...
              switch (*molIterator) {
                case '~': {
                  // if the character is '~' it indicates a state. add an iface
                  // to the vector with iface~state as the ifaceName and state
                  // as the state
                  ...
                  tmpStateName.emplace_back(
                      static_cast<char>(std::toupper(*molIterator)));
                  break;
                }
                case ',': {
                  std::cerr << "ERROR: Implicit molecule can only have one "
                               "interface. Exiting...\n.";
                  exit(1);
                  break;
                }
                default: {
                  std::cerr << "ERROR: Character " << *molIterator
                            << " is not valid in starting copy numbers for "
                               "each state. Exiting...\n.";
                  exit(1);
                }
              }
        ...
        for (int i = 0; i < tmpLength; i++) {
          ...
          for (int stateIndex = 0; stateIndex < stateNum; stateIndex++) {
            if (tmpMol.interfaceList[0].stateList[stateIndex].iden ==
                tmpStateName[i]) {
              membraneObject.numberOfFreeLipidsEachState[stateIndex] =
                  tmpCopyNumbers[i];
            }
          }
        }
            ...
            if (isalnum(*molIterator))
              ...
              switch (*molIterator) {
                case '~': {
                  ...
                  buffer += "~";
                  buffer += static_cast<char>(std::toupper(*molIterator));
                  tmpStateName.back().emplace_back(
                      static_cast<char>(std::toupper(*molIterator)));
                  break;
                }
                case ',': {
          ...
          for (int j = 0; j < tmpIndex; j++) {
            ...
            for (auto& oneState : tmpMol.interfaceList[ifaceItr].stateList) {
              if (oneState.iden == stateName) {
                stateItr = static_cast<int>(
                    &oneState - &tmpMol.interfaceList[ifaceItr].stateList[0]);
                break;
              }
            }

            moleculeList[molItr].interfaceList[ifaceItr].index =
                tmpMol.interfaceList[ifaceItr].stateList[stateItr].index;
            moleculeList[molItr].interfaceList[ifaceItr].relIndex = ifaceItr;
            moleculeList[molItr].interfaceList[ifaceItr].stateIden =
                tmpMol.interfaceList[ifaceItr].stateList[stateItr].iden;
