...
void initialize_paramters_for_implicitlipid_and_compartment_model(int& implicitlipidIndex, const Parameters& params, std::vector<ForwardRxn>& forwardRxns, std::vector<BackRxn>& backRxns, std::vector<TransmissionRxn>& transmissionRxns,
    ...
    Membrane& membraneObject)
{
    ...
    // calculate RS3D 
    if (systemIL == true || membraneObject.hasCompartment == true) { 
        ///////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            for (const auto& oneMol : oneRxn.reactantListNew) {
                ...
                if (oneRxn.reactantListNew.size() > 1) {
                    for (int tmpImplicitLipidStateIndex = 0; tmpImplicitLipidStateIndex < membraneObject.nStates; tmpImplicitLipidStateIndex++) {
                        if (moleculeList[implicitlipidIndex].interfaceList[0].index == oneMol.absIfaceIndex - tmpImplicitLipidStateIndex) {
        ...
        for (const auto& oneRxn : transmissionRxns) {
            membraneObject.RS3Dvect[RS3Dindex] = oneRxn.bindRadius;
            membraneObject.RS3Dvect[RS3Dindex + 100] = oneRxn.rateList[0].rate;
            membraneObject.RS3Dvect[RS3Dindex + 200] = 1.0 / 3.0 * (molTemplateList[oneRxn.reactantListNew[0].molTypeIndex].D.x + membraneObject.droplet.D)
                            + 1.0 / 3.0 * (molTemplateList[oneRxn.reactantListNew[0].molTypeIndex].D.y + membraneObject.droplet.D)
                            + 1.0 / 3.0 * (molTemplateList[oneRxn.reactantListNew[0].molTypeIndex].D.z + 0.0);
            membraneObject.RS3Dvect[RS3Dindex + 300] = membraneObject.RS3Dvect[RS3Dindex] * membraneObject.RS3Dvect[RS3Dindex + 100] * 2.0 / (membraneObject.RS3Dvect[RS3Dindex + 100] * 2.0 + 4 * M_PI * membraneObject.RS3Dvect[RS3Dindex] * membraneObject.RS3Dvect[RS3Dindex + 200]);
            // default that the first mol in reactantList is not "compartment()"
            membraneObject.RS3Dvect[RS3Dindex + 400] = oneRxn.reactantListNew[0].molTypeIndex;

            RS3Dindex += 1;
        } //only set up Compartment model if they exist.
    }
...
void initialize_paramters_for_implicitlipid_model(
  ...
  for (int mol{0}; mol < moleculeList.size(); ++mol) {
    int molTypeIndex = moleculeList[mol].molTypeIndex;
    if (molTemplateList[molTypeIndex].isImplicitLipid == true) {
      membraneObject.implicitLipid =
          true;  // determine membraneObject.implicitLipid according to
                 // molTemp.isImplicitLipid
      membraneObject.lipidLength = molTemplateList[molTypeIndex].radius;
      continue;
    }
    if (molTemplateList[molTypeIndex].D.z > 0) is2D = false;
  }
  if (is2D || membraneObject.waterBox.z == 0) membraneObject.TwoD = true;

  // find the mol.index of the implicit-lipid, which should be
  // implicitlipidIndex=0;
  bool systemIL = false;  // Are there any IL's in this system?
  for (int mol{0}; mol < moleculeList.size(); ++mol) {
    int molTypeIndex = moleculeList[mol].molTypeIndex;
    if (molTemplateList[molTypeIndex].isImplicitLipid == true) {
      ...
      implicitlipidIndex = mol;
      membraneObject.implicitlipidIndex = mol;
      // moleculeList[mol].mass = 1; // such a large value that implicit-lipid
      // won't move when proteins bind to it.
      systemIL = true;  // yes, there are IL's in the system.
    } else {
      moleculeList[mol].isImplicitLipid = false;
    }
  ...
  for (const auto& oneRxn : forwardRxns) {
    if (oneRxn.rxnType == ReactionType::bimolecular ||
        oneRxn.rxnType == ReactionType::biMolStateChange) {
      int molTypeIndex = oneRxn.reactantListNew[0].molTypeIndex;
      if (molTemplateList[molTypeIndex].isImplicitLipid == true) {
        int newProteinIndex = oneRxn.reactantListNew[1].molTypeIndex;
        molTemplateList[newProteinIndex].bindToSurface = true;
  ...
  if (membraneObject.isSphere) {
    ...
    membraneObject.totalSA =
        membraneObject.waterBox.x * membraneObject.waterBox.y;
  }
  ...
  if (systemIL == true) {
    ...
    membraneObject.nSites = 0;
    for (auto onetem : molTemplateList) {
      if (onetem.isImplicitLipid) membraneObject.nSites = onetem.copies;
      // here, we assume each implicit-lipid has only one interface.
    }
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////
    // initialize the RS3Dvect according to sigma, ka, and D;we accord RS3D in
    // Rs3Dvect[i+300] and the reactmol.typeindex in RS3Dvect[i+400]
    int RS3Dindex{0};
    // const auto& implicitLipidStateList =
    // molTemplateList[moleculeList[membraneObject.implicitlipidIndex].molTypeIndex].interfaceList[0].stateList;
    for (const auto& oneRxn : forwardRxns) {
      for (const auto& oneMol : oneRxn.reactantListNew) {
        // for (auto& implicitLipidState : implicitLipidStateList) {
        if (oneRxn.reactantListNew.size() > 1) {
          for (int tmpImplicitLipidStateIndex = 0;
            ...
            if (moleculeList[implicitlipidIndex].interfaceList[0].index ==
                oneMol.absIfaceIndex - tmpImplicitLipidStateIndex) {
              membraneObject.RS3Dvect[RS3Dindex] = oneRxn.bindRadius;
              membraneObject.RS3Dvect[RS3Dindex + 100] =
                  oneRxn.rateList[0].rate;
              membraneObject.RS3Dvect[RS3Dindex + 200] =
                  1.0 / 3.0 *
                      (molTemplateList[oneRxn.reactantListNew[0].molTypeIndex]
                           .D.x +
                  ...
                  1.0 / 3.0 *
                      (molTemplateList[oneRxn.reactantListNew[0].molTypeIndex]
                       ...
                       molTemplateList[oneRxn.reactantListNew[1].molTypeIndex]
                           .D.z);
              membraneObject.RS3Dvect[RS3Dindex + 300] =
                  membraneObject.RS3Dvect[RS3Dindex] *
                  membraneObject.RS3Dvect[RS3Dindex + 100] * 2.0 /
                  (membraneObject.RS3Dvect[RS3Dindex + 100] * 2.0 +
                   4 * M_PI * membraneObject.RS3Dvect[RS3Dindex] *
                       membraneObject.RS3Dvect[RS3Dindex + 200]);
              if (molTemplateList[oneRxn.reactantListNew[0].molTypeIndex]
                      .isImplicitLipid == true)
                membraneObject.RS3Dvect[RS3Dindex + 400] =
                    oneRxn.reactantListNew[1].molTypeIndex;
              else
                membraneObject.RS3Dvect[RS3Dindex + 400] =
                    oneRxn.reactantListNew[0].molTypeIndex;
              RS3Dindex += 1;
            }
          }
        }
  ...
  // std::cout << "membraneObject.RS3Dvect[300]: " << std::setprecision(20) <<
  // membraneObject.RS3Dvect[300] << std::endl; exit(1);
}
