 ...
 *
 * ## TODO:
 *  - Change freelist from unordered list of relative iface indices to ordered
 * list of absolute indices. really speed up evaluate_binding_pair
 *  - Write species tracker (tracks connectivity)
 *  - Compress reflect_traj_complex_rad_rot, reflect_traj_check_span,
 * reflect_traj_rad_rot_nocheck
 */
...
int main(int argc, char* argv[]) {
  ...
  // use random_device instead of time so that multiple jobs started at the same
  // time have more unique seeds
  ...
  if (!params.fromRestart && paramFile != "") {
    ...
    parse_input(paramFile, params, observablesList, forwardRxns, backRxns,
                createDestructRxns, transmissionRxns, molTemplateList,
                membraneObject);
    ...
    std::cout << "NUMBER OF MOLECULE TYPES: " << params.numMolTypes
              ...
              << params.numTotalSpecies << std::endl;
    ...
    if (observablesList.size() == 1) {
      ...
      for (auto obsItr = observablesList.begin();
           obsItr != observablesList.end(); ++obsItr)
    ...
    initialize_paramters_for_implicitlipid_and_compartment_model(
        implicitlipidIndex, params, forwardRxns, backRxns, transmissionRxns,
    ...
    for (auto& molTemp : molTemplateList) {
      if (molTemp.countTransition == true) {
        ...
        for (int indexOne = 0; indexOne < molTemp.transitionMatrixSize;
             ++indexOne) {
          molTemp.transitionMatrix[indexOne].resize(
              molTemp.transitionMatrixSize);
        }
      }
      ...
      //     for (int indexOne = 0; indexOne < molTemp.transitionMatrixSize;
      //     ++indexOne) {
      //         for (int indexTwo = 0; indexTwo < molTemp.transitionMatrixSize;
      //         ++indexTwo) {
      //             std::cout <<' '<<
      //             molTemp.transitionMatrix[indexOne][indexTwo];
      //         }
      //         std::cout << std::endl;
      //     }
      // }
    ...
    if (addFileNameInput != "") {
      ...
      numCreatDestructRxnBdeforeAdd =
          static_cast<int>(createDestructRxns.size());
      ...
      parse_input_for_add(addFileNameInput, params, observablesList,
      ...
      for (int forwardReactIndex{0}; forwardReactIndex < forwardRxns.size();
           forwardReactIndex++) {
        if (forwardReactIndex < numForwardRxnBdeforeAdd) {
          if (forwardRxns[forwardReactIndex].rxnType ==
      ...
      for (int backReactIndex{0}; backReactIndex < backRxns.size();
           backReactIndex++) {
    ...
    std::cout
        << " Total number of states (reactant and product)  in the system "
    ...
    std::cout << " Total number of molecules: " << Molecule::numberOfMolecules
              << " Size of molecule list : " << moleculeList.size()
              << std::endl;
    ...
    std::cerr << "Please provide a parameter and/or restart file. Parameter "
                 "file Syntax is : ./rd_executable.exe "
                 "-f parameterfile.inp \n";
    exit(1);
  }
  ...
  // the variable params.numIfaces does not seem to be used anywhere for
  ...
  for (simItr += 1; simItr < params.nItr; ++simItr) {
    ...
    //     molTemplateList, observablesList, counterArrays, membraneObject);

    // Update member lists after creation and destruction
    //        simulVolume.update_memberMolLists(params, moleculeList,
    //        complexList, molTemplateList, membraneObject, simItr);

    // Zeroth order reactions (creation)
    check_for_zeroth_order_creation(
        simItr, params, simulVolume, forwardRxns, createDestructRxns,
        moleculeList, complexList, molTemplateList, observablesList,
        counterArrays, membraneObject);
    ...
    for (unsigned cellItr{0}; cellItr < simulVolume.subCellList.size();
      ...
      for (unsigned memItr{0};
        ...
        if (moleculeList[targMolIndex].freelist.size() > 0 ||
          ...
          // Check for binding to the compartment in / or out. there must be a
          // reaction type that involves binding/unbinding from the compartment
          // surface. Implementation might be very similar to implicit lipid.
          if (membraneObject.hasCompartment == true) {
            // only consider single molecule
            if (complexList[moleculeList[targMolIndex].myComIndex]
                    .memberList.size() == 1) {
              check_compartment_reaction(
                  targMolIndex, -1, simItr, params, moleculeList, complexList,
                  molTemplateList, transmissionRxns, backRxns, counterArrays,
                  membraneObject, IL2DbindingVec, IL2DUnbindingVec, ILTableIDs);
            }
          }
    ...
    for (int molItr{0}; molItr < moleculeList.size(); ++molItr) {
      ...
      // only monomer is allowed to transmit!
      if (moleculeList[molItr].transmissionProb > 0.0 &&
          complexList[moleculeList[molItr].myComIndex].memberList.size() == 1) {
        ...
        if (moleculeList[molItr].transmissionProb > rand1) {
          ...
          perform_transmission_reaction(molItr, moleculeList, complexList,
                                        ...
                                        counterArrays, params, transmissionRxns,
                                        forwardRxns, simulVolume);
          continue;
        } else {
          // enforce boundary conditions
          moleculeList[molItr].enforceCompartmentBC = true;
        }
      }
      ...
      if (moleculeList[molItr].crossbase.size() > 0) {
        ...
        /*determine if reaction occurs has to consider reactions across the
         * compartment surface*/
        ...
        if (willReact) {
          ...
          if (forwardRxns[rxnIndex[0]].rxnType == ReactionType::bimolecular) {
            if (moleculeList[molItr2].isImplicitLipid) {
              // std::cout << "Performing binding of molecules " << molItr << "
              ...
              //           << ")] with [reaction, rate] = [" << rxnIndex[0] <<
              //           ',' << rxnIndex[1]
              //           << "] Rate: " <<
              //           forwardRxns[rxnIndex[0]].rateList[rxnIndex[1]].rate
              //           << " at iteration " << simItr << ".\n";
              // std::cout << " Complex 1 size: " <<
              // complexList[moleculeList[molItr].myComIndex].memberList.size()
              // << "\n"; std::cout << "Implicit lipid, my Com Index: " <<
              // moleculeList[molItr2].myComIndex << " size of memberlist: " <<
            ...
            if (moleculeList[molItr2].isImplicitLipid == false) {
              ...
              //           molTemplateList[moleculeList[molItr].molTypeIndex].interfaceList[ifaceIndex1].name
              //           << ") and " <<
              //           molTemplateList[moleculeList[molItr2].molTypeIndex].molName
              //           << "("
              //           <<
              ...
              if (moleculeList[molItr].interfaceList[ifaceIndex1].index ==
                ...
                associate(simItr, ifaceIndex1, ifaceIndex2,
                          moleculeList[molItr], moleculeList[molItr2],
                          complexList[moleculeList[molItr].myComIndex],
                          complexList[moleculeList[molItr2].myComIndex], params,
            ...
            if (moleculeList[molItr2].isImplicitLipid) {
              ...
              // figure out if the current molecule is the facilitator molecule
              // or the IL which has its interface state changed
              ...
              // std::cout << "Performing bimolecular state change on molecule
              // (IL) " << stateMolIndex
              //           << " as facilitated by molecule " << facilMolIndex <<
              //           " at iteration " << simItr
              //           << '\n';
              perform_implicitlipid_state_change(
                  ...
                  complexList, molTemplateList, observablesList,
                  membraneObject);
            }
            if (moleculeList[molItr2].isImplicitLipid == false) {
              // In this case, after two molecules collide, at least one of them
              // changes state, rather than forming a complex.
              int facilMolIndex{molItr};
              int facilIfaceIndex{ifaceIndex1};
              int facilComIndex{moleculeList[molItr].myComIndex};
              int stateMolIndex{molItr2};
              ...
              int stateIfaceIndex{ifaceIndex2};
              // figure out if the current molecule is the facilitator molecule
              // or the molecule which has its interface state changed
              if (moleculeList[molItr].molTypeIndex !=
                  forwardRxns[rxnIndex[0]].reactantListNew[0].molTypeIndex) {
                facilMolIndex = molItr2;
                facilIfaceIndex = ifaceIndex2;
            ...
            std::cerr << "ERROR: Attemping bimolecular reaction which has no "
                         "reaction type. Exiting..\n";
            exit(1);
          }
        } else {
    ...
    if (params.transitionWrite != -1) {
      if (simItr % params.transitionWrite == 0) {
        ...
        write_transition((simItr - params.itrRestartFrom) * params.timeStep *
                                 Constants::usToSeconds +
                             params.timeRestartFrom,
                         transitionFile, molTemplateList);
        transitionFile.close();
      }
    }
    ...
    if (simItr % params.timeWrite == 0) {
      ...
      if (!observablesList.empty()) {
        ...
        write_observables((simItr - params.itrRestartFrom) * params.timeStep *
                                  Constants::usToSeconds +
                              params.timeRestartFrom,
                          observablesFile, observablesList);
        observablesFile.close();
      }
      // write all species
      ...
      std::cout
          ...
          << " minutes\n";
      auto estTimeLeft = std::chrono::time_point_cast<std::chrono::seconds>(
          MDTimer::now() + timeLeft);
